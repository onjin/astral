...
void violence_update( void )
{
	...
	if ( IS_NPC(ch) ) 
	{
		...
		if ( 50 + (ch->level/4) > number_percent( ) )
		{
		    ...
		    {
			case DFND_CURELIGHT:
			    act( AT_MAGIC, "$n wypowiada kilka slow i ... wyglada odrobine lepiej.", ch, NULL, NULL, TO_ROOM );
			    retcode = spell_smaug( skill_lookup( "cure light" ), ch->level, ch, ch );
			    break;
			case DFND_CURESERIOUS:
			    act( AT_MAGIC, "$n wypowiada kilka slow ... i wyglada znacznie lepiej.", ch, NULL, NULL, TO_ROOM );
			    retcode = spell_smaug( skill_lookup( "cure serious" ), ch->level, ch, ch );
			    break;
			case DFND_CURECRITICAL:
			    act( AT_MAGIC, "$n wypowiada kilka slow ... i wyglada zdrowiej.", ch, NULL, NULL, TO_ROOM );
			    retcode = spell_smaug( skill_lookup( "cure critical" ), ch->level, ch, ch );
			    break;
			case DFND_HEAL:
			    act( AT_MAGIC, "$n wypowiada kilka slow ... i wyglada duzo zdrowiej.", ch, NULL, NULL, TO_ROOM );
			    retcode = spell_smaug( skill_lookup( "heal" ), ch->level, ch, ch );
			    break;
			case DFND_DISPELMAGIC:
			    act( AT_MAGIC, "$n wypowiada zaklecie...", ch, NULL, NULL, TO_ROOM );
			...
			case DFND_SHOCKSHIELD:
			    ...
			    {
				act( AT_MAGIC, "$n wypowiada kilka slow...", ch, NULL, NULL, TO_ROOM );
...
ch_ret damage( CHAR_DATA *ch, CHAR_DATA *victim, int dam, int dt )
{
	    ...
	    || (anopc > 0 && !IS_NPC(victim)))
	{
		 ...
		 */
		act(AT_ACTION, "$n odchodzi z grupy $C.",
		...
		!IS_PKILL(ch->leader))
	    {
		act(AT_ACTION, "$n odchodzi z grupy $C.", ch, NULL,
	    ...
	    {
		act(AT_ACTION, "$n odchodzi z grupy $C.", victim, NULL,
		...
		for (gch = victim->in_room->first_person; gch; gch = gch->next_in_room)
		    ...
		    {
			act(AT_ACTION, "$n odchodzi z grupy $C.", gch, NULL,
			    gch->master, TO_NOTVICT);
			stop_follower(gch);
		    }
	    }
 ...
 */
	...
	if (IS_AFFECTED(ch, AFF_INVISIBLE))
	{
	    ...
	    act(AT_MAGIC, "$n pojawia sie znikad.", ch, NULL, NULL, TO_ROOM);
	}
...
Flash */
	...
	send_to_char("Wydajesz gluchy pomruk, gdy budzi sie w Tobie bestia!\n\r", victim);
      }
    }
	...
	sprintf( buf, "$n jest smiertelnie rann%s i umrze jesli nikt %s nie pomoze.",
	( victim->sex == SEX_MALE ) ? "y" : ( victim->sex == SEX_FEMALE ) ? "a" : "e",
	( victim->sex == SEX_FEMALE ) ? "jej" : "mu" );
	act( AT_DYING, buf, victim, NULL, NULL, TO_ROOM );
	sprintf( buf, "Jestes smiertelnie rann%s i umrzesz jesli nikt Ci nie pomoze",
	( victim->sex == SEX_MALE ) ? "y" : ( victim->sex == SEX_FEMALE ) ? "a" : "e" );
	act( AT_DYING, buf, victim, NULL, NULL, TO_CHAR );
	break;

    case POS_INCAP:

	sprintf( buf, "$n jest niezdoln%s do niczego i umrze bez czyjejs pomocy.",
	( victim->sex == SEX_MALE ) ? "y" : ( victim->sex == SEX_FEMALE ) ? "a" : "e" );
	act( AT_DYING, buf, victim, NULL, NULL, TO_ROOM );
	sprintf( buf, "Jestes niezdoln%s do niczego i umrzesz bez czyjejs pomocy.",
	( victim->sex == SEX_MALE ) ? "y" : ( victim->sex == SEX_FEMALE ) ? "a" : "e" );
	act( AT_DYING, buf, victim, NULL, NULL, TO_CHAR );
	break;
    ...
    case POS_STUNNED:
        ...
        {
	    sprintf( buf, "$n jest ogluszon%s, ale prawdopodobnie wyjdzie z tego.", 
	    ( victim->sex == SEX_MALE ) ? "y" : ( victim->sex == SEX_FEMALE ) ? "a" : "e" );
	    act( AT_ACTION, buf, victim, NULL, NULL, TO_ROOM );
	    sprintf( buf, "Jestes ogluszon%s, ale prawdopodobnie wyjdziesz z tego.",
	    ( victim->sex == SEX_MALE ) ? "y" : ( victim->sex == SEX_FEMALE ) ? "a" : "e" );
	    act( AT_ACTION, buf, victim, NULL, NULL, TO_ROOM );
	}
	break;
	...
	act( AT_DEAD, "$n nie ZYJE!!", victim, NULL, NULL, TO_ROOM );
	sprintf( buf, "Zostal%ss ZABIT%s!!\n\r", ( victim->sex == SEX_FEMALE ) ? "a" : "e",
	( victim->sex == SEX_MALE ) ? "Y" : ( victim->sex == SEX_FEMALE ) ? "A" : "E" );
	act( AT_DEAD, buf, victim, NULL, NULL, TO_CHAR );
	break;

    default:
	if ( dam > victim->max_hit / 4 )
	{
	   act( AT_HURT, "Auc, to naprawde boli!", victim, 0, 0, TO_CHAR );
	   if ( number_bits(3) == 0 )
	...
	{
	   act( AT_DANGER, "Zyczysz sobie by twe rany przestaly KRWAWIC tak mocno!",
		victim, 0, 0, TO_CHAR );
	   if ( number_bits(2) == 0 )
		worsen_mental_state( ch, 1 );
	}
	...
	if ( !npcvict )
	{
	    ...
	    if( !IS_NPC(ch) && !IS_NPC(victim) ) /* Flash  info o killu */
	    { 
	        sprintf( buf, "zabil%s %s z zimna krwia!", 
		(ch->sex == 2) ? "am" : "em",
		IS_NPC( victim ) ? victim->short_descr : victim->nbie);
	    if ( ch == victim )
		sprintf(buf, " ");
	    else if ( IS_PKILL(ch) )
		check_virtual_channels( ch, "pk", buf );
	    else
		do_yell( ch, buf );
	    }
/* Flash koniec info o killu */
...
bool is_safe( CHAR_DATA *ch, CHAR_DATA *victim )
{
	...
	send_to_char( "Magiczne sily powstrzymuja Cie przed atakiem.\n\r", ch );
	return TRUE;
    }
    ...
    {
    	set_char_color(AT_MAGIC, ch);
    	ch_printf(ch, "Jestes pacyfista i nie bedziesz walczyc.\n\r");
	return TRUE;
    }
    ...
    {
        char buf[MAX_STRING_LENGTH];
        sprintf(buf, "%s jest pacyfista i nie bedzie walczyc.\n\r",
    ...
    &&   IS_SET( victim->in_room->area->flags, AFLAG_NOPKILL ) ) 
    {
        ...
        send_to_char( "Bogowie nie zezwalaja na zabijanie graczy w tej krainie.\n\r", ch );
        return TRUE;
    }
	...
	send_to_char( "Brakuje Ci jeszcze doswiadczenie lub wieku lub obu tych rzeczy naraz. \n\r", ch );
	return TRUE;
    }
    ...
    {
	set_char_color( AT_WHITE, ch );
	send_to_char( "Hmm... ta istota jest zbbyt mloda by umrzec.\n\r", ch );
	return TRUE;
    }
    ...
    {
	set_char_color( AT_IMMORT, ch );
	send_to_char( "Bogowie nie pozwalaja Ci na walke przy takiej roznicy poziomow.\n\r", ch );
	return TRUE;
    }
	...
	set_char_color( AT_GREEN, ch );
        send_to_char( "Ta postac umarla w ciagu ostatnich 5 minut.\n\r", ch);
        return TRUE;
    }
    ...
    {
	set_char_color( AT_GREEN, ch );
        send_to_char( "Zmarlo Ci sie w ciagu ostatnich 5 minut.\n\r", ch );
        return TRUE;
    }
...
void check_killer( CHAR_DATA *ch, CHAR_DATA *victim )
{
	...
	if ( victim != ch )
	{
	  act( AT_MAGIC, "$n przejmuje energie zyciowa z pokonanego przeciwnika!", ch, victim->name, NULL, TO_ROOM );
	  act( AT_MAGIC, "Przejmujesz energie zyciowa z pokonanego przeciwnika!", ch, victim->name, NULL, TO_CHAR ); 
	}
	if ( victim->pcdata->clan )
	{
            if ( ch->level < 10 )
    ...
    set_char_color( AT_IMMORT, ch );
    send_to_char( "Slyszysz grzmiacy glos, 'Od tej chwili wszyscy wiedza, ze jestes MORDERCA!!!'\n\r", ch );
...
void update_pos( CHAR_DATA *victim )
{
    ...
    if ( victim->mount )
    {
	act( AT_ACTION, "$n spada z $C bez przytomnosci.",
		victim, NULL, victim->mount, TO_ROOM );
	xREMOVE_BIT( victim->mount->act, ACT_MOUNTED );
	victim->mount = NULL;
    }
...
void set_fighting( CHAR_DATA *ch, CHAR_DATA *victim )
{
    ...
    {
	send_to_char( "Zbyt wiele osob juz walczy by mozna bylo sie przylaczyc.\n\r", ch );
	return;
    }
    ...
    {
	send_to_char( "Cos Ci przeszkadza!\n\r", victim->switched );
...
char* part_messages[] =
{
	"Ostatni cios odcina glowe $x.",
	"Wykanczajacy cios odcina reke $x.",
	"Wykanczajacy cios odcina noge $x.",
	"Potezny atak wyrywa serce z $c.",
	"Mozg wylewa sie z rozrabanej glowy $c.",
	"Uderzenie wypruwa flaki z $c.",
	"Ostatni cios odcina dlon $x.",
	"Ostatni cios odcina stope $x.",
	"Wykanczajacy cios odcina palec.",
	"Ucho $c zostaje odciete do glowy.",
	"Cios wylupia $x oko.",
	"Jezyk $c zostaje wyrwany z ust.",
...
void death_cry( CHAR_DATA *ch )
{
    ...
    {
    default: msg  = "Slyszysz smiertelne zezenie $c.";				break;
    case  0:
    case  1: msg  = "$n pada na ziemie... TRUPEM.";	break;
    case  2:
    case  3: msg  = "$n rozbryzguje swa krew w promieniu 3 metrow.";	break;
    case  4:
    case  5:
    	...
    	if(!msg)
    		msg = "Slyszysz agonalne zezenie $c.";
    	break;
    }
	...
	name		= IS_NPC(ch) ? ch->short_descr_dop : ch->ndop;
    ...
    if ( IS_NPC(ch) )
	msg = "Slyszysz czyjs smiertelny skowyt.";
    else
	msg = "Slyszysz czyjs agonalny skowyt.";
...
Flash */
    ...
    {
        bug( "raw_kill: killing level 1", 0 );
        return;
    }
 ...
 /* death_cry( victim ); */
    ...
    ||   victim->in_room->sector_type == SECT_WATER_NOSWIM )
      act( AT_BLOOD, "Krew $c powoli rozplywa sie w wodzie.", victim, NULL, NULL, TO_ROOM );
    else if ( victim->in_room->sector_type == SECT_AIR )
      act( AT_BLOOD, "Krew $c rozbryzguje sie w powietrzu.", victim, NULL, NULL, TO_ROOM );
    else
      make_blood( victim );
...
neutral when they die given the difficulting of changing align */
    ...
    if ( xIS_SET( victim->act, PLR_KILLER) )
    {
      ...
      send_to_char("Bogowie wybaczaja Ci Twe morderstwa.\n\r",victim);
    }
    if ( xIS_SET( victim->act, PLR_THIEF) )
    {
      xREMOVE_BIT( victim->act, PLR_THIEF);
      send_to_char("Bogowie wybaczaja Ci Twe zlodziejskie postepki.\n\r",victim);
    }
...
void group_gain( CHAR_DATA *ch, CHAR_DATA *victim )
{
	...
	if ( gch->level - lch->level >  8 )
	{
        ...
        if (xIS_SET(ch->act, PLR_QUESTOR)&&IS_NPC(victim))
        {
            ...
            {
		send_to_char("Twoja misja zostala ukonczona!\n\r",ch);
                send_to_char("Szybko wracaj do swego zleceniodawcy na czas!\n\r",ch);
                ch->questmob = -1;
            }
        }
...
void new_dam_message( CHAR_DATA *ch, CHAR_DATA *victim, int dam, int dt, OBJ_DATA *obj )
{
    ...
    if ( dt == TYPE_HIT )
    {
	sprintf( buf1, "$n %s $B%c",  vp, punct );
	sprintf( buf2, "%s $B%c", vs, punct );
	sprintf( buf3, "$n %s Cie%c", vp, punct );
    }
    else
	...
	if ( skill )
	{
	    ...
	    if ( attack[strlen(attack) -1] == 'a')
		fprefix = "Twoja";
	    else if ( attack[strlen(attack) -1] == 'e'
		   || attack[strlen(attack) -1] == 'i'
		   || attack[strlen(attack) -1] == 'o')
		fprefix = "Twoje";
	    else
		fprefix = "Twoj";
	...
	sprintf( buf1, "%s $c %s $B%c",  attack, vp, punct );
	sprintf( buf2, "%s %s %s $B%c", fprefix, attack, vp, punct );
	sprintf( buf3, "%s $c %s Cie%c", attack, vp, punct );
    }
...
void do_kill( CHAR_DATA *ch, char *argument )
{
    ...
    {
	send_to_char("Ta kreatura wyglada cokolwiek dziwnie.  Przyjzyj jej sie dokladniej, nim ja zaatakujesz.", ch);
	return;
    }
	...
	&&   !xIS_SET(victim->act, PLR_THIEF) )
	{
	    send_to_char( "Musisz uzyc komendy MURDER.\n\r", ch );
	    return;
	}
    }
    ...
    *
	...
	{
	    send_to_char( "By zabic zawladnieta kreature uzyj MURDER.\n\r", ch );
	    return;
	}
    }
    *
    */
    ...
    if ( victim == ch )
    {
	send_to_char( "Trafiasz siebie.  Auc!\n\r", ch );
    ...
    if (IS_NPC(ch) && !IS_NPC(victim) ) /* Flash charmmob no kills players */
	send_to_char("Boskie Moce powstrzymuja Cie przed tym posunieciem.\n\r", ch);
...
void do_murder( CHAR_DATA *ch, char *argument )
{
    ...
    if (IS_NPC(ch) && !IS_NPC(victim) )
    {
    	send_to_char("Boskie Moce powstrzymuja Cie przed tym czynem.\n\r", ch);
	return;
    }
    if ( IS_AFFECTED(ch, AFF_CHARM) ) 
    {
...
*/
    ...
    WAIT_STATE( ch, 1 * PULSE_VIOLENCE );
    sprintf( buf, "Pomocy!  %s mnie atakuje!", 
                   IS_NPC( ch ) ? ch->short_descr : ch->name );
    if ( IS_PKILL(victim) )
	check_virtual_channels( victim, "pk", buf );
    else
	do_yell( victim, buf );
