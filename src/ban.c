...
void dispose_ban ( BAN_DATA *pban , int type)
{
  ...
  first = (type == BAN_SITE) ? first_ban: ( type==BAN_CLASS) ? first_ban_class : first_ban_race;
  last = (type == BAN_SITE) ? last_ban: ( type==BAN_CLASS) ? last_ban_class : last_ban_race;
  ...
  UNLINK( pban, 
    ...
    next, prev );
    */
  UNLINK( pban, first, last, next, prev );
  free_ban( pban );
  return;
}
