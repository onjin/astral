...
"nosupplicate", "arena", "nomissile", "noquest","prototype"
};
...
"antidemon", "nolocate", "groundrot", "noquest", "questobject" };
...
char *	const	w_flags_pl	[] =
{
"take", "palec", "szyje", "cialo", "glowe", "nogi", "stopy", "dlonie", "ramiona",
"shield", "about", "pas", "nadgarstek", "wield", "hold", "_dual_", "uszy", "oczy",
"missile", "plecy", "twarz", "kostke", "r4","r5","r6",
"r7","r8","r9","r10","r11","r12","r13"
};

char *	const	area_flags	[] =
{
"nopkill", "freekill", "noteleport", "noquest", "r4", "r5", "r6", "r7", "r8",
...
"pacifist", "noattack", "annoying", "noquest", "prototype", "r14"
};
...
void do_goto( CHAR_DATA *ch, char *argument )
{
	...
	act( AT_IMMORT, "$T", ch, NULL,
	    ...
	    ? ch->pcdata->bamfout : "Bog wraca do swej siedziby.",  TO_ROOM );
	...
	act( AT_IMMORT, "$T", ch, NULL,
	    ...
	    ? ch->pcdata->bamfin : "Bog pojawia sie przed Toba.", TO_ROOM );
...
void do_mset( CHAR_DATA *ch, char *argument )
{
	...
	case SUB_MOB_DESC:
	  ...
	  if ( char_died(victim) )
	  {
		send_to_char( "Twoja ofiara zmarla!\n\r", ch );
		stop_editing( ch );
		return;
	  }
	...
	||   !str_cmp( argument, "stat" ) )
	{
		...
		do_mstat( ch, victim->name );
	        ...
	        send_to_char( "Nie wybrales ofiary.  Wpisz '?' by uzyskac pomoc.\n\r", ch );
	    return;
	}
    ...
    if ( !str_cmp( arg1, "on" ) )
    {
	send_to_char( "Uzycie: mset <ofiara|vnum> on.\n\r", ch );
	return;
    }
	...
	{
	    if ( victim )
		send_to_char( "Uzycie: <pole>  <wartosc>\n\r",		ch );
	    else
		send_to_char( "Uzycie: <ofiara> <pole>  <wartosc>\n\r",	ch );
	}
	else
	    send_to_char( "Uzycie: mset <ofiara> <pole>  <wartosc>\n\r",	ch );
	send_to_char( "\n\r",						ch );
	...
	send_to_char( "  name short long description title spec clan\n\r", ch );
        send_to_char( "  council deity questpoints questnext questlevel\n\r",ch );
        send_to_char( "  password rank mentalstate emotion\n\r",	ch );
        send_to_char( "  favor r i s ri rs ris\n\r",	ch );
	send_to_char( "\n\r",						ch );
	...
	send_to_char( "Zmiana flagi do krainki: aloaded\n\r",ch);
	send_to_char( "Zmiana flagi pkill: pkill\n\r",ch);
	send_to_char( "Odmiana moba:\n\r",ch);
	send_to_char( "  shortd shortc shortb shortn shortm shortw\n\r",ch);
	return;
    }
	...
	ch_printf( ch, "Mset mode on. (Edytuje %s).\n\r",
		victim->name );
	...
	if ( value < minattr || value > maxattr )
	{
	    ch_printf( ch, "Zakres inteligencji %d - %d.\n\r", minattr, maxattr );
	    return;
	}
	...
	if ( value < minattr || value > maxattr )
	{
	    ch_printf( ch, "Zakres madrosci %d - %d.\n\r", minattr, maxattr );
	    return;
	}
	...
	if ( value < minattr || value > maxattr )
	{
	    ch_printf( ch, "Zakres zrecznosci %d - %d.\n\r", minattr, maxattr );
	    return;
	}
	...
	if ( value < minattr || value > maxattr )
	{
	    ch_printf( ch, "Zakres kondycji %d - %d.\n\r", minattr, maxattr );
	    return;
	}
	...
	if ( value < minattr || value > maxattr )
	{
	    ch_printf( ch, "Zakres charyzmy %d - %d.\n\r", minattr, maxattr );
	    return;
	}
	...
	if ( value < minattr || value > maxattr )
	{
	    ch_printf( ch, "Zakres szczescia %d - %d.\n\r", minattr, maxattr );
	    return;
	}
	...
	if ( value < 0 )
	{
	    send_to_char( "The current quest range is 0 to nieskonczonosc :)).\n\r", ch );
	    return;
	}
	...
	{
	    send_to_char( "The current quest point range is 0 to 500.\n\r", ch );
	    return;
	}

	victim->questlevel = value;
	return;
    }
    ...
    if ( !str_cmp( arg2, "shortd" ) )
    {
	STRFREE( victim->short_descr_dop );
	victim->short_descr_dop = STRALLOC( arg3 );
	if ( IS_NPC(victim) && xIS_SET(victim->act, ACT_PROTOTYPE) )
	{
	   STRFREE( victim->pIndexData->short_descr_dop );
	   victim->pIndexData->short_descr_dop = QUICKLINK( victim->short_descr_dop );
	}
	return;
    }
    if ( !str_cmp( arg2, "shortc" ) )
    {
	STRFREE( victim->short_descr_cel );
	victim->short_descr_cel = STRALLOC( arg3 );
	if ( IS_NPC(victim) && xIS_SET(victim->act, ACT_PROTOTYPE) )
	{
	   STRFREE( victim->pIndexData->short_descr_cel );
	   victim->pIndexData->short_descr_cel = QUICKLINK( victim->short_descr_cel );
	}
	return;
    }
    if ( !str_cmp( arg2, "shortb" ) )
    {
	STRFREE( victim->short_descr_bie );
	victim->short_descr_bie = STRALLOC( arg3 );
	if ( IS_NPC(victim) && xIS_SET(victim->act, ACT_PROTOTYPE) )
	{
	   STRFREE( victim->pIndexData->short_descr_bie );
	   victim->pIndexData->short_descr_bie = QUICKLINK( victim->short_descr_bie );
	}
	return;
    }
    if ( !str_cmp( arg2, "shortn" ) )
    {
	STRFREE( victim->short_descr_nar );
	victim->short_descr_nar = STRALLOC( arg3 );
	if ( IS_NPC(victim) && xIS_SET(victim->act, ACT_PROTOTYPE) )
	{
	   STRFREE( victim->pIndexData->short_descr_nar );
	   victim->pIndexData->short_descr_nar = QUICKLINK( victim->short_descr_nar );
	}
	return;
    }
    if ( !str_cmp( arg2, "shortm" ) )
    {
	STRFREE( victim->short_descr_mie );
	victim->short_descr_mie = STRALLOC( arg3 );
	if ( IS_NPC(victim) && xIS_SET(victim->act, ACT_PROTOTYPE) )
	{
	   STRFREE( victim->pIndexData->short_descr_mie );
	   victim->pIndexData->short_descr_mie = QUICKLINK( victim->short_descr_mie );
	}
	return;
    }
    if ( !str_cmp( arg2, "shortw" ) )
    {
	STRFREE( victim->short_descr_wol );
	victim->short_descr_wol = STRALLOC( arg3 );
	if ( IS_NPC(victim) && xIS_SET(victim->act, ACT_PROTOTYPE) )
	{
	   STRFREE( victim->pIndexData->short_descr_wol );
	   victim->pIndexData->short_descr_wol = QUICKLINK( victim->short_descr_wol );
	}
	return;
    }
...
void do_oset( CHAR_DATA *ch, char *argument )
{
	...
	if ( IS_OBJ_STAT(obj, ITEM_PROTOTYPE) )
	{
	   ...
	   obj->short_descr_dop = STRALLOC( arg3 );
	   STRFREE(obj->pIndexData->short_descr_dop );
	   obj->pIndexData->short_descr_dop = QUICKLINK( obj->short_descr_dop );
	}
        else
        /* Feature added by Narn, Apr/96 
	 * If the item is not proto, add the word 'rename' to the keywords
	...
	if ( IS_OBJ_STAT(obj, ITEM_PROTOTYPE) )
	{
	   ...
	   obj->short_descr_cel = STRALLOC( arg3 );
	   STRFREE(obj->pIndexData->short_descr_cel );
	   obj->pIndexData->short_descr_cel = QUICKLINK( obj->short_descr_cel );
	}
        else
        /* Feature added by Narn, Apr/96 
	 * If the item is not proto, add the word 'rename' to the keywords
	...
	if ( IS_OBJ_STAT(obj, ITEM_PROTOTYPE) )
	{
	   ...
	   obj->short_descr_bie = STRALLOC( arg3 );
	   STRFREE(obj->pIndexData->short_descr_bie );
	   obj->pIndexData->short_descr_bie = QUICKLINK( obj->short_descr_bie );
	}
        else
        /* Feature added by Narn, Apr/96 
	 * If the item is not proto, add the word 'rename' to the keywords
	...
	if ( IS_OBJ_STAT(obj, ITEM_PROTOTYPE) )
	{
	   ...
	   obj->short_descr_nar = STRALLOC( arg3 );
	   STRFREE(obj->pIndexData->short_descr_nar );
	   obj->pIndexData->short_descr_nar = QUICKLINK( obj->short_descr_nar );
	}
        else
        /* Feature added by Narn, Apr/96 
	 * If the item is not proto, add the word 'rename' to the keywords
	...
	if ( IS_OBJ_STAT(obj, ITEM_PROTOTYPE) )
	{
	   ...
	   obj->short_descr_mie = STRALLOC( arg3 );
	   STRFREE(obj->pIndexData->short_descr_mie );
	   obj->pIndexData->short_descr_mie = QUICKLINK( obj->short_descr_mie );
	}
        else
        /* Feature added by Narn, Apr/96 
	 * If the item is not proto, add the word 'rename' to the keywords
	...
	if ( IS_OBJ_STAT(obj, ITEM_PROTOTYPE) )
	{
	   ...
	   obj->short_descr_wol = STRALLOC( arg3 );
	   STRFREE(obj->pIndexData->short_descr_wol );
	   obj->pIndexData->short_descr_wol = QUICKLINK( obj->short_descr_wol );
	}
        else
        /* Feature added by Narn, Apr/96 
	 * If the item is not proto, add the word 'rename' to the keywords
...
void do_aassign( CHAR_DATA *ch, char *argument )
{
	...
	if ( argument[0] == '\0' )
	{
	    send_to_char( "Uzycie: aassign <nazwapliku.are>\n\r", ch );
	    return;
	}
	...
	||   !str_cmp( "clear", argument ) )
	{
	    ...
	    assign_area( ch );
	    if ( !ch->pcdata->area )
	      send_to_char( "Wskaznik krainki wyczyszczony.\n\r", ch );
	    else
	      send_to_char( "Oryginalnie przyporzadkowana krainka przywrocona.\n\r", ch );
	    return;
	} 
...
/*		if ( get_trust(ch) >= sysdata.level_modify_proto  */
		...
		{
		    send_to_char( "Nie masz dostepu do tej krainki.\n\r", ch );
		    return;
		}
	    }
	...
	{
	    if ( get_trust(ch) >= sysdata.level_modify_proto )
		send_to_char( "Nie ma takiej krainki.  Uzyj 'zones'.\n\r", ch );
	    else
		send_to_char( "Nie ma takiej krainki.  Uzyj 'newzones'.\n\r", ch );
	    return;
	}
	ch->pcdata->area = tarea;
	ch_printf( ch, "Przypisuje Ci: %s\n\r", tarea->name );
	return;
}
...
void fold_area( AREA_DATA *tarea, char *filename, bool install )
{
    ...
    sprintf( buf, "Nagrywam %s...", tarea->filename );
	...
	fprintf( fpout,	"%s~\n",	pMobIndex->short_descr_dop	);
	fprintf( fpout,	"%s~\n",	pMobIndex->short_descr_cel	);
	fprintf( fpout,	"%s~\n",	pMobIndex->short_descr_bie	);
	fprintf( fpout,	"%s~\n",	pMobIndex->short_descr_nar	);
	fprintf( fpout,	"%s~\n",	pMobIndex->short_descr_mie	);
	fprintf( fpout,	"%s~\n",	pMobIndex->short_descr_wol	);
...
/* Need to convert to new positions correctly on loadup sigh -Shaddai */
	...
	fprintf( fpout, "%s~\n",	pObjIndex->short_descr_dop	);
	fprintf( fpout, "%s~\n",	pObjIndex->short_descr_cel	);
	fprintf( fpout, "%s~\n",	pObjIndex->short_descr_bie	);
	fprintf( fpout, "%s~\n",	pObjIndex->short_descr_nar	);
	fprintf( fpout, "%s~\n",	pObjIndex->short_descr_mie	);
	fprintf( fpout, "%s~\n",	pObjIndex->short_descr_wol	);
