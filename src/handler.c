...
void obj_from_room( OBJ_DATA *obj )
{
	...
	write_corpses( NULL, obj->short_descr+6, obj );
    return;
}
...
OBJ_DATA *obj_to_room( OBJ_DATA *obj, ROOM_INDEX_DATA *pRoomIndex )
{
    ...
    falling--;
    if ( obj->pIndexData->vnum == OBJ_VNUM_CORPSE_PC && falling < 1 )
	write_corpses( NULL, obj->short_descr+6, NULL );
    return obj;
}
...
char *extra_bit_name( EXT_BV *extra_flags )
{
    ...
    if ( xIS_SET(*extra_flags, ITEM_ANTI_DEMON)) strcat( buf, " anti-demon" );
...
void name_stamp_stats( CHAR_DATA *ch )
{
    ...
    for ( x = strlen(ch->ncel); x > -1 ; x-- ) /* Flash zmiany */
    {
	c = ch->ncel[x] + x - 1;
	b = c % 14;
	a = (c % 1) + 1;
...
OBJ_DATA *clone_object( OBJ_DATA *obj )
{
    ...
    clone->short_descr_dop	= QUICKLINK( obj->short_descr_dop );
    clone->short_descr_cel	= QUICKLINK( obj->short_descr_cel );
    clone->short_descr_bie	= QUICKLINK( obj->short_descr_bie );
    clone->short_descr_nar	= QUICKLINK( obj->short_descr_nar );
    clone->short_descr_mie	= QUICKLINK( obj->short_descr_mie );
    clone->short_descr_wol	= QUICKLINK( obj->short_descr_wol );
...
*/
    ...
    &&   QUICKMATCH( obj1->short_descr_dop,	obj2->short_descr_dop )
    &&   QUICKMATCH( obj1->short_descr_cel,	obj2->short_descr_cel )
    &&   QUICKMATCH( obj1->short_descr_bie,	obj2->short_descr_bie )
    &&   QUICKMATCH( obj1->short_descr_nar,	obj2->short_descr_nar )
    &&   QUICKMATCH( obj1->short_descr_mie,	obj2->short_descr_mie )
    &&   QUICKMATCH( obj1->short_descr_wol,	obj2->short_descr_wol )
...
CHAR_DATA *clone_mobile_nested( CHAR_DATA *mob )
{
    ...
    clone->short_descr_dop              = QUICKLINK( mob->short_descr_dop );
    clone->short_descr_cel              = QUICKLINK( mob->short_descr_cel );
    clone->short_descr_bie              = QUICKLINK( mob->short_descr_bie );
    clone->short_descr_nar              = QUICKLINK( mob->short_descr_nar );
    clone->short_descr_mie              = QUICKLINK( mob->short_descr_mie );
    clone->short_descr_wol              = QUICKLINK( mob->short_descr_wol );
