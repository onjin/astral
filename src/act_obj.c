...
void get_obj( CHAR_DATA *ch, OBJ_DATA *obj, OBJ_DATA *container )
{
    ...
    {
	send_to_char( "Nie mozesz tego podniesc.\n\r", ch );
	return;
    }
    /* Flash , questowych rzeczy nie zabierze ktos inny */

    if ( xIS_SET(obj->extra_flags, ITEM_QUEST ) && str_cmp( ch->name , obj->action_desc ) && ch->level < 56 )
	{
	send_to_char("Magiczne Moce Sensei Nazono nie pozwalaja Ci podniesc tego przedmiotu\n\r", ch );
	return;
	}
	...
	if ( CAN_PKILL( ch ) && !get_timer( ch, TIMER_PKILLED ) )
	{
	    ...
	    {
		send_to_char_color( "\n\r&bBoskie Sily nie pozwalaja Ci podniesc tego przedmiotu.\n\r", ch );
		return;
	    }
	    else
	    {
	...
	{
	    send_to_char_color( "\n\r&BBoskie sily nie pozwalaja Ci podniesc tego przedmmiotu.\n\r", ch );
 	    return;
	}
    }
    ...
    &&  !can_take_proto( ch ) )
    {
	send_to_char( "Boskie Sily nie pozwalaja Ci tego podniesc.\n\r", ch );
	return;
    }

    if ( ch->carry_number + get_obj_number( obj ) > can_carry_n( ch ) )
    {
	act( AT_PLAIN, "$d: nie mozesz uniesc tylu przedmiotow.",
    ...
    {
	act( AT_PLAIN, "$d: nie mozesz uniesc takiego ciezaru.",
	    ch, NULL, obj->name, TO_CHAR );
	return;
    }
	...
	{
	    act( AT_ACTION, "Zdejmujesz $v z  $F.", ch, obj, container, TO_CHAR );
	    act( AT_ACTION, "$n zdejmuje $v z $F.", ch, obj, container, TO_ROOM );
	}
	else
	{
	    act( AT_ACTION, IS_OBJ_STAT(container, ITEM_COVERING) ? 
		"Bierzesz $v spod $F." : "Wyciagasz $v z $F.",
		ch, obj, container, TO_CHAR );
	    act( AT_ACTION, IS_OBJ_STAT(container, ITEM_COVERING) ?
		"$n bierze $v spod $F." : "$n wyciaga $v z $F.",
    ...
    {
	act( AT_ACTION, "Podnosisz $v.", ch, obj, container, TO_CHAR );
	act( AT_ACTION, "$n podnosi $v.", ch, obj, container, TO_ROOM );
	obj_from_room( obj );
    }
...
void do_get( CHAR_DATA *ch, char *argument )
{
	...
	if ( number <= 1 && str_cmp( arg1, "all" ) && str_prefix( "all.", arg1 ) )
	{
	    ...
	    if ( !obj )
	    {
		act( AT_PLAIN, "Nie widzisz tu czegos takiego.", ch, NULL, arg1, TO_CHAR );
		return;
	    }
	    separate_obj(obj);
	    get_obj( ch, obj, NULL );
	...
	else
	{
	    ...
	    {
		send_to_char( "Bogowie zle patrza na taka chciwosc!\n\r", ch );
		return;
	    }
	    if ( !str_cmp(arg1, "all") )
		fAll = TRUE;
	    ...
	    {
		if ( fAll )
		  send_to_char( "Nic tu nie widzisz.\n\r", ch );
		else
		  act( AT_PLAIN, "Nie widzisz tu czegos takiego.", ch, NULL, chk, TO_CHAR );
	    }
	    else
	    if ( IS_SET( sysdata.save_flags, SV_GET ) )
		save_char_obj( ch );
	...
	{
	    act( AT_PLAIN, "Nie widzisz tu czegos takiego.", ch, NULL, arg2, TO_CHAR );
	    return;
	}
	...
	default:
	    ...
	    {
		send_to_char( "To jest zbyt ciezkie by moc to podniesc.\n\r", ch );
		return;
	    }
	    break;
...
/*		pd = one_argument( pd, name );
		...
		&& str_cmp( name, ch->ncel ) )
		{
		     send_to_char( "Nie mozesz okrasc tego ciala... jeszcze.\n\r", ch );
		     return;
		}
		...
		{
		    send_to_char( "Nie ty zadales smiertelny cios.\n\r", ch );
		    return;
		}

		if ( IS_OBJ_STAT( container, ITEM_CLANCORPSE )
		&&  !IS_NPC(ch) && str_cmp( name, ch->ncel )
		&&   container->value[5] >= 3 )
		{
		     send_to_char( "Bogowie nie pozwalaja wiecej okradac tego ciala.\n\r", ch);
		     return;
		}
		...
		if ( str_cmp( name, ch->ncel ) && !IS_IMMORTAL(ch) )
		{
		    bool fGroup;
		    ...
		    if ( !fGroup )
		    {
			send_to_char( "To nie jest Twoje cialo.\n\r", ch );
			return;
		    }
		}
	    }
	...
	{
	    act( AT_PLAIN, "$d trzeba otworzyc.", ch, NULL, container->name, TO_CHAR );
	    return;
	}
	...
	if ( number <= 1 && str_cmp( arg1, "all" ) && str_prefix( "all.", arg1 ) )
	{
	    ...
	    || ( obj->pIndexData->vnum == OBJ_VNUM_MONEY_ONE ) ) )
	    {
		tax = ( obj->value[0] / 100 * ch->pcdata->clan->clantax );
		ch->gold += obj->value[0];
		sprintf( buf, "Wyciagasz %d szt. zlota z %s.\n\r", obj->value[0], 
		container->short_descr_dop );
		send_to_char( buf, ch );
		sprintf( buf, "$n wyciaga %d szt. zlota z %s.", obj->value[0],
		container->short_descr_dop );
		...
		write_corpses( NULL, container->short_descr+6, NULL );
...
void do_put( CHAR_DATA *ch, char *argument )
{
    ...
    {
	send_to_char( "Co wlozyc i do czego?\n\r", ch );
	return;
    }
    ...
    if ( (container=get_obj_here(ch, arg2)) == NULL )
    {
	act( AT_PLAIN, "Nie widze tu czegos takiego.", ch, NULL, arg2, TO_CHAR );
	return;
    }
	...
	if ( ch->carry_weight + container->weight > can_carry_w(ch) )
	{
	    send_to_char( "To jest zbyt ciezkie.\n\r", ch );
	    return;
	}
    }
    else
	...
	{
	    act( AT_PLAIN, "$d trzeba najpierw otworzyc.", ch, NULL, container->name, TO_CHAR );
	    return;
	}
    }
	...
	{
	    send_to_char( "Nie posiadasz czegos takiego.\n\r", ch );
	    return;
	}

	if ( obj == container )
	{
	    send_to_char( "Hmmm...niby jak?.\n\r", ch );
	    return;
	}
	...
	{
	    send_to_char( "To nie jest klucz.\n\r", ch );
	    return;
	}
	...
	&&   (get_obj_weight(obj) / obj->count)
	  ...
	  -   container->weight)) )
	{
	    send_to_char( "To sie tam nie zmniesci.\n\r", ch );
	    return;
	}
	...
	if ( (get_real_obj_weight(obj) / obj->count)
	   ...
	   >  container->value[0] )
	{
	    send_to_char( "To sie tam nie zmiesci.\n\r", ch );
	    return;
	}
	...
	if ( container->item_type == ITEM_KEYRING && !IS_OBJ_STAT(container, ITEM_COVERING) )
	{
	    act( AT_ACTION, "$n wklada $v na $V.", ch, obj, container, TO_ROOM );
	    act( AT_ACTION, "Wkladasz $v na $V.", ch, obj, container, TO_CHAR );
	}
	else
	{
	    act( AT_ACTION, IS_OBJ_STAT(container, ITEM_COVERING)
		? "$n chowa $v pod $Y." : "$n wklada $v do $F.",
		ch, obj, container, TO_ROOM );
	    act( AT_ACTION, IS_OBJ_STAT(container, ITEM_COVERING)
		? "Chowasz $v pod $Y." : "Wkladasz $v do $F.",
		ch, obj, container, TO_CHAR );
	}
	obj->count = count;
...
/*	   if (!char_died && !save_char ) save_char_obj(ch); */
	...
	for ( obj = ch->first_carrying; obj; obj = obj_next )
	{
		...
		{
		    act( AT_ACTION, "$n zaklada $v na $V.", ch, obj, container, TO_ROOM );
		    act( AT_ACTION, "Zakladasz $v na $V.", ch, obj, container, TO_CHAR );
		}
		else
		{
		    act( AT_ACTION, "$n wklada $v do $F.", ch, obj, container, TO_ROOM );
		    act( AT_ACTION, "Wkladasz $v do $F.", ch, obj, container, TO_CHAR );
		}
		obj = obj_to_obj( obj, container );
		found = TRUE;
	...
	{
	    if ( fAll )
	      act( AT_PLAIN, "Nie masz nic przy sobie.",
		    ch, NULL, NULL, TO_CHAR );
	    else
	      act( AT_PLAIN, "Nie masz przy sobie czegos takiego.",
		    ch, NULL, chk, TO_CHAR );
	    return;
	}
...
void do_drop( CHAR_DATA *ch, char *argument )
{
    ...
    if ( arg[0] == '\0' )
    {
	send_to_char( "Co chcesz wyrzucic?\n\r", ch );
	return;
    }
    ...
    if ( xIS_SET( ch->act, PLR_LITTERBUG ) )
    {
       ...
       send_to_char( "Boskie sily zabraniaja Ci cokolwiek wyrzucic...\n\r", ch );
       return;
    }
    ...
    &&   ch != supermob )
    {
       ...
       send_to_char( "Magiczne sily powstrzymuja Cie przed tym!\n\r", ch );
       set_char_color( AT_TELL, ch );
       send_to_char( "Ktos mowi Ci, 'Nie zanieczyszczaj tego pomieszczenia!'\n\r", ch );
       return;
    }
	...
	if ( !str_cmp( arg, "coins" ) || !str_cmp( arg, "coin" ) )
	{
	    ...
	    act( AT_ACTION, "$n wyrzuca troche zlota.", ch, NULL, NULL, TO_ROOM );
	...
	separate_obj( obj );
	act( AT_ACTION, "$n wyrzuca $v.", ch, obj, NULL, TO_ROOM );
	act( AT_ACTION, "Wyrzucasz $v.", ch, obj, NULL, TO_CHAR );
...
/*	   if (!char_died) save_char_obj(ch); */
	...
	{
	    send_to_char( "Nie mozesz tego tutaj uczynic...\n\r", ch );
	    return;
	}
	found = FALSE;
	for ( obj = ch->first_carrying; obj; obj = obj_next )
...
void do_give( CHAR_DATA *ch, char *argument )
{
	...
	{
	    send_to_char( "Niestety, nie mozesz tego uczynic.\n\r", ch );
	    return;
	}
	...
	if ( ch->gold < amount )
	{
	    send_to_char( "Twa szczodrosc mnie zadziwia, lecz nie posiadasz tyle zlota.\n\r", ch );
	    return;
	}
	...
	victim->gold += amount;
        ...
        strcat(buf, arg1);
        strcat(buf, (amount > 1) ? " szt. zlota." : " zloty krazek.");

	act( AT_ACTION, buf, ch, NULL, victim, TO_VICT    );
	act( AT_ACTION, "$n daje $X troche zlota.",  ch, NULL, victim, TO_NOTVICT );
	act( AT_ACTION, "Dajesz $X troche zlota.",  ch, NULL, victim, TO_CHAR    );
	send_to_char( "OK.\n\r", ch );
    ...
    {
	send_to_char( "Musisz to najpierw zdjac z siebie.\n\r", ch );
	return;
    }
    ...
    {
	act( AT_PLAIN, "$N ma juz pelne rece.", ch, NULL, victim, TO_CHAR );
	return;
    }

    if ( victim->carry_weight + (get_obj_weight(obj)/obj->count) > can_carry_w( victim ) )
    {
	act( AT_PLAIN, "$N nie moze nic wiecej uniesc.", ch, NULL, victim, TO_CHAR );
	return;
    }

    if ( !can_see_obj( victim, obj ) )
    {
	act( AT_PLAIN, "$N nie widzi tego.", ch, NULL, victim, TO_CHAR );
	return;
    }

    if (IS_OBJ_STAT( obj, ITEM_PROTOTYPE ) && !can_take_proto( victim ) )
    {
	act( AT_PLAIN, "Nie mozesz tego dac $X!", ch, NULL, victim, TO_CHAR );
	return;
    }

    separate_obj(obj);
    obj_from_char(obj);
    act(AT_ACTION, "$n daje $v $X.", ch, obj, victim, TO_NOTVICT);
    act(AT_ACTION, "$n daje Ci $v.",   ch, obj, victim, TO_VICT   );
    act(AT_ACTION, "Dajesz $v $X.", ch, obj, victim, TO_CHAR   );
...
obj_ret damage_obj( OBJ_DATA *obj )
{
	...
	act( AT_OBJECT, "($p - uszkodzone)", ch, obj, NULL, TO_CHAR );
    else
    if ( obj->in_room && ( ch = obj->in_room->first_person ) != NULL )
    {
	act( AT_OBJECT, "($p - uszkodzone)", ch, obj, NULL, TO_ROOM );
	act( AT_OBJECT, "($p - uszkodzone)", ch, obj, NULL, TO_CHAR );
	ch = NULL;
    }
...
bool remove_obj( CHAR_DATA *ch, int iWear, bool fReplace )
{
    ...
    {
	act( AT_PLAIN, "Nie mozesz zdjac $f.", ch, obj, NULL, TO_CHAR );
	return FALSE;
    }
    ...
    act( AT_ACTION, "$n przestaje uzywac $f.", ch, obj, NULL, TO_ROOM );
    act( AT_ACTION, "Przestajesz uzywac $f.", ch, obj, NULL, TO_CHAR );
...
bool can_dual( CHAR_DATA *ch )
{
    ...
    {
	send_to_char( "Uzywasz juz dwoch broni!\n\r", ch );
	return FALSE;
    }
    if ( get_eq_char( ch, WEAR_SHIELD ) )
    {
	send_to_char( "Nie mozesz uzywac dwoch broni gdy trzymasz tarcze!\n\r", ch );
	return FALSE;
    }
    if ( get_eq_char( ch, WEAR_HOLD ) )
    {
	send_to_char( "Nie mozesz uzywac dwoch broni gdy trzymasz cos w dloni!\n\r", ch );
	return FALSE;
    }
    return TRUE;
}
...
void wear_obj( CHAR_DATA *ch, OBJ_DATA *obj, bool fReplace, sh_int wear_bit )
{
    ...
    {
	sprintf( buf, "Musisz miec poziom %d by moc uzywac ten przedmiot.\n\r",
	    obj->level );
	send_to_char( buf, ch );
	act( AT_ACTION, "$n probuje uzyc $v, lecz ma zbyt malo doswiadczenia.",
	    ch, obj, NULL, TO_ROOM );
	return;
    }
    ...
    {
	act( AT_MAGIC, "Ty nie mozesz uzywac tego przedmiotu.", ch, NULL, NULL, TO_CHAR );
	act( AT_ACTION, "$n probuje uzyc $v, lecz nie moze tego uczynic.",
	    ch, obj, NULL, TO_ROOM );
	return;
    }
	...
	if ( !CAN_WEAR(obj, 1 << bit) )
	{
		...
		{
		    case ITEM_HOLD:
			send_to_char( "Nie mozesz tego trzymac w dloni.\n\r", ch );
			break;
		    case ITEM_WIELD:
		    case ITEM_MISSILE_WIELD:
			send_to_char( "Nie mozesz tym walczyc.\n\r", ch );
			break;
		    default:
			sprintf( buf, "Nie mozesz tego zalozyc na %s.\n\r",
				w_flags_pl[bit] );
			send_to_char( buf, ch );
		}
	    }
	...
	if ( !remove_obj( ch, WEAR_LIGHT, fReplace ) )
        ...
        {
	    act( AT_ACTION, "$n unosi $v jako swiatlo.", ch, obj, NULL, TO_ROOM );
	    act( AT_ACTION, "Unosisz $v jako swiatlo.",  ch, obj, NULL, TO_CHAR );
        }
    ...
    {
	if ( fReplace )
	    send_to_char( "Nie mozesz tego zalozyc, ani tym walczyc ani tego trzymac.\n\r", ch );
	return;
    }
	...
	case ITEM_WEAR_FINGER:
	    ...
	    if ( !get_eq_char( ch, WEAR_FINGER_L ) )
	    {
                if ( !oprog_use_trigger( ch, obj, NULL, NULL, NULL ) )
                {
		    act( AT_ACTION, "$n wsuwa lewy palec w $v.",    ch, obj, NULL, TO_ROOM );
		    act( AT_ACTION, "Wsuwasz lewy palec w $v.",  ch, obj, NULL, TO_CHAR );
                }
	    ...
	    if ( !get_eq_char( ch, WEAR_FINGER_R ) )
	    {
                if ( !oprog_use_trigger( ch, obj, NULL, NULL, NULL ) )
                {
		    act( AT_ACTION, "$n wsuwa prawy palec w $v.",   ch, obj, NULL, TO_ROOM );
		    act( AT_ACTION, "Wsuwasz prawy palec w $v.", ch, obj, NULL, TO_CHAR );
                }
	    ...
	    send_to_char( "Masz juz cos zalozone na palcach.\n\r", ch );
	    return;
	...
	case ITEM_WEAR_NECK:
	    ...
	    if ( !get_eq_char( ch, WEAR_NECK_1 ) )
	    {
                if ( !oprog_use_trigger( ch, obj, NULL, NULL, NULL ) )
                {
		  act( AT_ACTION, "$n zaklada $v wokol swej szyi.",   ch, obj, NULL, TO_ROOM );
		  act( AT_ACTION, "Zakladasz $v wokol swej szyi.", ch, obj, NULL, TO_CHAR );
                }
	    ...
	    if ( !get_eq_char( ch, WEAR_NECK_2 ) )
	    {
                ...
                {
		  act( AT_ACTION, "$n zaklada $v wokol swej szyi.",   ch, obj, NULL, TO_ROOM );
		  act( AT_ACTION, "Zakladasz $p wokol swej szyi.", ch, obj, NULL, TO_CHAR );
                }
	    ...
	    send_to_char( "Nic wiecej nie zmiesci sie na Twej szyi.\n\r", ch );
	    return;
	...
	*/
	    ...
	    {
		send_to_char( "Tego nie da sie wlozyc na to, co juz nosisz.\n\r", ch );
		return;
	    }
            if ( !oprog_use_trigger( ch, obj, NULL, NULL, NULL ) )
            {
		act( AT_ACTION, "$n ubiera $v.",   ch, obj, NULL, TO_ROOM );
		act( AT_ACTION, "Ubierasz $v.", ch, obj, NULL, TO_CHAR );
            }
	...
	/*    if ( !remove_obj( ch, WEAR_HEAD, fReplace ) )
		return; Flash */
	    if ( !can_layer( ch, obj, WEAR_HEAD ) ) /*Flash */
	    {
		send_to_char( "Tego nie da sie wlozyc na to, co juz nosisz.\n\r", ch );
		return;
	    }
            if ( !oprog_use_trigger( ch, obj, NULL, NULL, NULL ) )
            {
		act( AT_ACTION, "$n zaklada $v na swa glowe.",   ch, obj, NULL, TO_ROOM );
		act( AT_ACTION, "Zakladasz $v na swa glowe.", ch, obj, NULL, TO_CHAR );
            }
	...
	case ITEM_WEAR_EYES:
		...
		return;
            if ( !oprog_use_trigger( ch, obj, NULL, NULL, NULL ) )
            {
		act( AT_ACTION, "$n umieszcza $v na swych oczach.",   ch, obj, NULL, TO_ROOM );
		act( AT_ACTION, "Umieszczasz $v na swych oczach.", ch, obj, NULL, TO_CHAR );
            }
	...
	case ITEM_WEAR_FACE:
		...
		return;
            if ( !oprog_use_trigger( ch, obj, NULL, NULL, NULL ) )
            {
		act( AT_ACTION, "$n umieszcza $v na swej twarzy.",   ch, obj, NULL, TO_ROOM );
		act( AT_ACTION, "Umieszczasz $v na swej twarzy.", ch, obj, NULL, TO_CHAR );
            }
	...
	case ITEM_WEAR_EARS:
		...
		return;
            if ( !oprog_use_trigger( ch, obj, NULL, NULL, NULL ) )
            {
		act( AT_ACTION, "$n naklada $v na swe uszy.",   ch, obj, NULL, TO_ROOM );
		act( AT_ACTION, "Nakladasz $v na swe uszy.", ch, obj, NULL, TO_CHAR );
            }
...
*/
		...
		return;
            ...
            if ( !oprog_use_trigger( ch, obj, NULL, NULL, NULL ) )
            {
		act( AT_ACTION, "$n wsuwa sie w $v.",   ch, obj, NULL, TO_ROOM );
		act( AT_ACTION, "Wsuwasz sie w $v.", ch, obj, NULL, TO_CHAR );
            }
...
*/
		...
		return;
            ...
            if ( !oprog_use_trigger( ch, obj, NULL, NULL, NULL ) )
            {
	      act( AT_ACTION, "$n wsuwa stopy w $v.",   ch, obj, NULL, TO_ROOM );
	      act( AT_ACTION, "Wsuwasz stopy w $v.", ch, obj, NULL, TO_CHAR );
            }
...
*/
		...
		return;
            ...
            if ( !oprog_use_trigger( ch, obj, NULL, NULL, NULL ) )
            {
	      act( AT_ACTION, "$n zaklada $v na swe dlonie.",   ch, obj, NULL, TO_ROOM );
	      act( AT_ACTION, "Zakladasz $v na swe dlonie.", ch, obj, NULL, TO_CHAR );
            }
...
*/
	    ...
	    if ( !oprog_use_trigger( ch, obj, NULL, NULL, NULL ) )
            {
	      act( AT_ACTION, "$n zaklada $v na swe ramiona.",   ch, obj, NULL, TO_ROOM );
	      act( AT_ACTION, "Zakladasz $v na swe ramiona.", ch, obj, NULL, TO_CHAR );
            }
	...
	*/
		...
		return;
            ...
            if ( !oprog_use_trigger( ch, obj, NULL, NULL, NULL ) )
            {
	      act( AT_ACTION, "$n zarzuca $v wokol ciala.",   ch, obj, NULL, TO_ROOM );
	      act( AT_ACTION, "Zarzucasz $v wokol ciala.", ch, obj, NULL, TO_CHAR );
            }
	...
	case ITEM_WEAR_BACK:
		...
		return;
            if ( !oprog_use_trigger( ch, obj, NULL, NULL, NULL ) )
            {
		act( AT_ACTION, "$n zarzuca $v na plecy.",   ch, obj, NULL, TO_ROOM );
		act( AT_ACTION, "Zarzucasz $v na plecy.", ch, obj, NULL, TO_CHAR );
            }
...
*/
		...
		return;
            ...
            if ( !oprog_use_trigger( ch, obj, NULL, NULL, NULL ) )
            {
	      act( AT_ACTION, "$n zaklada $v wool pasa.",   ch, obj, NULL, TO_ROOM );
	      act( AT_ACTION, "Zakladasz $v wokol pasa.", ch, obj, NULL, TO_CHAR );
            }
	...
	case ITEM_WEAR_WRIST:
		...
		if ( !oprog_use_trigger( ch, obj, NULL, NULL, NULL ) )
		{
		   act( AT_ACTION, "$n zaklada $v na lewy nadgarstek.",
			ch, obj, NULL, TO_ROOM );
		   act( AT_ACTION, "Zakladasz $v na lewy nadgarstek.",
			ch, obj, NULL, TO_CHAR );
		}
		...
		if ( !oprog_use_trigger( ch, obj, NULL, NULL, NULL ) )
		{
		    act( AT_ACTION, "$n zaklada $v na prawy nadgarstek.",
			ch, obj, NULL, TO_ROOM );
		    act( AT_ACTION, "Zakladasz $v na prawy nadgarstek.",
			ch, obj, NULL, TO_CHAR );
		}
	    ...
	    send_to_char( "Nie masz wolnych nadgarstkow.\n\r", ch );
	    return;
	...
	case ITEM_WEAR_ANKLE:
		...
		if ( !oprog_use_trigger( ch, obj, NULL, NULL, NULL ) )
		{
		   act( AT_ACTION, "$n zaklada $v na lewej kostce.",
			ch, obj, NULL, TO_ROOM );
		   act( AT_ACTION, "Zakladasz $v na lewej kostce.",
			ch, obj, NULL, TO_CHAR );
		}
		...
		if ( !oprog_use_trigger( ch, obj, NULL, NULL, NULL ) )
		{
		    act( AT_ACTION, "$n zaklada $v na prawej kostce.",
			ch, obj, NULL, TO_ROOM );
		    act( AT_ACTION, "Zakladasz $v na prawej kostce.",
			ch, obj, NULL, TO_CHAR );
		}
	    ...
	    send_to_char( "Hmm masz tylko dwie kostki.\n\r", ch );
	    return;
	...
	case ITEM_WEAR_SHIELD:
	    ...
	    {
		send_to_char( "Nie mozesz uzywac tarczy i dwoch broni jednoczesnie!\n\r", ch );
		return;
	    }
		...
		return;
            if ( !oprog_use_trigger( ch, obj, NULL, NULL, NULL ) )
            {
		act( AT_ACTION, "$n chwyta $v jako tarcze.", ch, obj, NULL, TO_ROOM );
		act( AT_ACTION, "Chwytasz $v jako tarcze.", ch, obj, NULL, TO_CHAR );
            }
	...
	case ITEM_WIELD:
		...
		{
		    send_to_char( "Uzywasz juz dwoch dloni.\n\r", ch );
		    return;
		}
	    }
		...
		if ( can_dual(ch) )
		{
		    ...
		    {
			send_to_char( "To jest zbyt ciezkie dla Ciebie.\n\r", ch );
			return;
	      	    }
                    if ( !oprog_use_trigger( ch, obj, NULL, NULL, NULL ) )
                    {
			act( AT_ACTION, "$n bierze jako druga bron $v.", ch, obj, NULL, TO_ROOM );
			act( AT_ACTION, "Bierzesz jako druga bron $v.", ch, obj, NULL, TO_CHAR );
                    }
		...
		return;
            ...
            if ( !oprog_use_trigger( ch, obj, NULL, NULL, NULL ) )
            {
		act( AT_ACTION, "$n bierze jako bron $v.", ch, obj, NULL, TO_ROOM );
		act( AT_ACTION, "Bierzesz jako bron $v.", ch, obj, NULL, TO_CHAR );
            }
	    if ( 1 << bit == ITEM_MISSILE_WIELD )
		equip_char( ch, obj, WEAR_MISSILE_WIELD );
	    else
	...
	case ITEM_HOLD:
	    ...
	    ||  (get_eq_char(ch, WEAR_WIELD) && get_eq_char(ch, WEAR_MISSILE_WIELD)) )
	    {
		send_to_char( "Nie mozesz trzymac czegos i uzywac dwoch broni naraz!\n\r", ch );
		return;
	    }
		...
		return;
	       ...
	       || obj->item_type == ITEM_COOK
            ...
            {
		act( AT_ACTION, "$n chwyta $v w swe dlonie.",   ch, obj, NULL, TO_ROOM );
		act( AT_ACTION, "Chwytasz $v w swe dlonie.", ch, obj, NULL, TO_CHAR );
            }
...
void do_wear( CHAR_DATA *ch, char *argument )
{
    ...
    if ( arg1[0] == '\0' )
    {
	send_to_char( "Co chcesz zalozyc, czy trzymac?\n\r", ch );
	return;
    }
...
void do_remove( CHAR_DATA *ch, char *argument )
{
   ...
   if ( !str_cmp( arg, "all" ) )  /* SB Remove all */
    ...
    {
	send_to_char( "Nie uzywasz czegos takiego.\n\r", ch );
	return;
    }
    if ( (obj_next=get_eq_char(ch, obj->wear_loc)) != obj )
    {
	act( AT_PLAIN, "Najpierw musisz zdjac $p.", ch, obj_next, NULL, TO_CHAR );
	return;
    }
