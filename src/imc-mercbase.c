...
#ifdef CIRCLE
  if (!strcasecmp((char *)mud, imc_name))
#else
...
static void send_rchannel(CHAR_DATA *ch, char *argument, int number)
{
  ...
  if (!strcasecmp(arg, ",") || !strcasecmp(arg, "emote"))
...
DEFINE_DO_FUN(do_rchannels)
{
  ...
  while((argument=one_argument(argument, arg)), arg[0])
  {
    ...
    else if (!strcasecmp(arg+1, "rtell") && CAN(ch, IMC_RTELL, IMC_LEVEL_RTELL))
    {
      name="RTell";
      flag=IMC_RTELL;
    }
    else if (!strcasecmp(arg+1, "rbeep") && CAN(ch, IMC_RBEEP, IMC_LEVEL_RBEEP))
    {
      name="RBeep";
      flag=IMC_RBEEP;
    }
    else if (!strcasecmp(arg+1, "rinvis"))
    {
      name="RInvis";
      flag=IMC_RINVIS;
      toggle=!toggle;
...
#ifdef CIRCLE
	    !strcasecmp((char *)imc_channels[i].name, arg+1))
#else
	    !strcasecmp(imc_channels[i].name, arg+1))
#endif
	  break;
      }
...
DEFINE_DO_FUN(do_rchanset)
{
  ...
  if (!strcasecmp(argument, "rtell"))
  {
    flag=IMC_RTELL;
    name="RTell";
  }
  else if (!strcasecmp(argument, "rbeep"))
  {
    flag=IMC_RBEEP;
    name="RBeep";
  }
  else
  {
    for (i=0; i<numchannels; i++)
#ifdef CIRCLE
      if (!strcasecmp((char *)imc_channels[i].name, argument))
#else
      if (!strcasecmp(imc_channels[i].name, argument))
#endif
	break;
...
#ifdef CIRCLE
	(!strcasecmp((char *)to, GET_NAME(victim)) ||
#else
	(!strcasecmp(to, GET_NAME(victim)) ||
#endif
		   ...
		   const char *argument, int isreply)
{
  ...
  for (d=descriptor_list; d; d=d->next)
  {
    ...
    {
      if (!strcasecmp((char *)to, GET_NAME(vch)))
      {
	victim=vch;
	break;
      }
  ...
  if (victim)
  {
    ...
    {
#ifdef CIRCLE
      if (strcasecmp((char *)imc_nameof(from->name), "*"))
#else
	...
	strcasecmp(imc_nameof(from->name), "*"))
    {
...
#ifdef CIRCLE
    if (strcasecmp((char *)imc_nameof(from->name), "*"))     /* not a system message */
#else
		       ...
		       const char *subject, const char *text)
{
  ...
  if (strcasecmp("all", toname))
    return "Notes to 'all' are not accepted.";
  
  if (strcasecmp("immortal", toname) || strcasecmp("imm", toname) ||
      strcasecmp("imms", toname) || strcasecmp("immortals", toname))
  {
