...
void do_drink( CHAR_DATA *ch, char *argument )
{
    ...
    case ITEM_BLOOD:
	if (( IS_VAMPIRE(ch) || IS_DEMON(ch)) && !IS_NPC(ch) )
	{
...
void do_recite( CHAR_DATA *ch, char *argument )
{
    ...
    if ( arg1[0] == '\0' )
    {
	send_to_char( "Jaki zwoj chcesz odczytac?\n\r", ch );
	return;
    }
    ...
    {
	act( AT_ACTION, "$n chwyta $p i probuje to odczytac...",  ch, scroll, NULL, TO_ROOM );
	act( AT_ACTION, "Chwytasz $p i stajesz z otwartymi ustami. (I co teraz?)", ch, scroll, NULL, TO_CHAR );
	return;
    }
    ...
    {
        send_to_char( "Ten zwoj jest zbyt skomplikowany jak na Ciebie.\n\r", ch);
        return;
    }
	...
	{
        send_to_char( "Nie posiadasz takiego przedmiotu lub nikogo takiego tu nie ma.\n\r", ch );
	return;
	}
    ...
    separate_obj( scroll );
    act( AT_MAGIC, "$n odczytuje $p.", ch, scroll, NULL, TO_ROOM );
    act( AT_MAGIC, "Odczytujesz $p.", ch, scroll, NULL, TO_CHAR );
