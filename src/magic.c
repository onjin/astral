...
void say_spell( CHAR_DATA *ch, int sn )
{
    ...
    sprintf( buf2, "$n wypowiada slowa, '%s'.", buf );
    sprintf( buf,  "$n wypowiada slowa, '%s'.", skill->name );
...
void do_cast( CHAR_DATA *ch, char *argument )
{
    ...
    switch( ch->substate )
    {
        default:
            ...
            {
                send_to_char( "Nie mozesz teraz tego uczynic...\n\r", ch );
                return;
            }
            ...
            {
                set_char_color( AT_MAGIC, ch );
                send_to_char( "Magia tego miejsca jest silniejsza od Twojej.\n\r", ch );
                return;
            }
            ...
            if ( arg1[0] == '\0' )
            {
                send_to_char( "Jaki czar rzucic i ma kogo?\n\r", ch );
                return;
            }
            ...
            if ( get_trust(ch) < LEVEL_GOD )
            {
                ...
                {
                    send_to_char( "Nie mozesz tego teraz uczynic...\n\r", ch );
                    return;
                }
            }
            else
                 ...
                 */
            {
                ...
                {
                    send_to_char( "Nie istnieje taki czar...\n\r", ch );
                    return;
                }
                if ( sn >= MAX_SKILL )
                {
                    send_to_char( "Hmm... to moze zabolec.\n\r", ch );
                    return;
                }
                if ( (skill=get_skilltype(sn)) == NULL )
                {
                    send_to_char( "Cos jest z tym nie tak jak trzeba...\n\r", ch );
                    return;
                }
                if ( skill->type != SKILL_SPELL )
                {
                ...
                if ( !skill->spell_fun )
                {
                    send_to_char( "Ten czar nie jest w pelni ukonczony...\n\r", ch );
                    return;
                }
            }
            ...
            if ( ch->position < skill->minimum_position && !IS_NPC(ch) )
            {
                ...
                {
                    default:
                        send_to_char( "Nie mozesz sie odpowiednio skoncentrowac.\n\r", ch );
                        break;
                    case POS_SITTING:
                        send_to_char( "Nie mozesz zebrac tyle energii gdy siedzisz.\n\r", ch );
                        break;
                    case POS_RESTING:
                        send_to_char( "Nie mozesz rzucic czaru gdy odpoczywsz.\n\r", ch );
                        break;
                    case POS_FIGHTING:
                        if(skill->minimum_position<=POS_EVASIVE){
                            send_to_char( "Twoj styl walki jest zbyt wymagajacy by to uczynic!\n\r", ch);
                        } else {
                            send_to_char( "Nie ma mowy!  Zostan i walcz!\n\r", ch);
                        }
                        break;
                    case POS_DEFENSIVE:
                        if(skill->minimum_position<=POS_EVASIVE){
                    ...
                    case POS_EVASIVE:
                        ...
                        break;
                    case POS_SLEEPING:
                        send_to_char( "Snisz o wielkich, magicznych czynach.\n\r", ch );
                        break;
                }
                return;
            }
            ...
            if ( !skill->spell_fun )
            {
                send_to_char( "Nie mozesz jeszcze rzucic tego czaru.\n\r", ch );
                return;
            }
            ...
            {
                send_to_char( "Ten czar jest tylko dla czlonkow odpowiedniej gildii.\n\r", ch);
                return;
            }
            ...
            {
                set_char_color( AT_MAGIC, ch );
                send_to_char( "Bogowie nie pozwalaja Ci rzucic czaru na Ta postac.\n\r", ch );
                return;
            }
            ...
            if ( IS_VAMPIRE(ch) || IS_DEMON(ch) )
            {
                ...
                {
                    send_to_char( "Nie masz na tyle krwi w swym ciele.\n\r", ch );
                    return;
                }
            }
            else
                ...
                {
                    send_to_char( "Brakuje Ci magicznych mocy.\n\r", ch );
                    return;
                }
            ...
            add_timer( ch, TIMER_DO_FUN, UMIN(skill->beats / 10, 3),
                    do_cast, 1 );
            act( AT_MAGIC, "Zaczynasz monotonnie nucic...", ch, NULL, NULL, TO_CHAR );
            act( AT_MAGIC, "$n zaczyna monotonnie nucic...", ch, NULL, NULL, TO_ROOM );
        ...
        case SUB_TIMER_DO_ABORT:
            ...
            if ( IS_VALID_SN((sn = ch->tempnum)) )
            {
                if ( (skill=get_skilltype(sn)) == NULL )
                {
                    send_to_char( "Cos poszlo nie tak...\n\r", ch );
            ...
            set_char_color( AT_MAGIC, ch );
            send_to_char( "Przestajesz nucic...\n\r", ch );
        ...
        case 1:
            ...
            {
                send_to_char( "Cos przerwalo Twoj czar!\n\r", ch );
            ...
            if ( skill->participants > 1 )
            {
                ...
                if ( cnt >= skill->participants )
                {
                    for ( tmp = ch->in_room->first_person; tmp; tmp = tmp->next_in_room )
                        ...
                        {
                            extract_timer( tmp, t );
                            act( AT_MAGIC, "Przesylasz swa energie $x, pomagajac w rzucaniu czaru!", ch, NULL, tmp, TO_VICT );
                            act( AT_MAGIC, "$N przesyla Ci energie, pomagajac w rzucaniu czaru!", ch, NULL, tmp, TO_CHAR );
                            act( AT_MAGIC, "$N przesyla swa energie $x!", ch, NULL, tmp, TO_NOTVICT );
                    ...
                    dont_wait = TRUE;
                    send_to_char( "Koncentrujesz cala energie by wypowiedziec slowa zaklecia!\n\r", ch );
                    vo = locate_targets( ch, arg2, sn, &victim, &obj );
                    if ( vo == &pAbort )
                        return;
                }
                ...
                {
                    set_char_color( AT_MAGIC, ch );
                    send_to_char( "Jest zbyt malo energii by rzucic ten czar...\n\r", ch );
                    if ( IS_VAMPIRE(ch) || IS_DEMON(ch) )
            ...
            &&   (number_percent( ) + skill->difficulty * 5) > ch->pcdata->learned[sn] )
    {
        ...
        switch( number_bits(2) )
        {
            case 0:	/* too busy */
                if ( ch->fighting )
                    send_to_char( "Ta runda walki jest zbyt zajmujaca.\n\r", ch );
                else
                    send_to_char( "Cos Cie rozprasza.\n\r", ch );
                break;
            case 1:	/* irritation */
                if ( number_bits(2) == 0 )
                {
                    switch( number_bits(2) )
                    {
                        case 0: send_to_char( "Zakrecilo Ci sie w nosie i cala magia prysla.\n\r", ch ); break;
                        case 1: send_to_char( "Ugryzienie pchly zmienia Twe zaklecie w przeklenstwo.\n\r", ch ); break;
                        case 2: send_to_char( "Piorko wpada Ci do gardla i rzucasz na siebie czar rozsmieszenia.\n\r", ch ); break;
                        case 3: send_to_char( "Nie mozesz rzucic czary gdyz aktualnie wyciagasz muche z oka.\n\r", ch ); break;
                    }
                }
                else
                    send_to_char( "Cos Cie rozprasza i nie mozesz rzucic czaru.\n\r", ch );
                break;
            case 2:	/* not enough time */
                if ( ch->fighting )
                    send_to_char( "Nie masz tyle czasu by wypowiedziec zaklecie.\n\r", ch );
                else
                    send_to_char( "Nie mozesz sie skoncentrowac.\n\r", ch );
                break;
            case 3:
                send_to_char( "Cos blokuje Twoj umysl.\n\r", ch );
                break;
        }
        if ( !str_cmp( skill->name, "death" ) ) /* Flash */
...
ch_ret obj_cast_spell( int sn, int level, CHAR_DATA *ch, CHAR_DATA *victim, OBJ_DATA *obj )
{
    ...
    {
        set_char_color( AT_MAGIC, ch );
        send_to_char( "Nic sie nie dzieje...\n\r", ch );
        return rNONE;
    }
    ...
    if ( IS_SET( ch->in_room->area->flags, AFLAG_NOPKILL ) && 
            skill->target == TAR_CHAR_OFFENSIVE && !IS_NPC( victim ))
    {
        set_char_color( AT_IMMORT, ch );
        send_to_char( "Bogowie zabraniaja atakowania graczy w tym miejscu...\n\r", ch );
        return rNONE;
    }
 ...
 * Spell functions.
 */
ch_ret spell_transfer_energy( int sn, int level, CHAR_DATA *ch, void *vo )
{
    CHAR_DATA *victim = (CHAR_DATA *) vo;
    int mana_gain, mana_loss;

    if ( ch == victim)
    {
        send_to_char( "Na kogo chcesz rzucic czar?\n\r", ch );
        return rNONE;
    }
    if ( IS_DEMON( victim ) || IS_VAMPIRE( victim ) )
    {
        send_to_char( "Wampiry i demony potrzebuja krwi, nie zas Twoich mocy!\n\r", ch );
        return rSPELL_FAILED;
    }
    mana_loss = ch->level * 2;
    if ( ch->mana < mana_loss );
    mana_loss = ch->mana;
    if ( victim->class == 0 )
        mana_gain = mana_loss * 3 / 4;
    else
        mana_gain = mana_loss * 2 / 3;    
    if ( victim->mana + mana_gain > victim->max_mana )
        mana_gain = victim->max_mana - victim->mana;
    ch->mana -= mana_loss;
    victim->mana += mana_gain;
    act( AT_MAGIC, "Przekazujesz czesc swych magicznych mocy $X.", ch, NULL, victim, TO_CHAR );
    act( AT_MAGIC, "$n przekazuje czesc swych magicznych mocy $X.", ch, NULL, victim, TO_NOTVICT );
    act( AT_MAGIC, "$n przekazuje Ci czesc swych magicznych mocy.", ch, NULL, victim, TO_VICT );
    return rNONE;
}

ch_ret spell_astral_blade( int sn, int level, CHAR_DATA *ch, void *vo )
{
    OBJ_DATA *obj;
    AFFECT_DATA *paf;

    obj = ( create_object( get_obj_index( OBJ_VNUM_ASTRAL_BLADE ), 0 ) );
    if ( !obj )
        return rNONE;
    obj->cost = ch->level * 1000;
    obj->timer = ch->level + 10;
    obj->value[0] = ( int ) ( ch->level / 5 );
    obj->value[1] = ( int ) ch->level / 3;
    obj->value[2] = ( int ) ch->level * 2 / 3;
    obj->value[3] = 2;
    obj->level = ch->level;
    ...
    obj_to_char( obj, ch );
    act( AT_MAGIC, "Tworzysz astralne ostrze!", ch, NULL, NULL, TO_CHAR );
    act( AT_MAGIC, "$n tworzy astralne ostrze!", ch, NULL, NULL, TO_ROOM );
    return rNONE;
}
...
ch_ret spell_blindness( int sn, int level, CHAR_DATA *ch, void *vo )
{
    ...
    if ( ch->sex == SEX_MALE )
        send_to_char( "Zostales oslepiony!\n\r", victim );
    else if ( ch->sex == SEX_MALE )
        send_to_char( "Zostalas oslepiona!\n\r", victim );
    else
        send_to_char( "Zostales oslepione!\n\r", victim );
    if ( ch != victim )
    {
        act( AT_MAGIC, "Oslepiasz $B!", ch, NULL, victim, TO_CHAR );
        act( AT_MAGIC, "$n oslepia $B!", ch, NULL, victim, TO_NOTVICT );
    }
    return rNONE;
}
...
ch_ret spell_call_lightning( int sn, int level, CHAR_DATA *ch, void *vo )
{
    ...
    if ( !IS_OUTSIDE(ch) )
    {
        send_to_char( "Musisz byc na wolnym powietrzu.\n\r", ch );
        return rSPELL_FAILED;
    }

    if ( ch->in_room->area->weather->precip <= 0 )
    {
        send_to_char( "Musi byc zla pogoda.\n\r", ch );
        return rSPELL_FAILED;
    }
    ...
    set_char_color( AT_MAGIC, ch );
    send_to_char( "Boska blyskawica uderza w Twoich wrogow!\n\r", ch );
    if ( ch->sex == SEX_FEMALE )
        act( AT_MAGIC, "$n przyzywa Boska blyskawice, by uderzyla jej wrogow!",
                ch, NULL, NULL, TO_ROOM );
    else
        act( AT_MAGIC, "$n przyzywa Boska blyskawice, by uderzyla jego wrogow!",
                ch, NULL, NULL, TO_ROOM );
    ch_died = FALSE;
    for ( vch = first_char; vch; vch = vch_next )
    {
        ...
        if ( !ch_died
                ...
                &&   IS_AWAKE(vch) ) {
            if ( number_bits( 3 ) == 0 )
                send_to_char_color( "&BBlyskawica przecina niebo.\n\r", vch );
        }
    }
...
ch_ret spell_charm_person( int sn, int level, CHAR_DATA *ch, void *vo )
{
    ...
    {
        if ( ch->sex == SEX_FEMALE )
            send_to_char( "Jestes zauroczona swoja osoba!\n\r", ch );
        else
            send_to_char( "Jestes zauroczony swoja osoba!\n\r", ch );
        return rSPELL_FAILED;
    }
    ...
    if ( !IS_NPC( victim ) && !IS_NPC( ch ) )
    {
        ...
        send_to_char( "Ktos probowal Cie zauroczyc...\n\r", victim );
        return rSPELL_FAILED;
    }
...
ch_ret spell_create_food( int sn, int level, CHAR_DATA *ch, void *vo )
{
    ...
    act( AT_MAGIC, "Pojawia sie $p.", ch, mushroom, NULL, TO_ROOM );
    act( AT_MAGIC, "Pojawia sie $p.", ch, mushroom, NULL, TO_CHAR );
...
ch_ret spell_create_water( int sn, int level, CHAR_DATA *ch, void *vo )
{
    ...
    {
        send_to_char( "To sie nie nadaje na wode.\n\r", ch );
        return rSPELL_FAILED;
    }

    if ( obj->value[2] != LIQ_WATER && obj->value[1] != 0 )
    {
        send_to_char( "W tym juz jest jakis plyn.\n\r", ch );
        return rSPELL_FAILED;
    }
    ...
    if ( water > 0 )
    {
        ...
        act( AT_MAGIC, "$p napelnia sie woda.", ch, obj, NULL, TO_CHAR );
    }
...
ch_ret spell_cure_blindness( int sn, int level, CHAR_DATA *ch, void *vo )
{
    ...
    {
        if ( ch != victim )
            send_to_char( "Rzucasz zaklecie uleczenia, ale nie ma zadnego widocznego efektu.\n\r", ch );
        else
            send_to_char( "Przeciez widzisz!\n\r", ch );
        return rSPELL_FAILED;
    }
    affect_strip( victim, gsn_blindness );
    set_char_color( AT_MAGIC, victim);
    send_to_char( "Znowu widzisz!\n\r", victim );
    if ( ch != victim )
        send_to_char( "Rzucasz zaklecie uleczenia, przywracajac wzrok.\n\r", ch );
    return rNONE;
}
...
ch_ret spell_sacral_divinity( int sn, int level, CHAR_DATA *ch, void *vo )
{
    ...
    if ( ch->alignment < 350 ) {
        act( AT_MAGIC, "Twoja modlitwa przechodzi bez echa.", ch, NULL, NULL, TO_CHAR );
        return rSPELL_FAILED;
    }
    ...
    act( AT_MAGIC, "Swietlisty calun powoli formuje sie wokol $c.", ch, NULL, NULL, TO_ROOM);
    act( AT_MAGIC, "Swielisty calun powoli formuje sie wokol Ciebie.", ch, NULL, NULL, TO_CHAR );
    return rNONE;
}
...
ch_ret spell_expurgation( int sn, int level, CHAR_DATA *ch, void *vo )
{
    ...
    act( AT_MAGIC, "Odmawiasz prastara modlitwe, proszac swego Boga o oczyszczenie.", ch, NULL, NULL, TO_CHAR );
    act( AT_MAGIC, "$n odmawia prastara modlitwe, proszac swego boga o oczyszczenie.", ch, NULL, NULL, TO_ROOM );
    return rNONE;
}
...
ch_ret spell_bethsaidean_touch( int sn, int level, CHAR_DATA *ch, void *vo )
{
    ...
    send_to_char( "Znowu widzisz!\n\r", victim );
    if ( ch != victim ) {
        act( AT_MAGIC, "$n kladzie rece na Twoich oczach i cicho odmawia modlitwe...", ch, NULL, victim, TO_VICT );
        act( AT_MAGIC, "$n kladzie rece na oczach $C i cicho odmawia modliwte...", ch, NULL, victim, TO_NOTVICT );
        act( AT_MAGIC, "Kladziesz rece na oczach $C i cicho odmawiasz modlitwe.", ch, NULL, victim, TO_CHAR );
    }
    return rNONE;
}
...
ch_ret spell_cure_poison( int sn, int level, CHAR_DATA *ch, void *vo )
{
    ...
    if ( is_affected( victim, gsn_poison ) )
    {
        affect_strip( victim, gsn_poison );
        set_char_color( AT_MAGIC, victim);
        send_to_char( "Uczucie ciepla plynie przez Twoje cialo.\n\r", victim );
        victim->mental_state = URANGE( -100, victim->mental_state, -10 );
        if ( ch != victim )
        {
            act( AT_MAGIC, "$N wyglada duzo zdrowiej.", ch, NULL, victim, TO_NOTVICT );
            act( AT_MAGIC, "Usuwasz trucizne z ciala $C.", ch, NULL, victim, TO_CHAR );
        }
        return rNONE;
    }
    else
    {
        set_char_color( AT_MAGIC, ch );
        if ( ch != victim )
            send_to_char( "Rzucasz zaklecie uleczenia, ale nie zadnego widocznego efektu.\n\r", ch );
        else
            send_to_char( "Nic nie zatruwa Twojego ciala.\n\r", ch );
        return rSPELL_FAILED;
    }
}
...
ch_ret spell_curse( int sn, int level, CHAR_DATA *ch, void *vo )
{
    ...
    set_char_color( AT_MAGIC, victim);
    if ( ch->sex == SEX_MALE )
        send_to_char( "Czujesz sie nieczysty.\n\r", victim );
    else if ( ch->sex == SEX_FEMALE )
        send_to_char( "Czujesz sie nieczysta.\n\r", victim );   
    else
        send_to_char( "Czujesz sie nieczyste.\n\r", victim );
    if ( ch != victim )
    {
        act( AT_MAGIC, "Rzucasz klatwe na $B.", ch, NULL, victim, TO_CHAR );
        act( AT_MAGIC, "$n rzuca klatwe na $B.", ch, NULL, victim, TO_NOTVICT );
    }
    return rNONE;
}
...
ch_ret spell_dispel_magic (int sn, int level, CHAR_DATA * ch, void *vo)
{
    ...
    if ( ch == victim )
    {
        if ( ch->first_affect )
        {
            send_to_char ( "Przesuwasz rece wzdluz swojego ciala...\n\r", ch );
    ...
    {
        send_to_char ("Nie wyczuwasz magii, ktora mozna by rozproszyc.\n\r", ch );
...
ch_ret spell_earthquake( int sn, int level, CHAR_DATA *ch, void *vo )
{
    ...
    act( AT_MAGIC, "Ziemia trzesie sie pod Twoimi stopami!", ch, NULL, NULL, TO_CHAR );
    act( AT_MAGIC, "$n sprawia, ze ziemia sie trzesie.", ch, NULL, NULL, TO_ROOM );
    ...
    for ( vch = first_char; vch; vch = vch_next )
    {
        ...
        {
            if ( number_bits( 3 ) == 0 )
                send_to_char_color( "&BZiemia trzesie sie.\n\r", vch );
        }
    }
...
ch_ret spell_enchant_armor( int sn, int level, CHAR_DATA *ch, void *vo )
{
    ...
    if ( IS_GOOD(ch) )
    {
        ...
        act( AT_BLUE, "$p zablyska niebieskim kolorem.", ch, obj, NULL, TO_CHAR );
    }
    else if ( IS_EVIL(ch) )
    {
        xSET_BIT(obj->extra_flags, ITEM_ANTI_GOOD);
        act( AT_RED, "$p zablyska czerwonym kolorem.", ch, obj, NULL, TO_CHAR );
    }
    else
    {
        xSET_BIT(obj->extra_flags, ITEM_ANTI_EVIL); 
        xSET_BIT(obj->extra_flags, ITEM_ANTI_GOOD);
        act( AT_YELLOW, "$p zablyska zoltym kolorem.", ch, obj, NULL, TO_CHAR );
    }
...
ch_ret spell_enchant_weapon( int sn, int level, CHAR_DATA *ch, void *vo )
{
    ...
    if ( obj->item_type != ITEM_WEAPON
            ...
            ||   obj->first_affect )
    {
        act( AT_MAGIC, "Twoja magia nie ima sie $p.", ch, obj, NULL, TO_CHAR );
        act( AT_MAGIC, "Magia $c nie ima sie $p.", ch, obj, NULL, TO_NOTVICT );
        return rSPELL_FAILED;
    }
    ...
    {
        xSET_BIT(obj->extra_flags, ITEM_ANTI_EVIL);
        act( AT_BLUE, "$p zablyska niebieskim kolorem.", ch, obj, NULL, TO_ROOM );
        act( AT_BLUE, "$p zablyska niebieskim kolorem.", ch, obj, NULL, TO_CHAR );
    }
    else if ( IS_EVIL(ch) )
    {
        xSET_BIT(obj->extra_flags, ITEM_ANTI_GOOD);
        act( AT_BLOOD, "$p zablyska czerwonym kolorem.", ch, obj, NULL, TO_CHAR );
        act( AT_BLOOD, "$p zablyska czerwonym kolorem.", ch, obj, NULL, TO_ROOM );
    }
    else
    {
        xSET_BIT(obj->extra_flags, ITEM_ANTI_EVIL);
        xSET_BIT(obj->extra_flags, ITEM_ANTI_GOOD);
        act( AT_YELLOW, "$p zablyska zoltym kolorem.", ch, obj, NULL, TO_ROOM );
        act( AT_YELLOW, "$p zablyska zoltym kolorem.", ch, obj, NULL, TO_CHAR );
    }
    return rNONE;
}
...
ch_ret spell_disenchant_weapon( int sn, int level, CHAR_DATA *ch, void *vo )
{
    ...
    if ( obj->item_type != ITEM_WEAPON )
    {
        send_to_char("Tylko bronie mozna rozmagiczniac.",ch);
        return rSPELL_FAILED;
    }

    if ( !IS_OBJ_STAT(obj, ITEM_MAGIC) || !obj->first_affect )
    {
        send_to_char("Ta bron nie byla umagiczniana.",ch);
        return rSPELL_FAILED;
    }

    if ( xIS_SET(obj->pIndexData->extra_flags, ITEM_MAGIC) )
    {
        send_to_char("Nie mozesz rozmagicznic broni, ktora sama w sobie jest magiczna.",ch);
        return rSPELL_FAILED;
    }
    ...
    {
        send_to_char("Nie mozesz rozmagicznic broni, ktora sama w sobie jest dobra lub zla.",ch);
        return rSPELL_FAILED;
    }
    ...
    if ( IS_OBJ_STAT(obj, ITEM_ANTI_GOOD) )
    {
        ...
        act( AT_BLUE, "$p przez chwilke zdaje sie pochlaniac cale niebieskie swialo wokol siebie.", ch, obj, NULL, TO_CHAR );
    }
    if ( IS_OBJ_STAT(obj, ITEM_ANTI_EVIL) )
    {
        xREMOVE_BIT(obj->extra_flags, ITEM_ANTI_GOOD);
        act( AT_RED, "$p przez chwilke zdaje sie pochlaniac cale czerwone swiatlo wokol siebie.", ch, obj, NULL, TO_CHAR );
    }
...
ch_ret spell_faerie_fire( int sn, int level, CHAR_DATA *ch, void *vo )
{
    ...
    affect_to_char( victim, &af );
    act( AT_PINK, "Otacza Cie rozowa aura.", victim, NULL, NULL, TO_CHAR );
    act( AT_PINK, "Rozowa aura otacza $b.", victim, NULL, NULL, TO_ROOM );
    return rNONE;
}
...
ch_ret spell_faerie_fog( int sn, int level, CHAR_DATA *ch, void *vo )
{
    ...
    act( AT_MAGIC, "$n przywoluje chmure purpurowego dymu.", ch, NULL, NULL, TO_ROOM );
    act( AT_MAGIC, "Przywolujesz chmure purpurowego dymu.", ch, NULL, NULL, TO_CHAR );
    ...
    for ( ich = ch->in_room->first_person; ich; ich = ich->next_in_room )
    {
        ...
        if ( ch->sex == SEX_MALE )
        {
            act( AT_MAGIC, "$n zostaje odkryty!", ich, NULL, NULL, TO_ROOM );
            act( AT_MAGIC, "Zostajesz odkryty!", ich, NULL, NULL, TO_CHAR );
        }
        else if ( ch->sex == SEX_FEMALE )
        {
            act( AT_MAGIC, "$n zostaje odkryta!", ich, NULL, NULL, TO_ROOM );
            act( AT_MAGIC, "Zostajesz odkryta!", ich, NULL, NULL, TO_CHAR );
        }
        else
        {
            act( AT_MAGIC, "$n zostaje odkryty!", ich, NULL, NULL, TO_ROOM );
...
ch_ret spell_identify( int sn, int level, CHAR_DATA *ch, void *vo )
{
    ...
    if ( target_name[0] == '\0' )
    {
        send_to_char( "Na co lub kogo chcesz rzucic czar?\n\r", ch );
        return rSPELL_FAILED;
    }
    ...
    if ( ( obj = get_obj_carry( ch, target_name ) ) != NULL )
    {
        ...
        if ( obj->item_type != ITEM_LIGHT && obj->wear_flags-1 > 0 )
            ch_printf( ch, ", with wear locations:  %s\n\r",
...
ch_ret spell_invis( int sn, int level, CHAR_DATA *ch, void *vo )
{
    ...
    if( victim )
    {
        ...
        if ( ch->sex == SEX_MALE )
            act( AT_MAGIC, "$n staje sie niewidzialny.", victim, NULL, NULL, TO_ROOM );
        else if ( ch->sex == SEX_FEMALE )
            act( AT_MAGIC, "$n staje sie niewidzialna.", victim, NULL, NULL, TO_ROOM );
        else
            act( AT_MAGIC, "$n staje sie niewidzialne.", victim, NULL, NULL, TO_ROOM );
        af.type      = sn;
        ...
        affect_to_char( victim, &af );
        if ( ch->sex == SEX_MALE )
            act( AT_MAGIC, "Stajesz sie niewidzialny.", victim, NULL, NULL, TO_CHAR );
        else if ( ch->sex == SEX_FEMALE )
            act( AT_MAGIC, "Stajesz sie niewidzialna.", victim, NULL, NULL, TO_CHAR );
        else
            act( AT_MAGIC, "Stajesz sie niewidzialne.", victim, NULL, NULL, TO_CHAR );
        return rNONE;
    }
    else
    {
        ...
        if (obj)
        {
            ...
            act( AT_MAGIC, "$p staje sie niewidzialne.", ch, obj, NULL, TO_CHAR );
            return rNONE;
        }
    }
    ch_printf(ch, "Nie mozesz znalezc %s!\n\r", target_name);
    return rSPELL_FAILED;
}
...
ch_ret spell_know_alignment( int sn, int level, CHAR_DATA *ch, void *vo )
{
    ...
    if ( ap >  700 ) msg = "$N ma anielski charakter.";
    else if ( ap >  350 ) msg = "$N ma dobry charakter.";
    else if ( ap >  100 ) msg = "$N zazwyczaj jest mily i uprzejmy.";
    else if ( ap > -100 ) msg = "$N ma neutralny charakter.";
    else if ( ap > -350 ) msg = "$N oklamuje swoich przyjaciol.";
    else if ( ap > -700 ) msg = "$N zabilby Cie tak szybko, jak ty na niego spogladasz.";
    else msg = "Najlepiej nie mowmy nic na temat charakeru $C.";
...
ch_ret spell_locate_object( int sn, int level, CHAR_DATA *ch, void *vo )
{
    ...
    for ( obj = first_object; obj; obj = obj->next )
    {
        ...
        if ( in_obj->carried_by )
        {
            ...
            sprintf( buf, "%s noszony przez %s.\n\r",
                    obj_short(obj), PERSB(in_obj->carried_by, ch) );
        }
        else
        {
            sprintf( buf, "%s lezy w %s.\n\r",
...
ch_ret spell_pass_door( int sn, int level, CHAR_DATA *ch, void *vo )
{
    ...
    if ( ch->sex == SEX_MALE )
    {
        act( AT_MAGIC, "$n staje sie niematerialny.", victim, NULL, NULL, TO_ROOM );
        act( AT_MAGIC, "Stajesz sie niematerialny.", victim, NULL, NULL, TO_CHAR );
    }
    else if ( ch->sex == SEX_FEMALE )
    {
        act( AT_MAGIC, "$n staje sie niematerialna.", victim, NULL, NULL, TO_ROOM );
        act( AT_MAGIC, "Stajesz sie niematerialna.", victim, NULL, NULL, TO_CHAR );
    }
    else
    {
        act( AT_MAGIC, "$n staje sie niematerialne.", victim, NULL, NULL, TO_ROOM );
        act( AT_MAGIC, "Stajesz sie niematerialne.", victim, NULL, NULL, TO_CHAR );
    }
    return rNONE;
}
...
ch_ret spell_poison( int sn, int level, CHAR_DATA *ch, void *vo )
{
    ...
    {
        set_char_color( AT_MAGIC, ch );
        send_to_char( "Twoja magia nie zadzialala.\n\r", ch );
        return rSPELL_FAILED;
    }
    ...
    send_to_char( "Trucizna rozprzestrzenia sie w Twoim ciele.\n\r", victim );
    if ( IS_PKILL( victim) )
    ...
    if ( ch != victim )
    {
        if ( ch->sex == SEX_MALE )
        {	
            act( AT_GREEN, "$N trzesie sie gdy Twoja trucizna rozprzestrzenia sie w jego ciele.", ch, NULL, victim, TO_CHAR );
            act( AT_GREEN, "$N trzesie sie gdy trucizna $c rozprzestrzenia sie w jego ciele.", ch, NULL, victim, TO_NOTVICT );
        }
        else if ( ch->sex == SEX_FEMALE )
        {
            act( AT_GREEN, "$N trzesie sie gdy Twoja trucizna rozprzestrzenia sie w jej ciele.", ch, NULL, victim, TO_CHAR );
            act( AT_GREEN, "$N trzesie sie gdy trucizna $c rozprzestrzenia sie w jego ciele.", ch, NULL, victim, TO_NOTVICT );
        }
        else
        {
...
ch_ret spell_remove_curse( int sn, int level, CHAR_DATA *ch, void *vo )
{
    ...
    if ( is_affected( victim, gsn_curse ) )
    {
        ...
        set_char_color( AT_MAGIC, victim );
        send_to_char( "Przeklenstwo zostalo zdjete z Ciebie.\n\r", victim );
        if ( ch != victim )
        {
            act( AT_MAGIC, "Zdejmujesz przeklenstwo z $N.", ch, NULL, victim, TO_CHAR );
            act( AT_MAGIC, "$n zdejmuje przeklenswo z $C.", ch, NULL, victim, TO_NOTVICT );
        }
    }
    else
        if ( victim->first_carrying )
        {
            for ( obj = victim->first_carrying; obj; obj = obj->next_content )
                ...
                {
                    separate_obj( obj );/* Crevan */
                    if ( IS_OBJ_STAT( obj, ITEM_NOREMOVE ) )
                        xREMOVE_BIT( obj->extra_flags, ITEM_NOREMOVE );
                    if ( IS_OBJ_STAT( obj, ITEM_NODROP ) )
                        xREMOVE_BIT( obj->extra_flags, ITEM_NODROP );
                    set_char_color( AT_MAGIC, victim );
                    send_to_char( "Czujesz, ze zdjeto z Ciebie ciezar.\n\r", victim );

                    if ( ch != victim )
                    {
                        act( AT_MAGIC, "Zdejmujesz przeklenstwo z $C.", ch, NULL, victim, TO_CHAR );
                        act( AT_MAGIC, "$n zdejmuje przeklenstwo z $C.", ch, NULL, victim, TO_NOTVICT );
                    }
                    return rNONE;
                }
        }
...
ch_ret spell_sleep( int sn, int level, CHAR_DATA *ch, void *vo )
{
    ...
    {
        send_to_char( "Nikogo takiego nie ma tutaj.\n\r", ch );
        return rSPELL_FAILED;
    }

    if ( !IS_NPC(victim) && victim->fighting )
    {
        send_to_char( "Nie mozesz uspic walczacego gracza.\n\r", ch );
        return rSPELL_FAILED;
    }
    ...
    if ( IS_AWAKE(victim) )
    {
        act( AT_MAGIC, "Strasznie chce Ci sie spac..... zzzzzz.", victim, NULL, NULL, TO_CHAR );
        act( AT_MAGIC, "$n idzie spac.", victim, NULL, NULL, TO_ROOM );
...
ch_ret spell_summon( int sn, int level, CHAR_DATA *ch, void *vo )
{
    ...
    if ( !IS_NPC( ch ) )
    {
        act( AT_MAGIC, "Ogarnia Cie potworne zmeczenie...", ch, NULL,
                NULL, TO_CHAR );
        act( AT_MAGIC, "$n pada na ziemie!", ch, NULL, NULL, TO_ROOM );
    ...
    act( AT_MAGIC, "$n znika nagle.", victim, NULL, NULL, TO_ROOM );
    char_from_room( victim );
    char_to_room( victim, ch->in_room );
    act( AT_MAGIC, "$n pojawia sie nagle.", victim, NULL, NULL, TO_ROOM );
    act( AT_MAGIC, "$N przyzwal Cie!", victim, NULL, ch,   TO_CHAR );
    do_look( victim, "auto" );
    return rNONE;
}
...
ch_ret spell_astral_walk( int sn, int level, CHAR_DATA *ch, void *vo )
{
    ...
    if ( skill->hit_room && skill->hit_room[0] != '\0' )
        ...
        act( AT_MAGIC, "$n znika w blysku swiatla!", ch, NULL, NULL, TO_ROOM );
    char_from_room( ch );
    char_to_room( ch, victim->in_room );
    if ( skill->hit_dest && skill->hit_dest[0] != '\0' )
        act( AT_MAGIC, skill->hit_dest, ch, NULL, victim, TO_NOTVICT );
    else
        act( AT_MAGIC, "$n pojawia sie w blysku swiatla!", ch, NULL, NULL, TO_ROOM );
    do_look( ch, "auto" );
    return rNONE;
}
...
ch_ret spell_teleport( int sn, int level, CHAR_DATA *ch, void *vo )
{
    ...
    act( AT_MAGIC, "$n powoli rozplywa sie w powietrzu i znika.", victim, NULL, NULL, TO_ROOM );
    char_from_room( victim );
    char_to_room( victim, pRoomIndex );
    act( AT_MAGIC, "$n powoli materializuje sie.", victim, NULL, NULL, TO_ROOM );
    do_look( victim, "auto" );
    return rNONE;
}
...
ch_ret spell_ventriloquate( int sn, int level, CHAR_DATA *ch, void *vo )
{
    ...
    sprintf( buf1, "%s mowi '%s'.\n\r",              speaker, target_name );
    sprintf( buf2, "Ktos sprawia, ze %s mowi '%s'.\n\r", speaker, target_name );
    buf1[0] = UPPER(buf1[0]);
...
ch_ret spell_weaken( int sn, int level, CHAR_DATA *ch, void *vo )
{
    ...
    affect_to_char( victim, &af );
    set_char_color( AT_MAGIC, victim );
    send_to_char( "Atrofia ogarnia Twoje miesnie!\n\r", victim );
    if ( ch != victim )
    {
...
ch_ret spell_transport( int sn, int level, CHAR_DATA *ch, void *vo )
{
    ...
    {
        send_to_char("Hmm... sa tuz kolo Ciebie!", ch);
        return rSPELL_FAILED;
    }
    ...
    {
        send_to_char( "Ten przedmiot nie jest dla smiertelnikow!\n\r", ch );
        return rSPELL_FAILED;   /* Thoric */
    }

    act( AT_MAGIC, "$p powoli dematerializuje sie...", ch, obj, NULL, TO_CHAR );
    act( AT_MAGIC, "$p powoli dematerializuje sie z rak $c..", ch, obj, NULL, TO_ROOM );
    obj_from_char( obj );
    obj_to_char( obj, victim );
    act( AT_MAGIC, "$p od $c pojawia sie w Twoich rekach!", ch, obj, victim, TO_VICT );
    act( AT_MAGIC, "$p pojawia sie w rekach $c!", victim, obj, NULL, TO_ROOM );
    save_char_obj(ch);
...
ch_ret spell_portal( int sn, int level, CHAR_DATA *ch, void *vo )
{
    ...
    if (victim->in_room == ch->in_room)
    {
        send_to_char("Hmm... sa w tuz obok Ciebie!", ch);
        return rSPELL_FAILED;
    }
    ...
    for ( pexit = fromRoom->first_exit; pexit; pexit = pexit->next )
    {
        ...
        {
            send_to_char("W tym miejscu juz jest portal.\n\r",ch);
            return rSPELL_FAILED;
        }

        if ( pexit->vdir == DIR_PORTAL )
        {
            send_to_char("W tym miejscu nie mozesz stworzyc portalu.\n\r",ch);
            return rSPELL_FAILED;
        }
    }
    ...
    pexit->description	= STRALLOC( "Zagladasz w lsniacy portal...\n\r" );
    pexit->key     	= -1;
    ...
    portalObj->timer = 3;
    sprintf( buf, "portal stworzony przez %s", ch->nbie );
    ...
    {
        set_char_color( AT_MAGIC, ch );
        send_to_char("Wypowiadasz slowa zaklecia i portal otwiera sie przed Toba!\n\r", ch);
    }
    else
        act( AT_MAGIC, skill->hit_char, ch, NULL, victim, TO_CHAR );
    if ( !skill->hit_room || skill->hit_room[0] == '\0' )
        act( AT_MAGIC, "$n wypowiada slowa zaklecia i portal otwiera sie przed Toba!", ch, NULL, NULL, TO_ROOM );
    else
        act( AT_MAGIC, skill->hit_room, ch, NULL, victim, TO_ROOM );
    if ( !skill->hit_vict || skill->hit_vict[0] == '\0' )
        act( AT_MAGIC, "Lsniacy portal otwiera sie przed Toba!", victim, NULL, NULL, TO_ROOM );
    else
    ...
    sprintf( buf, "%s utworzyl portal z pokoju %d do pokoju %d.", 
...
ch_ret spell_plant_pass( int sn, int level, CHAR_DATA *ch, void *vo )
{
    ...
    if ( ch->in_room->sector_type == SECT_FOREST )
        act( AT_MAGIC, "$n znika za pobliskim drzewem!", ch, NULL, NULL, TO_ROOM );
    else
        act( AT_MAGIC, "$n znika w trawie!", ch, NULL, NULL, TO_ROOM );
    char_from_room( ch );
    char_to_room( ch, victim->in_room );
    if ( ch->in_room->sector_type == SECT_FOREST )
        act( AT_MAGIC, "$n wychodzi zza pobliskiego drzewa!", ch, NULL, NULL, TO_ROOM );
    else
        act( AT_MAGIC, "$n wylania sie z trawy!", ch, NULL, NULL, TO_ROOM );
    do_look( ch, "auto" );
    return rNONE;
}
...
ch_ret spell_mist_walk( int sn, int level, CHAR_DATA *ch, void *vo )
{
    ...
    {
        failed_casting( skill, ch, victim, NULL );
        send_to_char( "Nie mozesz wyczuc swojej ofiary...", ch );
        return rSPELL_FAILED;
    }
    ...
    act( AT_DGREEN, "$n rozplywa sie w chmurke lsniacej mgly, po czym znika!", 
            ch, NULL, NULL, TO_ROOM );
    char_from_room( ch );
    char_to_room( ch, victim->in_room );
    act( AT_DGREEN, "Chmurka lsniacej mgly otacza Cie, po czym rozprasza sie odkrywajac $b!", 
            ch, NULL, NULL, TO_ROOM );
    do_look( ch, "auto" );
    return rNONE;
}
...
ch_ret spell_solar_flight( int sn, int level, CHAR_DATA *ch, void *vo )
{
    CHAR_DATA *victim;
    SKILLTYPE *skill = get_skilltype(sn);
    /*    WEATHER_DATA *weath = ch->in_room->area->weather; Flash */
    ...
    act( AT_MAGIC, "$n znika w oslepiajacym blysku swiatla!",
            ch, NULL, NULL, TO_ROOM );
    char_from_room( ch );
    char_to_room( ch, victim->in_room );
    act( AT_MAGIC, "$n pojawia sie w oslepiajacym blysku swiatla!", 
            ch, NULL, NULL, TO_ROOM );
    do_look( ch, "auto" );
    return rNONE;
}
...
ch_ret spell_remove_invis( int sn, int level, CHAR_DATA *ch, void *vo )
{
    ...
    if ( obj )
    {
        if ( !IS_OBJ_STAT(obj, ITEM_INVIS) )
        {
            send_to_char( "To nie jest niewidzialne!\n\r", ch );
            return rSPELL_FAILED;
        }

        xREMOVE_BIT(obj->extra_flags, ITEM_INVIS);
        act( AT_MAGIC, "$p z powrotem staje sie widzialne.", ch, obj, NULL, TO_CHAR );
    ...
    else
    {
        ...
        if (victim)
        {
            if(!can_see(ch, victim))
            {
                ch_printf(ch, "Nie widzisz %s!\n\r", target_name);
                return rSPELL_FAILED;
            }

            if( !IS_AFFECTED(victim, AFF_INVISIBLE) )
            {
                send_to_char("Oni nie sa niewidzialni!\n\r", ch);
                return rSPELL_FAILED;
            }
...
ch_ret spell_animate_dead( int sn, int level, CHAR_DATA *ch, void *vo )
{
    ...
    if ( !found )
    {
        send_to_char("Nie ma tu odpowiedniego ciala.\n\r", ch);
        return rSPELL_FAILED;
    }

    if ( get_mob_index(MOB_VNUM_ANIMATED_CORPSE) == NULL )
    {
        bug("Vnum 5 nie znaleziony dla spell_animate_dead!", 0);
        return rNONE;
    }
    ...
    if ( !IS_NPC(ch) )
    {
        if ( IS_VAMPIRE(ch) || IS_DEMON (ch) )
        {
            if ( !IS_IMMORTAL(ch) && ch->pcdata->condition[COND_BLOODTHIRST] -
                    (pMobIndex->level/3) < 0 )
            {
                send_to_char("Nie masz na tyle krwi w swoim ciele.\n\r", ch );
                return rSPELL_FAILED;
            }
    ...
    { 
        ...
        act(AT_MAGIC, "$n sprawia, ze $T wstaje z martwych!", ch, NULL, pMobIndex->short_descr, TO_ROOM);
        act(AT_MAGIC, "Sprawiasz, ze $T wstaje z martwych!", ch, NULL, pMobIndex->short_descr, TO_CHAR);

        sprintf(buf, "ozywione cialo %s", pMobIndex->player_name);
        STRFREE(mob->name);
        mob->name = STRALLOC(buf);

        sprintf(buf, "Ozywione cialo %s", pMobIndex->short_descr_dop);
        STRFREE(mob->short_descr);
        mob->short_descr = STRALLOC(buf);

        sprintf(buf, "Ozywione cialo %s zmaga sie ze zgroza swojego istnienia.\n\r", pMobIndex->short_descr_dop);
...
ch_ret spell_death( int sn, int level, CHAR_DATA *ch, void *vo )
{
    ...
    if ( victim->fighting ) /* Flash */
    {
        send_to_char(" Nie mozesz tego uczynic podczas walki", ch );
        return rSPELL_FAILED;
    }	
    if ( ch->fighting ) /* Flash */
    {
        send_to_char(" Nie mozesz tego uczynic komus kto walczy", ch );
        return rSPELL_FAILED;
    }	
    if (victim->alignment > 900 ) /* Flash */
    {
        send_to_char("Ta postac jest chroniona przez Boskie moce", ch );
        return rSPELL_FAILED;
    }
    if (ch->alignment > -900 ) /* Flash */
    {
        send_to_char("Czynisz zbyt malo zla, by moc przywolac taka moc", ch);
        return rSPELL_FAILED;
    }	
    raw_kill( ch, victim );
...
ch_ret spell_assassinate( int sn, int level, CHAR_DATA *ch, void *vo )
{
    ...
    if ( IS_IMMORTAL( victim ) )
    {
        send_to_char( "Hmm...to nie jest dobry pomysl\n\r", ch );
        return rSPELL_FAILED;
    }

    if ( ch->mount )
    {
        send_to_char( "Nie mozesz tego uczynic gdy czegos dosiadasz.\n\r", ch );
        return rSPELL_FAILED;
    }

    if ( victim == ch )
    {
        send_to_char( "Jak chcesz zakrasc sie do siebie?\n\r", ch );
        return rSPELL_FAILED;
    }
    ...
    {
        send_to_char( "Potrzebujesz krotkiej, przebijajacej broni.\n\r", ch );
        return rSPELL_FAILED;
    }
    ...
    if ( victim->fighting )
    {
        send_to_char( "Nie mozesz zamordowac kogos kto walczy.\n\r", ch );
        return rSPELL_FAILED;
    }
    ...
    {
        if (victim->sex == 0)
            act( AT_PLAIN, "$N jest ranne i zmeczone ... trudno jest sie zakrasc.",
                    ch, NULL, victim, TO_CHAR );
        else if (victim->sex == 1)
            act( AT_PLAIN, "$N jest ranny i zmeczony ... trudno jest sie zakrasc.",
                    ch, NULL, victim, TO_CHAR );
        else 
            act( AT_PLAIN, "$N jest ranna i zmeczona ... trudno jest sie zakrasc.",
                    ch, NULL, victim, TO_CHAR );
    ...
    if ( !IS_AWAKE(victim)
            || ( 1+(int)(100.0*rand()/RAND_MAX+1.0)) > 50 )
    {
        raw_kill( ch, victim );
        act( AT_MAGIC, "Zamordowales $C!", ch, NULL, victim, TO_CHAR );
        act( AT_MAGIC, "$n zamordowal $C."
                , ch, NULL,
                victim, TO_NOTVICT );
        do_get( ch, "coins corpse" );
        do_look( ch, "in corpse" );
    ...
    {
        global_retcode = damage( ch, victim, 0, gsn_second_attack );
        return rSPELL_FAILED;
    }
...
ch_ret spell_revive( int sn, int level, CHAR_DATA *ch, void *vo )
{
    ...
    if ( !victim )
        victim = ch;
    tmp = victim->mental_state * 3 / 4; 
    victim->mental_state = tmp;
    if ( ch == victim )
    {
        act( AT_MAGIC, "Udalo Ci sie zlagodzic Twoje cierpienia umyslowe.", ch, NULL, NULL, TO_CHAR );
        return rNONE;
    }
    act( AT_MAGIC, "Udalo Ci sie zlagodzic cierpienia umyslowe $C.", ch, NULL, victim, TO_CHAR );
    act( AT_MAGIC, "$x udalo sie zlagodzic cierpienia umyslowe $C.", ch, NULL, victim, TO_NOTVICT );    
    act( AT_MAGIC, "$x udalo sie zlagodzic Twoje cierpienia umyslowe.", ch, NULL, victim, TO_VICT );    
    return rNONE;

    /* Hmm... co to w ogole mialo byc? - Crevan */
...
ch_ret spell_magnetic_thrust( int sn, int level, CHAR_DATA *ch, void *vo )
{
    ...
    level       = UMAX(0, level);
    level       = UMIN(29, level);
    dam         = .50* ((5*level*number_range( 1, 6 )) +16);
