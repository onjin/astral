...
char *	const	dir_name_pl	[]		=
{
    "polnoc", "wschod", "poludnie", "zachod", "gora", "dol",
    "polnocny-wschod", "polnocny-zachod", "poludniowy-wschod", 
    "polodniowy-zachod", "gdzies"
};
...
void decorate_room( ROOM_INDEX_DATA *room )
{
    ...
    room->name	  = STRALLOC( "Nieziemska okolica" );
    room->description = STRALLOC( "Znajdujesz sie na dziwnej, mglistej sciezce.\n\r" );
...
void do_open( CHAR_DATA *ch, char *argument )
{
    ...
    if ( ( obj = get_obj_here( ch, arg ) ) != NULL )
    {
        ...
        { 
            ch_printf( ch, "%s to nie pojemnik.\n\r", capitalize( obj->short_descr ) ); 
            return;
        } 
...
void do_stand( CHAR_DATA *ch, char *argument )
{
    switch ( ch->position )
    {
        case POS_SLEEPING:
            ...
            { send_to_char( "Jakos nie mozesz sie obudzic!\n\r", ch ); return; }

            send_to_char( "Budzisz sie i szybko zrywasz na rowne nogi.\n\r", ch );
            act( AT_ACTION, "$n budzi sie ze swej drzemki.", ch, NULL, NULL, TO_ROOM );
            ch->position = POS_STANDING;
            break;

        case POS_RESTING:
            send_to_char( "Konczysz odpoczynek i wstajesz.\n\r", ch );
            act( AT_ACTION, "$n konczy swoj wypoczynek i wstaje.", ch, NULL, NULL, TO_ROOM );
            ch->position = POS_STANDING;
            break;

        case POS_SITTING:
            send_to_char( "Szybko zrywasz sie na nogi.\n\r", ch );
            act( AT_ACTION, "$n wstaje.", ch, NULL, NULL, TO_ROOM );
            ch->position = POS_STANDING;
            break;

        case POS_STANDING:
            send_to_char( "Przeciez juz stoisz.\n\r", ch );
            break;
        ...
        case POS_AGGRESSIVE:
        case POS_BERSERK:
            send_to_char( "Jestes w trakcie walki!\n\r", ch );
            break;
    }
...
void do_sit( CHAR_DATA *ch, char *argument )
{
    switch ( ch->position )
    {
        case POS_SLEEPING:
            ...
            send_to_char( "Budzisz sie i siadasz.\n\r", ch );
            act( AT_ACTION, "$n budzi sie i siada.", ch, NULL, NULL, TO_ROOM );
            ch->position = POS_SITTING;
            break;

        case POS_RESTING:
            send_to_char( "Konczysz odpoczynek i siadasz.\n\r", ch );
            act( AT_ACTION, "$n konczy wypoczynek i siada.", ch, NULL, NULL, TO_ROOM );
            ch->position = POS_SITTING;
            break;

        case POS_STANDING:
            send_to_char( "Siadasz.\n\r", ch );
            act( AT_ACTION, "$n siada.", ch, NULL, NULL, TO_ROOM );
            ch->position = POS_SITTING;
            break;
        case POS_SITTING:
            send_to_char( "Przeciez juz siedzisz.\n\r", ch );
            return;
        ...
        case POS_AGGRESSIVE:
        case POS_BERSERK:
            send_to_char( "Zajmij sie lepiej walka!\n\r", ch );
            return;
        case POS_MOUNTED:
            send_to_char( "Przeciez jus siedzisz - na wierzchowcu.\n\r", ch );
            return;
    }
...
void do_rest( CHAR_DATA *ch, char *argument )
{
    switch ( ch->position )
    {
        case POS_SLEEPING:
            ...
            send_to_char( "Budzisz sie.\n\r", ch );
            act( AT_ACTION, "$n budzi sie ze snu.", ch, NULL, NULL, TO_ROOM );
            ch->position = POS_RESTING;
            break;

        case POS_RESTING:
            send_to_char( "Przeciez juz wypoczywasz.\n\r", ch );
            return;

        case POS_STANDING:
            send_to_char( "Rozciagasz sie wygodnie.\n\r", ch );
            act( AT_ACTION, "$n rozciaga sie wygodnie.", ch, NULL, NULL, TO_ROOM );
            ch->position = POS_RESTING;
            break;

        case POS_SITTING:
            send_to_char( "Kladziesz sie w tyl i wypoczywasz.\n\r", ch );
            act( AT_ACTION, "$n kladzie sie w tyl i wypoczywa.", ch, NULL, NULL, TO_ROOM );
            ch->position = POS_RESTING;
            break;
        ...
        case POS_BERSERK:
            ...
            return;
        case POS_MOUNTED:
            send_to_char( "Hmm... a moze by tak najpierw zsiasc.\n\r", ch );
            return;
    }
...
void do_sleep( CHAR_DATA *ch, char *argument )
{
    ...
    {
        case POS_SLEEPING:
            send_to_char( "Przeciez juz spisz.\n\r", ch );
            return;

        case POS_RESTING:
            if ( ch->mental_state > 30 && (number_percent()+10) < ch->mental_state )
            {
                send_to_char( "Nie mozesz spokojnie zasnac.\n\r", ch );
                act( AT_ACTION, "$n zamyka oczy ale nie moze usnac.", ch, NULL, NULL, TO_ROOM );
                return;
            }
            send_to_char( "Zamykasz oczy i zaczynasz snic.\n\r", ch );
            act( AT_ACTION, "$n zamyka oczy i zasypia.", ch, NULL, NULL, TO_ROOM );
        ...
        case POS_SITTING:
            ...
            {
                send_to_char( "Nie mozesz spokojnie zasnac.\n\r", ch );
                act( AT_ACTION, "$n zamyka oczy ale nie moze zasnac.", ch, NULL, NULL, TO_ROOM );
                return;
            }
            send_to_char( "Przewracasz sie na bok i zasypiasz.\n\r", ch );
            act( AT_ACTION, "$n przewraca sie na bok i zapada w sen.", ch, NULL, NULL, TO_ROOM );
            ch->position = POS_SLEEPING;
            break;
        ...
        case POS_STANDING: 
            ...
            send_to_char( "Zapadasz w gleboki sen.\n\r", ch );
            act( AT_ACTION, "$n zapada w gleboki sen.", ch, NULL, NULL, TO_ROOM );
            ch->position = POS_SLEEPING;
            break;
        ...
        case POS_BERSERK:
            ...
            return;
        case POS_MOUNTED:
            send_to_char( "Chcesz spac w siodle??.\n\r", ch );
            return;
    }
...
void do_wake( CHAR_DATA *ch, char *argument )
{
    ...
    if ( !IS_AWAKE(ch) )
    { send_to_char( "Przeciez Ty takze spisz!\n\r",       ch ); return; }

    if ( ( victim = get_char_room( ch, arg ) ) == NULL )
    { send_to_char( "Nie ma tu kogos takiego.\n\r",              ch ); return; }

    if ( IS_AWAKE(victim) )
    { act( AT_PLAIN, "$N juz dawno nie spi.", ch, NULL, victim, TO_CHAR ); return; }

    if ( IS_AFFECTED(victim, AFF_SLEEP) || victim->position < POS_SLEEPING )
    { act( AT_PLAIN, "Jakos nie mozesz dobudzic $C!",  ch, NULL, victim, TO_CHAR );  return; }

    act( AT_ACTION, "Budzisz $B.", ch, NULL, victim, TO_CHAR );
    victim->position = POS_STANDING;
    act( AT_ACTION, "$n budzi Cie.", ch, NULL, victim, TO_VICT );
    return;
}
...
void teleportch( CHAR_DATA *ch, ROOM_INDEX_DATA *room, bool show )
{
    ...
    if ( room_is_private( room ) )
        return;
    act( AT_ACTION, "$n nagle znika!", ch, NULL, NULL, TO_ROOM );
    char_from_room( ch );
    char_to_room( ch, room );
    act( AT_ACTION, "$n nagle sie pojawia!", ch, NULL, NULL, TO_ROOM );
    if ( show )
        do_look( ch, "auto" );
    if ( IS_SET( ch->in_room->room_flags, ROOM_DEATH ) && !IS_IMMORTAL( ch ) )
    {
        act( AT_DEAD, "$n pada ofiara strasznej smierci!", ch, NULL, NULL, TO_ROOM );
        set_char_color( AT_DEAD, ch );
        send_to_char( "Oopsie... nie zyjesz!\n\r", ch );
        sprintf(buf, "%s wpada na SMIERTELNA PULAPKE w pokoju %d!",
                ch->name, ch->in_room->vnum );
        log_string( buf );
        to_channel( buf, CHANNEL_MONITOR, "Monitor", LEVEL_IMMORTAL );
        extract_char( ch, FALSE );
...
void do_climb( CHAR_DATA *ch, char *argument )
{
    ...
    if ( argument[0] == '\0' )
    {
        ...
        send_to_char( "Nie mozesz sie tu wspinac.\n\r", ch );
        return;
    }
...
void do_enter( CHAR_DATA *ch, char *argument )
{
    ...
    if ( argument[0] == '\0' )
    {
        ...
        send_to_char( "Nie mozesz znalesc zadnego wejscia.\n\r", ch );
        return;
    }
    ...
    send_to_char( "Nie mozesz do tego wejsc.\n\r", ch );
    return;
}
...
void do_leave( CHAR_DATA *ch, char *argument )
{
    ...
    if ( argument[0] == '\0' )
    {
        ...
        send_to_char( "Nie mozesz znalesc wyjscia.\n\r", ch );
        return;
    }
    ...
    send_to_char( "Nie mozesz tedy wyjsc.\n\r", ch );
    return;
}
