...
int imc_readconfig(void)
{
  ...
  const char *value;
  int version=-1;
  unsigned long localip = INADDR_ANY;
  
  char iname[200] = "", ihost[200] = "", iemail[200] = "", iimail[200] = "",
    iwww[200] = "", idetails[1000] = "", iflags[200] = "";
...
int imc_command(const char *argument)
{
  ...
  else if (!strcasecmp(arg1, "localaddr"))
  {
    unsigned long localip = INADDR_ANY;
    
    if (strcasecmp(arg2, "any"))
      localip = inet_addr(arg2);
    
    if (localip == (unsigned long)-1)
    {
      imc_qerror("Invalid IP");
      return -1;
    }
...
#ifdef WIN32
  return str_infix(haystack, needle);
#else
...
const char *imc_list(int level)
{
  ...
  if (level==5)
  {
    sprintf(buf,
	    ...
	    inet_ntoa(*(struct in_addr *)&imc_bind),
	    imc_port,
	    IMC_VERSIONID,
