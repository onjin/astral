...
void write_clan_list( )
{
    ...
    if ( !fpout )
    {
	bug( "FATALNIE: nie moge zapisywac do pliku clan.lst !\n\r", 0 );
 	return;
    }	  
...
void write_council_list( )
{
    ...
    if ( !fpout )
    {
	bug( "FATALNIE: nie moge zapisywac doo pliku council.lst !\n\r", 0 );
 	return;
    }	  
...
void save_clan( CLAN_DATA *clan )
{
    ...
    {
	sprintf( buf, "save_clan: %s nie ma swojego pliku", clan->name );
	bug( buf, 0 );
	return;
    }
...
void save_council( COUNCIL_DATA *council )
{
    ...
    {
	sprintf( buf, "save_council: %s nie ma swojego pliku", council->name );
	bug( buf, 0 );
	return;
    }
...
void fread_clan( CLAN_DATA *clan, FILE *fp )
{
	...
	case 'A':
	    KEY( "Account",	clan->account,		fread_string( fp ) );
	...
	case 'C':
	    KEY( "Channel",	clan->channel,		fread_string( fp ) );
	    ...
	    KEY( "ClanTax",	clan->clantax,		fread_number( fp ) );
	    KEY( "Class",	clan->class,		fread_number( fp ) );
	    break;

	case 'D':
	    KEY( "Deity",	clan->deity,		fread_string( fp ) );
	    KEY( "Deity2",	clan->deity2,		fread_string( fp ) );
	    KEY( "Deity3",	clan->deity3,		fread_string( fp ) );
	...
	case 'E':
		...
		if (!clan->deity)
		  clan->deity		= STRALLOC( "" );
		if (!clan->deity2)
		    clan->deity2	= STRALLOC( "" );
		if (!clan->deity3)
		    clan->deity3	= STRALLOC( "" );
		if (!clan->channel)
		    clan->channel	= STRALLOC( "" );
		if (!clan->badge)
	  	  clan->badge		= STRALLOC( "" );
		return;
	    }
...
bool load_clan_file( char *clanfile )
{
	...
	{
	    log_string( "Storeroom nie odnaleziony" );
	    return found;
	}
	...
	if ( ( fp = fopen( filename, "r" ) ) != NULL )
	{
	    ...
	    log_string( "Laduje storeroom klanu" );
...
void do_make( CHAR_DATA *ch, char *argument )
{
    ...
    if ( !pObjIndex || !is_name( arg, pObjIndex->name ) )
    {
	send_to_char( "Nie wiesz jak to zrobic.\n\r", ch );
	return;
    }
    ...
    act( AT_MAGIC, "$n stwarza $v!", ch, obj, NULL, TO_ROOM );
    act( AT_MAGIC, "Tworzysz $v!", ch, obj, NULL, TO_CHAR );
    return;
}
...
void do_induct( CHAR_DATA *ch, char *argument )
{
    ...
    if ( arg[0] == '\0' )
    {
	send_to_char( "Kogo chcesz przyjac?\n\r", ch );
	return;
    }
    ...
    {
	send_to_char( "Nie jestes w stanie przyjac Tej Osoby.\n\r", ch );
	return;
    }
    /* Flash noclan */
    if (IS_SET(victim->pcdata->flags, PCFLAG_NOCLAN))
    {
	send_to_char( "Ten gracz nie pozwala przylaczac sie do klanow", ch );
	return;
    }
	...
	{
	    send_to_char( "Ten gracz nie moze nalezec do tego klanu.\n\r", ch);
            return;
	}
    }
    else
	...
	if ( victim->level < 10 )
	{
	    send_to_char( "Ta osoba nie jest gotowa by wstapic do klanu.\n\r", ch );
	    return;
	}

	if ( victim->level > ch->level )
	{
	    send_to_char( "Ten gracz jest zbyt silny dla Ciebie by go przylaczyc do klanu.\n\r", ch );
	    return;
	}
    }
	...
	if ( victim->pcdata->clan == clan )
	  send_to_char( "Ten gracz juz nalezy do Twego zakonu!\n\r", ch );
	else
	  send_to_char( "Ten gracz juz nalezy do jakiegos zakonu!\n\r", ch );
	return;
      }
      else
      ...
      {
	if ( victim->pcdata->clan == clan )
	  send_to_char( "Ten gracz juz nalezy do Twojej gildii!\n\r", ch );
	else
	  send_to_char( "Ten gracz juz nalezy do jakiejs gildii!\n\r", ch );
	return;
      }
      else
      {
	if ( victim->pcdata->clan == clan )
	  send_to_char( "Ten gracz nalezy juz do Twego klanu!\n\r", ch );
	else
	  send_to_char( "Ten gracz nalezy juz do jakiegos klanu!\n\r", ch );
	return;
      }
    }
    if ( clan->mem_limit && clan->members >= clan->mem_limit )
    {
    	send_to_char( "W Twym klanie nie zmiesci sie wiecej osob.\n\r", ch );
	return;
    }
    clan->members++;
	...
	for ( sn = 0; sn < top_sn; sn++ )
	{
		...
		ch_printf( victim, "%s uczy Cie jak poslugiwac sie nowa umiejetnoscia %s.\n\r", ch->name, skill_table[sn]->name);
	    }
	}
    }
    ...
    act( AT_MAGIC, "Przyjmujesz $B do $t", ch, clan->name, victim, TO_CHAR );
    act( AT_MAGIC, "$n przyjmuje $B do $t", ch, clan->name, victim, TO_NOTVICT );
    act( AT_MAGIC, "$n przyjmuje Cie $t", ch, clan->name, victim, TO_VICT );
...
void do_outcast( CHAR_DATA *ch, char *argument )
{
    ...
    if ( arg[0] == '\0' )
    {
	send_to_char( "Kogo checsz wyrzucic?\n\r", ch );
	return;
    }
    ...
    if ( IS_NPC(victim) )
    {
	send_to_char( "Nie mozna wyrzucic moba.\n\r", ch );
	return;
    }
	...
	{
	    send_to_char( "Chesz wyrzucic siebie z WLASNEGO zakonu?\n\r", ch );
	    return;
	}
	else
	if ( ch->pcdata->clan->clan_type == CLAN_GUILD )
	{
	    send_to_char( "Chcesz wyrzucic siebie z WLASNEJ gildii?\n\r", ch );
	    return;
	}
	else
	{
	    send_to_char( "Chcesz wyrzucic siebie z WLASNEGO klanu?\n\r", ch );
	    return;
	}
    }
	...
	if ( ch->pcdata->clan->clan_type == CLAN_ORDER )
	{
	    send_to_char( "Ten gracz nie nalezy do Twego zakonu!\n\r", ch );
	    return;
	}
	else
	if ( ch->pcdata->clan->clan_type == CLAN_GUILD )
	{
	    send_to_char( "Ten gracz nie nalezy do Twej gildii!\n\r", ch );
	    return;
	}
	else
	{
	    send_to_char( "Ten gracz nie nalezy do Twego klanu!\n\r", ch );
	    return;
	}
    }
	...
	for ( sn = 0; sn < top_sn; sn++ )
		...
		ch_printf( victim, "Zapominasz jak uzywac %s.\n\r", skill_table[sn]->name);
	    }
    }
    ...
    act( AT_MAGIC, "Wurzucasz $B z $t", ch, clan->name, victim, TO_CHAR );
    act( AT_MAGIC, "$n wyrzuca $B z $t", ch, clan->name, victim, TO_ROOM );
    act( AT_MAGIC, "$n wyrzuca Cie z $t", ch, clan->name, victim, TO_VICT );
    if ( clan->clan_type != CLAN_GUILD )
    {
	sprintf(buf, "%s zostaje wyrzucon%s z %s!", 
	victim->name, 
...
{
    send_to_char("Jesli chcesz opuscic klan wpisz cala komende LEAVECLAN\n\r", ch);
    return;
}
void do_leaveclan( CHAR_DATA *ch, char *argument )
{
	...
	for ( sn = 0; sn < top_sn; sn++ )
	    if ( skill_table[sn]->guild == ch->pcdata->clan->class
	    &&   skill_table[sn]->name != NULL )
	    {
		ch->pcdata->learned[sn] = 0;
		ch_printf( ch, "Zapominasz jak uzywac %s.\n\r", skill_table[sn]->name);
	    }
    }
    ...
    act( AT_MAGIC, "Opuszczasz $t", ch, clan->name, ch, TO_CHAR );
    act( AT_MAGIC, "$n opuszcza $t", ch, clan->name, ch, TO_ROOM );
    if ( clan->clan_type != CLAN_GUILD )
    {
	sprintf(buf, "%s opuszcza %s!", ch->name, clan->name);
	echo_to_all(AT_MAGIC, buf, ECHOTAR_ALL);
    }
...
void do_setclan( CHAR_DATA *ch, char *argument )
{
	...
	{
	  send_to_char(" pkill1-7 pdeath1-7 clantax balance\n\r", ch );
	  send_to_char(" +balance -balance account\n\r", ch );
	}
	return;
    }
    ...
    {
	STRFREE( clan->channel );
	clan->channel = STRALLOC( argument );
...
/* Obsluga wielobostwa klanowego - Crevan */
/* Zerknij tez deity.c - bostwa klanowe   */

    if ( !str_cmp( arg2, "deity2" ) )
    {
	STRFREE( clan->deity2 );
	clan->deity2 = STRALLOC( argument );
	send_to_char( "Zrobione.\n\r", ch );
	save_clan( clan );
	return;
    }
    if ( !str_cmp( arg2, "deity3" ) )
    {
	STRFREE( clan->deity3 );
	clan->deity3 = STRALLOC( argument );
	send_to_char( "Zrobione.\n\r", ch );
	save_clan( clan );
	return;
    }
...
/* Obsluga konta i podatku klanowego - Crevan */

    if ( !str_cmp( arg2, "account" ) )
    {
	STRFREE( clan->account );
	clan->account = STRALLOC( argument );
	send_to_char( "Zrobione.\n\r", ch );
	save_clan( clan );
	return;
    }
    if ( !str_cmp( arg2, "clantax" ) ) 
    {
	if ( atoi( argument ) < 0 || atoi( argument ) > 100 )
	{
	    send_to_char( "Podatek klanowy musi byc podany w procentach ( 0 - 100 ).\n\r", ch );
	    return;
	}
	clan->clantax = atoi( argument );
	send_to_char( "Zrobione.\n\r", ch );
	save_clan( clan );
	return;
    }
...
{   
    ...
    ch_printf_color( ch, "\n\r&w%s: &W%s\t\tBadge: %s\n\r&wFilename : &W%s\n\r&wMotto    : &W%s\n\r",
			clan->clan_type == CLAN_ORDER ? "Order    " :
		       ...
		       (clan->clan_type == CLAN_NOKILL ? "Clan NoPk" :"Clan Pk  ")),
    			clan->name,
			clan->badge ? clan->badge : "(not set)",
    			clan->filename,
    			clan->motto );
    ch_printf_color( ch, "&wDesc     : &W%s\n\r&wDeity   : &W%s\t&wDeity2   : &W%s\t&wDeity3   : &W%s\n\r&wLeader   : &W%s\n\r",
    			clan->description,
    			clan->deity, clan->deity2, clan->deity3,
    			clan->leader );
    ...
    ch_printf_color( ch, "&wGuard1   : &W%-5d    &wGuard2 : &W%-5d\n\r",
 			clan->guard1,
			clan->guard2 );
    ch_printf_color( ch, "&wClantax  : %-2d%%      Account: %s   Balance:\t%d\n\r", clan->clantax, clan->account, clan->balance );
...
void do_makeclan( CHAR_DATA *ch, char *argument )
{
    ...
    {
	send_to_char( "Uzycie: makeclan <nazwa klanu>\n\r", ch );
	return;
    }
...
void do_clans( CHAR_DATA *ch, char *argument )
{
    ...
    if ( argument[0] == '\0' )
    {
        ...
        send_to_char( "\n\rKlan            Bostwo        Lider            Pkills:    Avatar      Inne \n\r___________________________________________________________________________\n\r\n\r", ch );
        for ( clan = first_clan; clan; clan = clan->next )
        {
            if ( clan->clan_type == CLAN_ORDER || clan->clan_type == CLAN_GUILD )
              continue;
            set_char_color( AT_GREY, ch);
            ch_printf( ch, "%-15s %-13s %-13s", clan->name, clan->deity, clan->leader );
            set_char_color( AT_BLOOD, ch );
	if ( clan->clan_type != CLAN_NOKILL )
        ...
        set_char_color( AT_BLOOD, ch );
        if ( !count )
          send_to_char( "Aktualnie nie istnieja zadne klany.\n\r", ch );
        else
    ...
    set_char_color( AT_GREY, ch );
    send_to_char_color( "Zwyciestwa:&w\n\r", ch );
   ch_printf_color( ch, "    &w15-19...  &r%-4d\n\r    &w20-29...  &r%-4d\n\r    &w30-39...  &r%-4d\n\r    &w40-49...  &r%-4d\n\r",
	...
	clan->pkills[6] );
    set_char_color( AT_GREY, ch );
    ch_printf( ch, "Wodz klanu      :  %s\n\rPierwszy Oficer :  %s\n\rDrugi Oficer    :  %s\n\r",
                        clan->leader,
                        clan->number1,
                        clan->number2 );
    ch_printf( ch, "Bostw%s          :  %s%s %s%s %s\n\r", 
			( str_cmp( clan->deity2, "" ) ) ? "a" : "o", 
			( str_cmp( clan->deity, "" ) ) ? clan->deity : "", 
			( str_cmp( clan->deity2, "" ) ) ? "," : "", 
			( str_cmp( clan->deity2, "" ) ) ? clan->deity2 : "",
			( str_cmp( clan->deity3, "" ) ) ? "," : "", 
			( str_cmp( clan->deity3, "" ) ) ? clan->deity3 : "" );
    ch_printf( ch, "Konto klanowe   :  %s\n\r", clan->account );
    if ( !str_cmp( ch->name, clan->deity   )
    ||   !str_cmp( ch->name, clan->leader  )
    ||   !str_cmp( ch->name, clan->number1 )
    ||   !str_cmp( ch->name, clan->number2 ) )
	ch_printf( ch, "Czlonkowie      :  %d\n\r",
			clan->members );
    set_char_color( AT_BLOOD, ch );
    ch_printf( ch, "\n\rOpis klanu:  %s\n\r", clan->description );
    return;
