...
#define MAX_PERSONAL		    6   /* Maximum personal skills */
...
#define PULSE_AUCTION				 (12 * PULSE_PER_SECOND)
...
#define CLASS_PALADIN	    11
/* #define CLASS_NEPHANDI	    12 */
...
struct	clan_data
{
    ...
    char *	deity2;		/* Politeizm klanowy - Crevan		*/
    char *	deity3;
    char *	channel;
    ...
    unsigned long	balance;	/* Konto klanowe			*/
    sh_int	clantax;	/* % kasy z zabicia kazdego moba na klan*/
    char *	account;	/* nazwa konta klanowego		*/
...
#define ACT_BANKER	         22		/* Free for use */
...
#define ACT_NOQUEST		 28		/* Other mobs will attack */
#define ACT_PROTOTYPE		 29		/* A prototype mob	*/
/* 26 acts */
...
typedef enum
{
  ...
  AFF_CONTAGIOUS, AFF_PESTILENCE, MAX_AFFECTED_BY
} affected_by_types;
...
#define OBJ_VNUM_ASTRAL_BLADE	     66
#define OBJ_VNUM_BLOODLET	     17 /* Flash */
...
/* Objecty do nowych czarow - Crevan */

#define OBJ_VNUM_SZKIELET	     67
#define OBJ_VNUM_SZKIELET_RESZTKI    68
...
typedef enum
{
  ...
  ITEM_PROTOTYPE, ITEM_ANTI_DEMON, ITEM_NOLOCATE, ITEM_GROUNDROT, ITEM_NOQUEST, ITEM_QUEST, MAX_ITEM_FLAG
} item_extra_flags;
...
#define ROOM_NOQUEST		BV28
#define ROOM_PROTOTYPE	     	BV29
...
#define PCFLAG_NOCLAN		   BV16 /* nie pozwala na przylaczanie */
#define PCFLAG_NOBEEP		   BV17  /* Nie pozwala na beepanie :) */
...
struct	mob_index_data
{
    ...
    char *		player_name;
    char *		short_descr;
    char *		short_descr_dop;
    char *		short_descr_cel;
    char *		short_descr_bie;
    char *		short_descr_nar;
    char *		short_descr_mie;
    char *		short_descr_wol;
    char *		long_descr;
    char *		description;
    int			vnum;
    sh_int		count;
...
#define IS_BANKER(ch)		(xIS_SET((ch)->act, ACT_BANKER))
...
#define PERS(ch, looker)	( can_see( (looker), (ch) ) ?		\
				...
				: (ch)->name ) : "someone" )
#define PERSC(ch, looker)	( can_see( (looker), (ch) ) ?		\
				( IS_NPC(ch) ? (ch)->short_descr_dop	\
				: (ch)->ncel ) : "someone" )
#define PERSD(ch, looker)	( can_see( (looker), (ch) ) ?		\
				( IS_NPC(ch) ? (ch)->short_descr_cel	\
				: (ch)->ndop ) : "someone" )
#define PERSB(ch, looker)	( can_see( (looker), (ch) ) ?		\
				( IS_NPC(ch) ? (ch)->short_descr_bie	\
				: (ch)->nbie ) : "someone" )
#define PERSN(ch, looker)	( can_see( (looker), (ch) ) ?		\
				( IS_NPC(ch) ? (ch)->short_descr_nar	\
				: (ch)->nnar ) : "someone" )
#define PERSM(ch, looker)	( can_see( (looker), (ch) ) ?		\
				( IS_NPC(ch) ? (ch)->short_descr_mie	\
				: (ch)->nmie ) : "someone" )
#define PERSW(ch, looker)	( can_see( (looker), (ch) ) ?		\
				( IS_NPC(ch) ? (ch)->short_descr_wol	\
				: (ch)->nwol ) : "someone" )

#define MORPHPERS(ch, looker)   ( can_see( (looker), (ch) ) ?           \
                                (ch)->morph->morph->short_desc       \
                                : "someone" )
#define MORPHPERSC(ch, looker)   ( can_see( (looker), (ch) ) ?           \
                                (ch)->morph->morph->short_desc       \
                                : "someone" )
#define MORPHPERSD(ch, looker)   ( can_see( (looker), (ch) ) ?           \
                                (ch)->morph->morph->short_desc       \
                                : "someone" )
#define MORPHPERSB(ch, looker)   ( can_see( (looker), (ch) ) ?           \
                                (ch)->morph->morph->short_desc       \
                                : "someone" )
#define MORPHPERSN(ch, looker)   ( can_see( (looker), (ch) ) ?           \
                                (ch)->morph->morph->short_desc       \
                                : "someone" )
#define MORPHPERSM(ch, looker)   ( can_see( (looker), (ch) ) ?           \
                                (ch)->morph->morph->short_desc       \
                                : "someone" )
#define MORPHPERSW(ch, looker)   ( can_see( (looker), (ch) ) ?           \
                                (ch)->morph->morph->short_desc       \
                                : "someone" )
...
extern	char *	const			attack_table_other	[18];
...
DECLARE_DO_FUN( do_odmie	);
DECLARE_DO_FUN( do_odmien	);
DECLARE_DO_FUN( do_oodstat	);
...
DECLARE_SPELL_FUN(	spell_astral_blade	);/* Crevan */
...
DECLARE_SPELL_FUN(	spell_transfer_energy	);/* Crevan */
...
#define BACKUP_DIR	"../backup/player/" /* Backup Player files	*/
...
char *	obj_shortd	args( ( OBJ_DATA *obj ) );
char *	obj_shortc	args( ( OBJ_DATA *obj ) );
char *	obj_shortb	args( ( OBJ_DATA *obj ) );
char *	obj_shortn	args( ( OBJ_DATA *obj ) );
char *	obj_shortm	args( ( OBJ_DATA *obj ) );
char *	obj_shortw	args( ( OBJ_DATA *obj ) );
...
void	show_list_to_char2	args( ( OBJ_DATA *list, CHAR_DATA *ch,
				    bool fShort, bool fShowNothing ) );
