  ...
  if ( !IS_PKILL(ch) && !IS_IMMORTAL(ch) )
{
  ...
  if ( argument[0] == '\0' )
  {
   send_to_char("Jakie cialo chcesz obedrzec ze skory?\n\r", ch);
   return;
  }
  if ( ( obj = get_eq_char( ch, WEAR_WIELD ) ) == NULL)
  {
  send_to_char("Potrzebujesz do tego bardzo ostrej broni.\n\r", ch);
  return;
  }

  if ( (corpse = get_obj_here( ch, argument )) == NULL)
  {
  send_to_char("Nie mozesz tu znalesc takiego ciala.\n\r", ch);
  return;
  }

  if (corpse->item_type != ITEM_CORPSE_PC )
  {
    send_to_char("Mozesz obedrzec ze skory tylko ciala innych graczy.\n\r", ch);
    return;
  }

  if (corpse->value[3] != 1)
  {
    send_to_char("Nie mozesz obedrzec tego ciala.\n\r", ch);
    return;
  }
  
  if(corpse->value[4] != 0)/*line added by Daht*/
  {
  ...
  name                = IS_NPC(ch) ? korps->short_descr : corpse->short_descr;
   ...
   sprintf( buf, skin->short_descr_dop, name );
   STRFREE( skin->short_descr_dop );
   skin->short_descr_dop = STRALLOC( buf );
   sprintf( buf, skin->short_descr_cel, name );
   STRFREE( skin->short_descr_cel );
   skin->short_descr_cel = STRALLOC( buf );
   sprintf( buf, skin->short_descr_bie, name );
   STRFREE( skin->short_descr_bie );
   skin->short_descr_bie = STRALLOC( buf );
   sprintf( buf, skin->short_descr_nar, name );
   STRFREE( skin->short_descr_nar );
   skin->short_descr_nar = STRALLOC( buf );
   sprintf( buf, skin->short_descr_mie, name );
   STRFREE( skin->short_descr_mie );
   skin->short_descr_mie = STRALLOC( buf );
   sprintf( buf, skin->short_descr_wol, name );
   STRFREE( skin->short_descr_wol );
   skin->short_descr_wol = STRALLOC( buf );
...
is not deleted */
  ...
  send_to_char ("To cialo juz jest obdarte ze skory!\n\r", ch);
  return;/*final touches--Daht*/
}


/* fletch przerobiony z czaru na skilla - Crevan */
void do_fletch( CHAR_DATA *ch, char *argument )
{
    ...
    {
	if ( ( obj->pIndexData->vnum == 21086 ) && ( obj->carried_by == ch ) )
	{
	    found = TRUE;
	    break;
	}
	...
	separate_obj( obj );
	extract_obj( obj );
	obj = create_object( get_obj_index( 21085 ), 0 );
	obj_to_char( obj, ch );
	act( AT_ACTION, "$n tworzy magiczna strzale!", ch, NULL, NULL, TO_ROOM );
	act( AT_ACTION, "Tworzysz magiczna strzale!", ch, NULL, NULL, TO_CHAR );
	return;
    }
    else
    {
    send_to_char( "Czegos tu brakuje...\n\r", ch );
    return;
    }
}
...
void do_bash( CHAR_DATA *ch, char *argument )
{
    ...
    if ( IS_NPC(ch) && IS_AFFECTED( ch, AFF_CHARM ) )
    {
	send_to_char( "Nie mozesz sie na tyle skoncentrowac, by to wykonac.\n\r", ch );
	return;
    }
    ...
    {
	send_to_char(
	    "Lepiej zostaw wojenne sztuczki wojownikom.\n\r", ch );
	return;
    }

    if ( ( victim = who_fighting( ch ) ) == NULL )
    {
	send_to_char( "Z nikim teraz nie walczysz.\n\r", ch );
	return;
    }
...
void do_bloodlet( CHAR_DATA *ch, char *argument )
{
    ...
    if ( ch->fighting )
    {
	send_to_char( "Walka jest zbyt zajmujaca...\n\r", ch );
	return;
    }
    if ( ch->pcdata->condition[COND_BLOODTHIRST] < 10 )
    {
	send_to_char( "Masz zbyt malo krwi, by ja komus ofiarowac...\n\r", ch );
	return;
    }
	...
	act( AT_BLOOD, "Ostrym paznokciem rozrywasz skore na przedramieniu, by podarowac swa krew.", ch, NULL, NULL, TO_CHAR );
	act( AT_BLOOD, "$n rozrywa ostrym paznokciem Swa skore, by krew mogla swobodnie wyplynac.", ch, NULL, NULL, TO_ROOM );
    ...
    {
	act( AT_BLOOD, "Nie udaje Ci sie wytoczyc ani kropli krwi...", ch, NULL, NULL, TO_CHAR );
	act( AT_BLOOD, "$n szarpie paznokciami Swe ramie, lecz krew nie plynie...", ch, NULL, NULL, TO_ROOM );
...
void do_feed( CHAR_DATA *ch, char *argument )
{
	...
	if ( IS_NPC(ch) && IS_AFFECTED( ch, AFF_CHARM ) )
	{
	  send_to_char( "Nie mozesz tego teraz zrobic.\n\r", ch );
	  return;
	}

	if ( (!IS_NPC(ch)
	&&   !IS_VAMPIRE(ch)) && !IS_DEMON(ch))
	{
	  send_to_char( "Picie krwi z zywych istot nie lezy w twej naturze.\n\r", ch );
	  return;
	}
        if ( !can_use_skill(ch, 0, gsn_feed ) )
	{
	  send_to_char( "Nie cwiczyles jeszcze swych nowych zebow.\n\r", ch );
	  return;
	}
	...
	if ( ch->mount )
	{
	  send_to_char( "Nie mozesz tego robic gdy czegos dosiadasz.\n\r", ch );
	  return;
	}
        ...
        if ( can_use_skill( ch, number_percent(), gsn_feed ) )
	{
	  ...
	  &&  ch->pcdata->condition[COND_BLOODTHIRST] < (10 + ch->level) )
	  {
	    ...
	    act( AT_BLOOD, "Wysysasz czesc zycia z $C.", ch, NULL,
		 victim, TO_CHAR );
	    act( AT_BLOOD, "$n wysysa ci troche krwi!", ch, NULL, victim, TO_VICT );
	...
	else
	{
	  ...
	  &&  ch->pcdata->condition[COND_BLOODTHIRST] < (10 + ch->level) )
	  {
	    act( AT_BLOOD, "Zapach krwi $C doprowadza cie do szalenstwa!",
		ch, NULL, victim, TO_CHAR );
	    act( AT_BLOOD, "$n ma ochote na twoja krew!", ch, NULL, victim, TO_VICT );
...
void do_recall( CHAR_DATA *ch, char *argument )
{
    ...
    {
	if (ch->sex == 0)
	send_to_char( "Czujesz sie kompletnie zagubiony.\n\r", ch );
	else if (ch->sex == 1)
	send_to_char( "Czujesz sie kompletnie zagubiony.\n\r", ch );
	else
	send_to_char( "Czujesz sie kompletnie zagubiony.\n\r", ch );
    ...
    {
	send_to_char( "Z dziwnego powodu... nic sie nie stalo.\n\r", ch );
	return;
    }

    if ( IS_AFFECTED(ch, AFF_CURSE) )
    {
        send_to_char("Ciazy na Tobie przeklenstwo i Bogowie odsuneli sie od Ciebie!\n\r", ch );
        return;
    }
	...
	if ( number_bits( 1 ) == 0 || ( !IS_NPC( opponent ) && number_bits( 3 ) > 1 ) )
	{
	    ...
	    if ( ch->desc )
	      lose /= 2;
	    gain_exp( ch, 0 - lose );
	    ch_printf( ch, "Nie udalo sie!  Tracisz %d pkt. doswiadczenia.\n\r", lose );
	    return;
	}
	...
	if ( ch->desc )
	  lose /= 2;
	gain_exp( ch, 0 - lose );
	ch_printf( ch, "Uciekasz z walki!  Tracisz %d pkt. doswiadczenia.\n\r", lose );
	stop_fighting( ch, TRUE );
    }

    act( AT_ACTION, "$n znika w klebach dymu.", ch, NULL, NULL, TO_ROOM );
    char_from_room( ch );
    ...
    act( AT_ACTION, "$n pojawia sie obok ciebie.", ch, NULL, NULL, TO_ROOM );
    do_look( ch, "auto" );
...
void do_aid( CHAR_DATA *ch, char *argument )
{
    ...
    if ( victim == ch )
    {
	send_to_char( "Chcesz leczyc siebie?\n\r", ch );
	return;
    }

    if ( victim->position > POS_STUNNED )
    {
	act( AT_PLAIN, "$N nie potrzebuje Twej pomocy.", ch, NULL, victim,
	     TO_CHAR);
	return;
    }
    ...
    if ( victim->hit <= -6 )
    {
	act( AT_PLAIN, "$N jest w takim stanie, ze nie mozesz juz nic pomoc.", ch,
	     NULL, victim, TO_CHAR);
	return;
    }
    ...
    if (ch->sex == 0)
    {
    act( AT_SKILL, "Wyleczyles $B!",  ch, NULL, victim, TO_CHAR    );
    act( AT_SKILL, "$n wyleczylo $B!",  ch, NULL, victim, TO_NOTVICT );
    }
    else if (ch->sex == 1)
    {
    act( AT_SKILL, "Wyleczyles $B!",  ch, NULL, victim, TO_CHAR    );
    act( AT_SKILL, "$n wyleczyl $B!",  ch, NULL, victim, TO_NOTVICT );
    }
    else
    {
    act( AT_SKILL, "Wyleczylas $B!",  ch, NULL, victim, TO_CHAR    );
    act( AT_SKILL, "$n wyleczyla $B!",  ch, NULL, victim, TO_NOTVICT );
    }
    ...
    update_pos( victim );
    if (ch->sex == 0)
    act( AT_SKILL, "$n wyleczylo cie!", ch, NULL, victim, TO_VICT    );
    else if (ch->sex == 1)
    act( AT_SKILL, "$n wyleczyl cie!", ch, NULL, victim, TO_VICT    );
    else
    act( AT_SKILL, "$n wyleczyla cie!", ch, NULL, victim, TO_VICT    );
    return;
}
...
bool check_parry( CHAR_DATA *ch, CHAR_DATA *victim )
{
    ...
    act( AT_SKILL, "Parujesz atak $c.",  ch, NULL, victim, TO_VICT    );

    if ( !IS_NPC(ch)
         && !IS_SET( ch->pcdata->flags, PCFLAG_GAG) )  /* SB */   
    act( AT_SKILL, "$N paruje Twoj atak.", ch, NULL, victim, TO_CHAR    );
...
bool check_dodge( CHAR_DATA *ch, CHAR_DATA *victim )
{
    ...
    act( AT_SKILL, "Unikasz ataku $c.", ch, NULL, victim, TO_VICT    );
    
    if ( !IS_NPC(ch) && !IS_SET( ch->pcdata->flags, PCFLAG_GAG) ) 
    act( AT_SKILL, "$N unika Twojego ataku.", ch, NULL, victim, TO_CHAR    );
...
void do_scribe( CHAR_DATA *ch, char *argument )
{
    ...
    {
	send_to_char( "Ta umiejetnosc wymaga wiecej magicznych zdolnosci niz Ty posiadasz.\n\r", ch );
	return;
    }

    if ( argument[0] == '\0' || !str_cmp(argument, "") )
    {
	send_to_char( "Co chcesz zapisac?\n\r", ch );
	return;
    }
    ...
    {
         send_to_char( "Nie znasz jeszcze tego czaru.\n\r", ch );
         return;
    }
    ...
    {
        send_to_char( "Nie mozesz zapisac tego czaru.\n\r", ch );
        return;
    }
    ...
    if ( !IS_NPC(ch) && ch->mana < mana )
    {
     ...
     {
	send_to_char( "Musisz trzymac w dloniach czysty pergamin.\n\r", ch );
	return;
     }
	...
	return;
     ...
     {
	send_to_char( "Ten pergamin jest juz zapisany.\n\r", ch);
	return;
     }
	...
	return;
     ...
     sprintf(buf1, "pergamin z czarem %s", skill_table[sn]->name);
     STRFREE(scroll->short_descr);
     scroll->short_descr = STRALLOC( buf1 );
     sprintf(buf1, "pergaminu z czarem %s", skill_table[sn]->name);
     STRFREE(scroll->short_descr_dop);
     scroll->short_descr_dop = STRALLOC( buf1 );
     sprintf(buf1, "pergaminowi z czarem %s", skill_table[sn]->name);
     STRFREE(scroll->short_descr_cel);
     scroll->short_descr_cel = STRALLOC( buf1 );
     sprintf(buf1, "pergamin z czarem %s", skill_table[sn]->name);
     STRFREE(scroll->short_descr_bie);
     scroll->short_descr_bie = STRALLOC( buf1 );
     sprintf(buf1, "pergaminem z czarem %s", skill_table[sn]->name);
     STRFREE(scroll->short_descr_nar);
     scroll->short_descr_nar = STRALLOC( buf1 );
     sprintf(buf1, "pergaminem z czarem %s", skill_table[sn]->name);
     STRFREE(scroll->short_descr_mie);
     scroll->short_descr_mie = STRALLOC( buf1 );
     sprintf(buf1, "pergaminie z czarem %s", skill_table[sn]->name);
     STRFREE(scroll->short_descr_wol);
     scroll->short_descr_wol = STRALLOC( buf1 );

     sprintf(buf2, "Pergamin z czarem '%s' lezy w pyle.", 
                                              skill_table[sn]->name);

     STRFREE(scroll->description);
     scroll->description = STRALLOC(buf2);

     sprintf(buf3, "pergamin %s", skill_table[sn]->name);
     STRFREE(scroll->name);
     scroll->name = STRALLOC(buf3);

     act( AT_MAGIC, "$n zapisuje $p.",   ch,scroll, NULL, TO_ROOM );
     act( AT_MAGIC, "Zapisujesz $p.",   ch,scroll, NULL, TO_CHAR );
...
void do_brew( CHAR_DATA *ch, char *argument )
{
    ...
    {
        send_to_char( "Ten czar wymaga wiekszych zdolnosci magicznych niz Ty posiadasz.\n\r", ch );
	return;
    }

    if ( argument[0] == '\0' || !str_cmp(argument, "") )
    {
	send_to_char( "Jaki eliksir chcesz stworzyc?\n\r", ch );
	return;
    }
    ...
    {
        send_to_char( "Nie mozesz stworzyc eliksiru z tym czarem.\n\r", ch );
        return;
    }
	...
	{
	    send_to_char( "Brakuje Ci na to magicznych mocy.\n\r", ch );
	    return;
	}  
    found = FALSE;
          ...
          fire = fire->next_content )
    {
     ...
     {
        send_to_char(
        "Musisz znajdowac sie przy ognisku by stworzyc eliksir.\n\r", ch );
        return;
     }
     ...
     {
        send_to_char(
        "Musisz trzymac w dloniach pusta flaszke.\n\r", ch );
        return;
     }

     if( potion->pIndexData->vnum != OBJ_VNUM_FLASK_BREWING )
     {
	send_to_char( "Musisz trzymac w dloniach pusta flaszke.\n\r", ch );
	return;
     }
     ...
     {
	send_to_char( "To nie jest pusta flaszka.\n\r", ch);
	return;
     }
	...
	return;
     ...
     sprintf(buf1, "eliksir z czarem %s", skill_table[sn]->name);
     STRFREE(potion->short_descr);
     potion->short_descr = STRALLOC( buf1 );
     sprintf(buf1, "eliksiru z czarem %s", skill_table[sn]->name);
     STRFREE(potion->short_descr_dop);
     potion->short_descr_dop = STRALLOC( buf1 );
     sprintf(buf1, "eliksirowi z czarem %s", skill_table[sn]->name);
     STRFREE(potion->short_descr_cel);
     potion->short_descr_cel = STRALLOC( buf1 );
     sprintf(buf1, "eliksir z czarem %s", skill_table[sn]->name);
     STRFREE(potion->short_descr_bie);
     potion->short_descr_bie = STRALLOC( buf1 );
     sprintf(buf1, "eliksirem z czarem %s", skill_table[sn]->name);
     STRFREE(potion->short_descr_nar);
     potion->short_descr_nar = STRALLOC( buf1 );
     sprintf(buf1, "eliksirze z czarem %s", skill_table[sn]->name);
     STRFREE(potion->short_descr_mie);
     potion->short_descr_mie = STRALLOC( buf1 );
     sprintf(buf1, "eliksirze z czarem %s", skill_table[sn]->name);
     STRFREE(potion->short_descr_wol);
     potion->short_descr_wol = STRALLOC( buf1 );

     sprintf(buf2, "Dziwny eliksir nazwany '%s' migocze w szklanej flaszce.",
                                              skill_table[sn]->name);

     STRFREE(potion->description);
     potion->description = STRALLOC(buf2);

     sprintf(buf3, "flaszka eliksir %s", skill_table[sn]->name);
     STRFREE(potion->name);
     potion->name = STRALLOC(buf3);

     act( AT_MAGIC, "$n tworzy $p.",   ch,potion, NULL, TO_ROOM );
     act( AT_MAGIC, "Tworzysz $p.",   ch,potion, NULL, TO_CHAR );
...
void do_slice( CHAR_DATA *ch, char *argument )
{
  ...
  sprintf(buf, "a slice of raw meat from %s", pMobIndex->short_descr);
  STRFREE(slice->short_descr_dop);
  slice->short_descr_dop = STRALLOC(buf);
  sprintf(buf, "a slice of raw meat from %s", pMobIndex->short_descr);
  STRFREE(slice->short_descr_cel);
  slice->short_descr_cel = STRALLOC(buf);
  sprintf(buf, "a slice of raw meat from %s", pMobIndex->short_descr);
  STRFREE(slice->short_descr_bie);
  slice->short_descr_bie = STRALLOC(buf);
  sprintf(buf, "a slice of raw meat from %s", pMobIndex->short_descr);
  STRFREE(slice->short_descr_nar);
  slice->short_descr_nar = STRALLOC(buf);
  sprintf(buf, "a slice of raw meat from %s", pMobIndex->short_descr);
  STRFREE(slice->short_descr_mie);
  slice->short_descr_mie = STRALLOC(buf);
  sprintf(buf, "a slice of raw meat from %s", pMobIndex->short_descr);
  STRFREE(slice->short_descr_wol);
  slice->short_descr_wol = STRALLOC(buf);
...
void do_cook ( CHAR_DATA *ch, char *argument )
{
   ...
   if ( number_percent() > LEARNED(ch, gsn_cook)  )
   {
	...
	sprintf( buf, "a burnt %s", food->pIndexData->name );
	STRFREE(food->short_descr_dop);
	food->short_descr_dop = STRALLOC(buf);
	sprintf( buf, "a burnt %s", food->pIndexData->name );
	STRFREE(food->short_descr_cel);
	food->short_descr_cel = STRALLOC(buf);
	sprintf( buf, "a burnt %s", food->pIndexData->name );
	STRFREE(food->short_descr_bie);
	food->short_descr_bie = STRALLOC(buf);
	sprintf( buf, "a burnt %s", food->pIndexData->name );
	STRFREE(food->short_descr_nar);
	food->short_descr_nar = STRALLOC(buf);
	sprintf( buf, "a burnt %s", food->pIndexData->name );
	STRFREE(food->short_descr_mie);
	food->short_descr_mie = STRALLOC(buf);
	sprintf( buf, "a burnt %s", food->pIndexData->name );
	STRFREE(food->short_descr_wol);
	food->short_descr_wol = STRALLOC(buf);
	sprintf( buf, "A burnt %s.", food->pIndexData->name);
