...
void write_deity_list( )
{
     ...
     if ( !fpout )
	bug( "FATALNIE: nie moge zapisywac do pliku deity.lst!\n\r", 0 );
     else
     {
...
void save_deity( DEITY_DATA *deity )
{
	...
	return;
     ...
     {
	sprintf( buf, "save_deity: %s nie ma swojego pliku", deity->name );
	bug( buf, 0 );
	return;
     }
...
void fread_deity( DEITY_DATA *deity, FILE *fp )
{
	...
	case 'G':
	    KEY( "Guild",	deity->guild,		fread_string( fp ) );
...
bool load_deity_file( char *deityfile )
{
	...
	for ( ; ; )
	{
	    ...
	    if ( letter != '#' )
	    {
		bug( "Load_deity_file: # nie odnaleziony.", 0 );
		break;
	    }
...
void do_setdeity( CHAR_DATA *ch, char *argument )
{
    ...
    if ( arg1[0] == '\0' )
    {
	send_to_char( "Uzycie: setdeity <bostwo> <pole> <toggle>\n\r", ch );
	send_to_char( "\n\rPola moga byc nastepujace:\n\r", ch );
	...
	send_to_char( "\n\rPunkty przychylnosci potrzebne do prosb:\n\r", ch );
	send_to_char( "scorpse savatar sdeityobj srecall\n\r\n\r", ch );
	send_to_char( "Statystyki przedmiotu - sa nastepujace:\n\r", ch );
    ...
    if ( !deity )
    {
 	send_to_char( "Nie ma takiego Bostwa.\n\r", ch );
	return;
    }
...
/* Crevan obsluga bostw klanowych */

    if ( !str_cmp( arg2, "guild" ) )
    {
	STRFREE( deity->guild );
	deity->guild = STRALLOC( argument );
	send_to_char( "Zrobione.\n\r", ch );
	save_deity( deity );
	return;
    }
...
/* Crevan koniec */
    ...
    if ( !str_cmp( arg2, "suscept" ) )
    {
      ...
      while ( argument[0] != '\0' )
        ...
        if ( fMatch ) ch_printf( ch, "Zrobione.\n\r" );
        save_deity( deity );
        return;
    }
...
void do_showdeity( CHAR_DATA *ch, char *argument )
{
    ...
    if ( argument[0] == '\0' )
    {
	send_to_char( "Uzycie: showdeity <bostwo>\n\r", ch );
	return;
    }
    ...
    ch_printf( ch, "Deity: %s\n\rFilename: %s\tGuild: %s\n\rDescription:\n\r%s\n\r", deity->name, deity->filename, deity->guild, deity->description );   
...
void do_makedeity( CHAR_DATA *ch, char *argument )
{
    ...
    {
	send_to_char( "Uzycie: makedeity <imie Bostwa>\n\r", ch );
	return;
    }
...
void do_devote( CHAR_DATA *ch, char *argument )
{
    ...
    if ( ch->level < 5 )
    {
	send_to_char( "Nie mozesz jeszcze wyznawac zadnego Bostwa.\n\r", ch );
	return;
    }
    ...
    if ( arg[0] == '\0' )
    {
	send_to_char( "Jakie Bostwo chcesz wyznawac?\n\r", ch );
	return;
    }
    ...
    if ( !str_cmp( arg, "none" ) )
    {
        ...
        if ( !ch->pcdata->deity )
        {
           send_to_char( "Juz nie wyznajesz zadnego Bostwa.\n\r", ch );
           return;
        }
        --ch->pcdata->deity->worshippers;
        ch->pcdata->favor = -2500;
	ch->mental_state = -80;
        send_to_char( "Dosiega Cie klatwa porzuconego przez Ciebie Bostwa!\n\r", ch );
	...
	affect_strip( ch, gsn_blindness );
        ...
        send_to_char( "Przestajesz wyznawac jakiekolwiek Bostwo.\n\r", ch );
    ...
    if ( !deity )
    {
	send_to_char( "Takie Bostwo nie istnieje w tym swiecie.\n\r", ch );
	return;
    }

    if ( ch->pcdata->deity )
    {
	send_to_char( "Niestety, wyznajesz juz jakies Bostwo.\n\r", ch );
	return;
    }

/* Crevan obsluga bostw klanowych */

    if ( !ch->pcdata->clan && ( str_cmp( deity->guild, "" ) ) )
    {
	send_to_char( "Nie mozesz wyznawac bostwa klanowego nie nalezac do klanu.\n\r", ch );
	return; 
    }

    if ( ch->pcdata->clan && ( str_cmp( deity->guild, ch->pcdata->clan->name ) ) )
    {
	send_to_char( "Nie mozesz wyznawac bostwa innego klanu niz Twoj.\n\r", ch );
	return;
    }
...
/* Crevan koniec */
    ...
    {
	send_to_char( "To Bostwo nie akceptuje Cie z powodu Twej profesji.\n\r", ch );
	return;
    }
    ...
    && ( deity->sex != ch->sex ) )
    {
	send_to_char( "To Bostwo nie akceptuje Cie z powodu Twej plci.\n\r", ch );
	return;
    }
    ...
    {
	send_to_char( "To Bostwo nie akceptuje Cie z powodu Twej rasy.\n\r", ch );
	return;
    }
    }
/*
	...
	--ch->pcdata->deity->worshippers;
	ch->pcdata->favor = -2500;
	send_to_char( "Dosiega Cie potezna klatwa, gdy porzucasz swe Bostwo!\n\r", ch );
	xREMOVE_BITs( ch->affected_by, ch->pcdata->deity->affected );
	REMOVE_BIT( ch->resistant, ch->pcdata->deity->element );
...
*/
    ...
    act( AT_MAGIC, "$t przenika w Twe cialo i w Twa dusze!", ch, ch->pcdata->deity_name, NULL, TO_CHAR );
    ++ch->pcdata->deity->worshippers;
    save_deity(ch->pcdata->deity);
    save_char_obj( ch );
    return;
...
void do_deities( CHAR_DATA *ch, char *argument )
{
    ...
    if ( argument[0] == '\0' )
    {
	send_to_pager_color( "&gDla dokladniejszych informacji wpisz 'deities <bostwo>' oraz 'help deities'\n\r", ch );
	send_to_pager_color( "Bostwo    		Wyznawcy\n\r", ch );
	for ( deity = first_deity; deity; deity = deity->next )
	{
	    if ( deity->guild == '\0' )
		deity->guild = STRALLOC( "" );
	    pager_printf_color( ch, "&G%-14s	&g%19d   %s\n\r", 
	    deity->name, deity->worshippers, 
	    ( str_cmp( deity->guild, "" ) ) ? "(bostwo klanowe)" : "" );
	    count++;
	}
	if ( !count )
	{
	    send_to_pager_color( "&gZadne Bostwo nie sprawuje teraz opieki nad tym swiatem.\n\r", ch );
	    return;
	}
        return;
     }
    ...
    if ( !deity )
    {
	send_to_pager_color( "&gNie ma takiego Bostwa.\n\r", ch );
	return;
    }

    pager_printf_color( ch, "&gBostwo:        &G%s\n\r", deity->name );
    pager_printf_color( ch, "&gOpis:\n\r&G%s", deity->description );
    return;
}
...
void do_supplicate( CHAR_DATA *ch, char *argument )
{
    ...
    {
	send_to_char( "Nie masz Bostwa, ktore moglbys o cos prosic.\n\r", ch );
	return;
    }
    ...
    if ( arg[0] == '\0' )
    {
	send_to_char( "O co chcesz prosic swe Bostwo?\n\r", ch );
	return;
    }
	...
	{
	   send_to_char( "Twe Bostwo nie jest na tyle Tobie przychylne by, przyniesc Ci cialo.\n\r", ch );
	   return;
	}

	if ( IS_SET( ch->in_room->room_flags, ROOM_CLANSTOREROOM ) )
	{
	   send_to_char( "Nie mozesz o to prosi bedac w magazynie klanu.\n\r", ch );
	   return;
	}

	found = FALSE;
	sprintf(buf3, " ");
	sprintf(buf2,"cialo %s", ch->ncel);
	for ( obj = first_object; obj; obj = obj->next )
	{
	   ...
	   && (obj->pIndexData->vnum == 11 ) )
	   {
		...
		{
		  act( AT_MAGIC, "Twe cialo pojawia sie, by zaraz rozplynac sie w powietrzu.", ch, NULL, NULL, TO_CHAR );
		  return;
		}
		act( AT_MAGIC, "Twe cialo pojawia sie nagle, otoczone Boska aura...", ch, NULL, NULL, TO_CHAR );		
		act( AT_MAGIC, "Cialo $c pojawia sie nagle, otoczone Boskimi mocami...", ch, NULL, NULL, TO_ROOM );
		obj_from_room(obj);
	...
	if ( !found )
	{
	   send_to_char( "Nie ma zadnego Twego ciala na tym swiecie...\n\r", ch );
	   return;
	}
   ...
   if ( !str_cmp( arg, "avatar" ) )
   {
	...
	CHAR_DATA *victim;
        ...
        {
           send_to_char( "Twe Bostwo nie jest na tyle Ci przychylne by Ci pomoc.\n\r", ch );
           return;
        }
	...
	victim->long_descr = STRALLOC( buf );
        ...
        act( AT_MAGIC, "$n przywoluje poteznego Awatara!", ch, NULL, NULL, TO_ROOM );
        act( AT_MAGIC, "Przywolujesz poteznego Awatara!", ch, NULL, NULL, TO_CHAR );
   ...
   if ( !str_cmp( arg, "object" ) )
   {
	...
	{
	   send_to_char( "Twe Bostwo nie jest Ci na tyle przychylne by Cie obdarowac.\n\r", ch );
	   return;
	}
	...
	sprintf( buf, "medalion srebrny %s", ch->pcdata->deity->name );
        STRFREE( obj->name );
	...
	act( AT_MAGIC, "$n przywoluje z Boskiego Wymiaru $p!", ch, obj, NULL, TO_ROOM );
	act( AT_MAGIC, "Przywolujesz z Boskiego Wymiaru $p!", ch, obj, NULL, TO_CHAR );
   ...
   if ( !str_cmp( arg, "recall" ) )
   {
	...
	{
	   send_to_char( "Musisz uczynic wiecej dla Twego Bostwa, zanim ono uczyni cos dla Ciebie.\n\r", ch );
 	   return;
	}

	if ( IS_SET( ch->in_room->room_flags, ROOM_NOSUPPLICATE ) )
	{
	   send_to_char( "Bogowie o tobie zapomnieli!\n\r", ch );
	   return;
	}

	if ( get_timer(ch, TIMER_RECENTFIGHT) > 0 && !IS_IMMORTAL(ch) )
        {
          send_to_char( "Nie mozesz prosic o przeniesienie, majac w ciele tyle adrenaliny!\n\r", ch );
          return;
        }
	...
	if ( !location )
    	...
    	if ( !location )
    	{
          send_to_char( "Tracisz orientacje w terenie.\n\r", ch );
          return;
	}

    	act( AT_MAGIC, "$n znika w magicznym pyle.", ch, NULL, NULL, TO_ROOM );
    	char_from_room( ch );
    	...
    	act( AT_MAGIC, "$n pojawia sie wsrod magicznego pyly.", ch, NULL, NULL, TO_ROOM );
    	do_look( ch, "auto" );
    ...
    send_to_char( "Nie mozesz prosic o cos takiego.\n\r", ch );
    return;
}
