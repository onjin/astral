...
void do_gold(CHAR_DATA * ch, char *argument)
{
   ...
   ch_printf( ch, "Posiadasz %d sztuk%s zlota.\n\r", ch->gold, ( ch->gold == 1 ) ? "e" : "" );
   return;
}
...
void do_worth(CHAR_DATA *ch, char *argument)
{
    ...
    pager_printf(ch, "|Quest: %-4d |Weight: %-9d |Style: %-13s |Gold: %-14d |\n\r",
                 ch->questpoints, ch->carry_weight, buf, ch->gold );
    send_to_pager(" ----------------------------------------------------------------------------\n\r", ch);
...
/*    pager_printf(ch, "Glory: %4.4d(%4.4d) \n\r",
    ...
    if (ch->level > 4)	
    {
        pager_printf(ch, "Pkt. questowe: %4.4d", ch->questpoints);
	...
	pager_printf(ch, "Bamfin: %s\n\r", (ch->pcdata->bamfin[0] != '\0')
		? ch->pcdata->bamfin : "Bog pojawia sie przed Toba.");
	pager_printf(ch, "Bamfout: %s\n\r", (ch->pcdata->bamfout[0] != '\0')
		? ch->pcdata->bamfout : "Bog wraca do swej siedziby.");
...
void do_newscore(CHAR_DATA * ch, char *argument)
{
    ...
    if (ch->level > 4)	
    {
        pager_printf(ch, "Pkt. questowe: %4.4d", ch->questpoints);
	if (ch->level >14 && ch->level < 50)	
	    pager_printf(ch, " ( w tym do poziomu %4.4d)\n\r)", (( ch->level +1 ) * 6) - ch->questlevel );
	else 
	    pager_printf( ch, "\n\r" );
    }
...
void do_inventory( CHAR_DATA *ch, char *argument )
{
    ...
    send_to_char( "Nosisz ze soba:\n\r", ch );
    show_list_to_char2( ch->first_carrying, ch, TRUE, TRUE );
    return;
}
...
void do_title( CHAR_DATA *ch, char *argument )
{
    ...
    if ( ch->level < 5 )
    {
	send_to_char( "Niestety... musisz posiadac conajmniej 5 poziom by moc ustawiac swoj tytul...\n\r", ch );
	return;
    }
/*    if ( IS_SET( ch->pcdata->flags, PCFLAG_NOTITLE ))
    ...
    if ( argument[0] == '\0' )
    {
	send_to_char( "Na co chcesz zmienic swoj tytul?\n\r", ch );
	return;
    }
...
{
     send_to_char("Jesli chcesz uzyc DELETE, wpisz to cale.\n\r", ch);
     return;
}
...
void do_delete( CHAR_DATA *ch, char *argument)
{
    ...
    if( IS_NPC(ch) )
    {
      send_to_char ( "Tak, zapewne...\n\r", ch );
      return;
    }

    if( ch->fighting != NULL )
    {
    send_to_char( "Zaczekaj az skonczysz walczyc.\n\r", ch );
    return;
    }

    /* Reimbursement warning added to code by Samson 1-18-98 */
    set_char_color( AT_YELLOW, ch );
    send_to_char( "Pamietaj, ze ta decyzja jest NIEODWRACALNA. Nie bedzie juz powrotu!\n\r", ch );

    /* Immortals warning added to code by Samson 1-18-98 */
    if ( IS_IMMORTAL(ch) )
    {
	ch_printf( ch, "Badz ostrozny %s, jesli usuniesz Swa postac\n\rNie bedziesz juz Niesmiertelnym!\n\r", ch->nwol );
	send_to_char( "Wszelkie dane o Twej kraince takze zostana utracone.\n\r", ch );
    }
    set_char_color( AT_RED, ch );
    send_to_char( "\n\rWpisz swoje haslo, jesli chcesz skasowac Swa postac.\n\r", ch );
    send_to_char( "[DELETE] Haslo: ", ch );
...
void tax_player( CHAR_DATA *ch )
{
  ...
  int gold = ( ch->gold + ch->pcdata->balance );
  int mgold = (ch->level * 4000000);
  int tax = ( ( ( gold - mgold ) * 0.3 ) + 1 );
  ...
  {
    set_char_color( AT_WHITE, ch );
    ch_printf( ch, "\n\rMasz %d poziom i posiadasz wiecej niz %d sztuk zlota.\n\r",
	ch->level, mgold );
    ch_printf( ch, "Nadwyzka (%d zlota) zostaje oblozona podatkiem w wysokosci 30\% (%d zlota).\n\r",
	gold - mgold, tax );
    ch_printf( ch, "Zostaje Ci wiec %d sztuk zlota.\n\r",
	(gold - tax));
    if ( tax > ch->gold )
    {
	tax -= ch->gold;
	ch->pcdata->balance -= tax;
	ch->gold = 0;
    }
    else
        ch->gold -= tax;
