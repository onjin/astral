...
void save_char_obj( CHAR_DATA *ch )
{
    ...
    {
        sprintf( strback,"../backup/player/%c/%s",tolower(ch->pcdata->filename[0]),
                                 capitalize( ch->pcdata->filename ) );
		 ...
		 sh_int os_type )
{
    ...
    if ( QUICKMATCH( obj->short_descr_dop, obj->pIndexData->short_descr_dop ) == 0 )
	fprintf( fp, "ShortDescrD  %s~\n",	obj->short_descr_dop     );
    if ( QUICKMATCH( obj->short_descr_cel, obj->pIndexData->short_descr_cel ) == 0 )
	fprintf( fp, "ShortDescrC  %s~\n",	obj->short_descr_cel     );
    if ( QUICKMATCH( obj->short_descr_bie, obj->pIndexData->short_descr_bie ) == 0 )
	fprintf( fp, "ShortDescrB  %s~\n",	obj->short_descr_bie     );
    if ( QUICKMATCH( obj->short_descr_nar, obj->pIndexData->short_descr_nar ) == 0 )
	fprintf( fp, "ShortDescrN  %s~\n",	obj->short_descr_nar     );
    if ( QUICKMATCH( obj->short_descr_mie, obj->pIndexData->short_descr_mie ) == 0 )
	fprintf( fp, "ShortDescrM  %s~\n",	obj->short_descr_mie     );
    if ( QUICKMATCH( obj->short_descr_wol, obj->pIndexData->short_descr_wol ) == 0 )
	fprintf( fp, "ShortDescrW  %s~\n",	obj->short_descr_wol     );
...
bool load_char_obj( DESCRIPTOR_DATA *d, char *name, bool preload )
{
    ...
    ch->ncel			= STRALLOC( name );
    ch->ndop			= STRALLOC( name );
    ch->nbie			= STRALLOC( name );
    ch->nnar			= STRALLOC( name );
    ch->nmie			= STRALLOC( name );
    ch->nwol			= STRALLOC( name );
	...
	ch->short_descr_dop		= STRALLOC( "" );
	ch->short_descr_cel		= STRALLOC( "" );
	ch->short_descr_bie		= STRALLOC( "" );
	ch->short_descr_nar		= STRALLOC( "" );
	ch->short_descr_mie		= STRALLOC( "" );
	ch->short_descr_wol		= STRALLOC( "" );
	ch->channels			= STRALLOC( "" );
	ch->channels_off		= STRALLOC( "" );
...
/* Flash alias koniec */ 	    	
	...
	case 'C':
	    KEY( "Channels",	ch->channels, fread_string( fp ) );
	    KEY( "Channels_off", ch->channels_off, fread_string( fp ) );
	    if ( !strcmp( word, "Clan" ) )
	    {
	...
	case 'N':
	    ...
	    KEY ("Ncel", ch->ncel, fread_string( fp ) );
	    KEY ("Ndop", ch->ndop, fread_string( fp ) );
	    KEY ("Nbie", ch->nbie, fread_string( fp ) );
	    KEY ("Nnar", ch->nnar, fread_string( fp ) );
	    KEY ("Nmie", ch->nmie, fread_string( fp ) );
	    KEY ("Nwol", ch->nwol, fread_string( fp ) );
	...
	case 'P':
        ...
        case 'Q':
            ...
            KEY( "QuestLevel",  ch->questlevel,         fread_number( fp ) );
            break;
	...
	case 'S':
	    ...
	    KEY( "ShortDescrD",	ch->short_descr_dop,	fread_string( fp ) );
	    KEY( "ShortDescrC",	ch->short_descr_cel,	fread_string( fp ) );
	    KEY( "ShortDescrB",	ch->short_descr_bie,	fread_string( fp ) );
	    KEY( "ShortDescrN",	ch->short_descr_nar,	fread_string( fp ) );
	    KEY( "ShortDescrM",	ch->short_descr_mie,	fread_string( fp ) );
	    KEY( "ShortDescrW",	ch->short_descr_wol,	fread_string( fp ) );
	...
	case 'E':
		...
		if (!ch->short_descr)
		  ch->short_descr	= STRALLOC( "" );
		if (!ch->short_descr_dop)
		  ch->short_descr_dop	= STRALLOC( "" );
		if (!ch->short_descr_cel)
		  ch->short_descr_cel	= STRALLOC( "" );
		if (!ch->short_descr_bie)
		  ch->short_descr_bie	= STRALLOC( "" );
		if (!ch->short_descr_nar)
		  ch->short_descr_nar	= STRALLOC( "" );
		if (!ch->short_descr_mie)
		  ch->short_descr_mie	= STRALLOC( "" );
		if (!ch->short_descr_wol)
		  ch->short_descr_wol	= STRALLOC( "" );
		if (!ch->long_descr)
		  ch->long_descr	= STRALLOC( "" );
		if (!ch->description)
...
void fread_obj( CHAR_DATA *ch, FILE *fp, sh_int os_type )
{
	...
	case 'E':
		...
		else
		{
		    ...
		    if ( !obj->short_descr_dop )
			obj->short_descr_dop = QUICKLINK( obj->pIndexData->short_descr_dop );
		    if ( !obj->short_descr_cel )
			obj->short_descr_cel = QUICKLINK( obj->pIndexData->short_descr_cel );
		    if ( !obj->short_descr_bie )
			obj->short_descr_bie = QUICKLINK( obj->pIndexData->short_descr_bie );
		    if ( !obj->short_descr_nar )
			obj->short_descr_nar = QUICKLINK( obj->pIndexData->short_descr_nar );
		    if ( !obj->short_descr_mie )
			obj->short_descr_mie = QUICKLINK( obj->pIndexData->short_descr_mie );
		    if ( !obj->short_descr_wol )
			obj->short_descr_wol = QUICKLINK( obj->pIndexData->short_descr_wol );
		    if ( !obj->action_desc )
	...
	case 'S':
	    ...
	    KEY( "ShortDescrD",	obj->short_descr_dop,	fread_string( fp ) );
	    KEY( "ShortDescrC",	obj->short_descr_cel,	fread_string( fp ) );
	    KEY( "ShortDescrB",	obj->short_descr_bie,	fread_string( fp ) );
	    KEY( "ShortDescrN",	obj->short_descr_nar,	fread_string( fp ) );
	    KEY( "ShortDescrM",	obj->short_descr_mie,	fread_string( fp ) );
	    KEY( "ShortDescrW",	obj->short_descr_wol,	fread_string( fp ) );
...
void write_corpses( CHAR_DATA *ch, char *name, OBJ_DATA *objrem )
{
  ...
  for ( corpse = first_object; corpse; corpse = corpse->next )
    if ( corpse->pIndexData->vnum == OBJ_VNUM_CORPSE_PC &&
         corpse->in_room != NULL &&
        !str_cmp(corpse->short_descr+6, name) &&
	objrem != corpse )
    {
      if ( !fp )
      {
...
void fwrite_mobile( FILE *fp, CHAR_DATA *mob )
{
  ...
  if ( QUICKMATCH( mob->short_descr, mob->pIndexData->short_descr) == 0 )
  	fprintf( fp, "Short	%s~\n", mob->short_descr );
  if ( QUICKMATCH( mob->short_descr_dop, mob->pIndexData->short_descr_dop) == 0 )
  	fprintf( fp, "ShortD	%s~\n", mob->short_descr_dop );
  if ( QUICKMATCH( mob->short_descr_cel, mob->pIndexData->short_descr_cel) == 0 )
  	fprintf( fp, "ShortC	%s~\n", mob->short_descr_cel );
  if ( QUICKMATCH( mob->short_descr_bie, mob->pIndexData->short_descr_bie) == 0 )
  	fprintf( fp, "ShortB	%s~\n", mob->short_descr_bie );
  if ( QUICKMATCH( mob->short_descr_nar, mob->pIndexData->short_descr_nar) == 0 )
  	fprintf( fp, "ShortN	%s~\n", mob->short_descr_nar );
  if ( QUICKMATCH( mob->short_descr_mie, mob->pIndexData->short_descr_mie) == 0 )
  	fprintf( fp, "ShortM	%s~\n", mob->short_descr_mie );
  if ( QUICKMATCH( mob->short_descr_wol, mob->pIndexData->short_descr_wol) == 0 )
  	fprintf( fp, "ShortW	%s~\n", mob->short_descr_wol );
...
/*		KEY( "ShortD", mob->short_descr_dop, fread_string(fp));
		KEY( "ShortC", mob->short_descr_cel, fread_string(fp));
		KEY( "ShortB", mob->short_descr_bie, fread_string(fp));
		KEY( "ShortN", mob->short_descr_nar, fread_string(fp));
		KEY( "ShortM", mob->short_descr_mie, fread_string(fp));
		KEY( "ShortW", mob->short_descr_wol, fread_string(fp)); */
		break;
	}
	if ( !fMatch )
	{
