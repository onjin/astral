...
void do_undeny( CHAR_DATA *ch, char *argument )
{
    ...
    xREMOVE_BIT(victim->act, PLR_DENY);
    send_to_char( "OK.\n\r", ch );
    do_quit( victim, "" );
    return;
}
...
/* void do_authorize( CHAR_DATA *ch, char *argument )
{
  ...
  if ( arg1[0] == '\0' )
     {
     send_to_char( "Uzycie:  authorize <gracz> <yes|name|no/deny>\n\r", ch );
     send_to_char( "Czeka na autoryzacje:\n\r", ch );
     send_to_char( " Wybierz imie postaci\n\r", ch );
     send_to_char( "---------------------------------------------\n\r", ch );
     for ( d = first_descriptor; d; d = d->next )
  ...
  if ( arg2[0]=='\0' || !str_cmp( arg2,"accept" ) || !str_cmp( arg2,"yes" ))
	...
	ch_printf( ch, "Zautoryzowales %s.\n\r", victim->nbie);
    
        "\n\r&GAdministratorzy MUD`a zaakceptowali Twe imie %s.\n\r"
        "Mozesz rozpoczac przygode z Astral Dominion po przejsciu "
        "tej krainy.\n\r", victim->nwol);
     return;
     }
     else if ( !str_cmp( arg2, "no" ) || !str_cmp( arg2, "deny" ) )
     {
	send_to_char_color( "&RNiestety nie dana Ci byla autoryzacja.\n\r", victim);
	sprintf( buf, "%s: denied authorization", victim->name);
	to_channel( buf, CHANNEL_AUTH, "Auth", LEVEL_NEOPHYTE );
	ch_printf( ch, "Odmowiles autoryzacji %s.\n\r", victim->ndop);
	do_quit(victim, "");
     }
	...
	to_channel( buf, CHANNEL_AUTH, "Auth", LEVEL_NEOPHYTE );
        ch_printf_color(victim,
          "&R\n\rAdministratorzy MUD`a uznali imie %s "
          "za nieakceptowalne.\n\r"
          "Mozesz zmienic imie po osiagnieciu konca "
          "tej krany.\n\r"
          "Twoje imie powinno byc oryginalne.\n\r"
          "Nie powinno byc podobne do imion postaci juz grajacych "
          "lub obrazliwe.\n\r", victim->name);
	ch_printf( ch, "Nakazeles %s zmiane imienia.\n\r", victim->ndop);
	return;
     }

     else
     {
	send_to_char("Nieprawidlowy parametr.\n\r", ch);
	return;
     }
}
Flash */
void do_bamfin( CHAR_DATA *ch, char *argument )
{
	...
	send_to_char_color( "&YBamfin ustawiony.\n\r", ch );
    }
    return;
}
...
void do_bamfout( CHAR_DATA *ch, char *argument )
{
	...
	send_to_char_color( "&YBamfout ustawiony.\n\r", ch );
    }
    return;
}
...
void do_echo( CHAR_DATA *ch, char *argument )
{
    ...
    if ( !color )
      color = AT_IMMORT;
    one_argument(argument, arg);
    if ( !str_cmp( arg, "Flash" )
    ||   !str_cmp( arg, "Crevan" )
    ||   !str_cmp( arg, "Sardo" )
    ||   !str_cmp( arg, "Jaskierka" )
    ||   !str_cmp( arg, "Isorn" )
    ||   !str_cmp( arg, "Bercik" )) {
...
Flash */
    ...
    {
	send_to_char( "Niestety misiaczku, to nie przejdzie...\n\r", ch );
	return;
    }
    if ( victim->fighting )
	stop_fighting( victim, TRUE );
    act( AT_MAGIC, "$n znika w chmurze migocacego kurzu.", victim, NULL, NULL, TO_ROOM );
    victim->retran = victim->in_room->vnum;
    char_from_room( victim );
    char_to_room( victim, location );
    act( AT_MAGIC, "$n pojawia sie w klebach dymu.", victim, NULL, NULL, TO_ROOM );
    if ( ch != victim )
      act( AT_IMMORT, "$n przetransferowal cie.", ch, NULL, victim, TO_VICT );
    do_look( victim, "auto" );
...
void do_oodstat( CHAR_DATA *ch, char *argument )
{
    char arg[MAX_INPUT_LENGTH];
    OBJ_DATA *obj;
    ...
    ch_printf_color( ch, "&cVnum: &w%d  ", obj->pIndexData->vnum );
    ch_printf_color( ch, "&cShort description:  &C%s\n\r", obj->short_descr);
    ch_printf_color( ch, "&cShort descriptionD: &C%s\n\r", obj->short_descr_dop);
    ch_printf_color( ch, "&cShort descriptionC: &C%s\n\r", obj->short_descr_cel);
    ch_printf_color( ch, "&cShort descriptionB: &C%s\n\r", obj->short_descr_bie);
    ch_printf_color( ch, "&cShort descriptionN: &C%s\n\r", obj->short_descr_nar);
    ch_printf_color( ch, "&cShort descriptionM: &C%s\n\r", obj->short_descr_mie);
    ch_printf_color( ch, "&cShort descriptionW: &C%s\n\r", obj->short_descr_wol);
    ch_printf_color( ch, "&cLong description : &C%s\n\r", obj->description);
    return;
}
...
void do_mstat( CHAR_DATA *ch, char *argument )
{
    ...
    if (( IS_VAMPIRE(victim) || IS_DEMON(victim)) && !IS_NPC(victim))  
        {
    ...
    if (victim->level > 4)	
    {
        pager_printf_color(ch, "Pkt. questowe: %4.4d", victim->questpoints);
	if (victim->level >14 && victim->level < 50)	
	    pager_printf(ch, " ( do poziomu %4.4d)\n\r", (( victim->level +1 ) * 6) - victim->questlevel );
	else 
	    pager_printf( ch, "\n\r" );
    }
...
void do_modstat( CHAR_DATA *ch, char *argument ) /*/ flash */
{
    char arg[MAX_INPUT_LENGTH];
    CHAR_DATA *victim;
    ...
    pager_printf_color( ch, "\n\r&c%s: &C%-20s", IS_NPC(victim) ? "Mobile name" : "Name", victim->name );
    pager_printf_color( ch, "&cVnum: &w%-5d    &cSex: &w%-6s    &cRoom: &w%-5d\n\r",
	IS_NPC(victim) ? victim->pIndexData->vnum : 0,
	victim->sex == SEX_MALE    ? "male"   :
	victim->sex == SEX_FEMALE  ? "female" : "neutral",
	victim->in_room == NULL    ?        0 : victim->in_room->vnum );
    ...
    if ( IS_NPC( victim ) )
      pager_printf_color( ch, "\n\r&cShortdesc   : &w%s\n\r&cShortdescD  : &w%s\n\r&cShortdescC  : &w%s\n\r&cShortdescB  : &w%s\n\r&cShortdescN  : &w%s\n\r&cShortdescM  : &w%s\n\r&cShortdescW  : &w%s\n\r&cLongdesc    : &w%s",
	victim->short_descr[0] != '\0' ? victim->short_descr : "(none set)",
	victim->short_descr_dop[0] != '\0' ? victim->short_descr_dop : "(none set)",
	victim->short_descr_cel[0] != '\0' ? victim->short_descr_cel : "(none set)",
	victim->short_descr_bie[0] != '\0' ? victim->short_descr_bie : "(none set)",
	victim->short_descr_nar[0] != '\0' ? victim->short_descr_nar : "(none set)",
	victim->short_descr_mie[0] != '\0' ? victim->short_descr_mie : "(none set)",
	victim->short_descr_wol[0] != '\0' ? victim->short_descr_wol : "(none set)",
	victim->long_descr[0] != '\0' ? victim->long_descr : "(none set)\n\r" );
    else
    {
          pager_printf_color( ch, "&cName  : &w%s\n\r",
	    victim->name );
          pager_printf_color( ch, "&cNameD  : &w%s\n\r",
	    victim->ncel );
          pager_printf_color( ch, "&cNameC  : &w%s\n\r",
	    victim->ndop );
          pager_printf_color( ch, "&cNameB  : &w%s\n\r",
	    victim->nbie );
          pager_printf_color( ch, "&cNameN  : &w%s\n\r",
	    victim->nnar );
          pager_printf_color( ch, "&cNameM  : &w%s\n\r",
	    victim->nmie );
          pager_printf_color( ch, "&cNameW  : &w%s\n\r",
	    victim->nwol );
    }
    return;
}
...
void do_reboot( CHAR_DATA *ch, char *argument )
{
    ...
    if ( auction->item )
	do_auction( ch, "stop");
    sprintf( buf, "Reboot zrobiony przez %s.", ch->nbie );
    do_echo( ch, buf );
...
void do_advance( CHAR_DATA *ch, char *argument )
{
    ...
    if ( arg1[0] == '\0' || arg2[0] == '\0' || !is_number( arg2 ) ) {
        send_to_char( "Uzycie:  advance <postac> <poziom>\n\r", ch );
        return;
    }
    if ( ( victim = get_char_room( ch, arg1 ) ) == NULL ) {
        send_to_char( "Nie ma przy tobie kogos takiego.\n\r", ch);
        return;
    }
    if ( IS_NPC( victim ) ) {
        send_to_char( "Nie mozesz awansowac moba.\n\r", ch );
        return;
    }
  /*You can demote yourself but not someone else at your own trust.-- Narn*/
    ...
    if ( ( level = atoi( arg2 ) ) < 1 || level > MAX_LEVEL ) {
        send_to_char( "Zakres poziomow jest od 1 do 65.\n\r", ch );
        return;
    }
    if ( level > get_trust( ch ) ) {
        send_to_char( "Poziom jest ograniczony do twojego poziomu.\n\r", ch );
        return;
    }
    /* Lower level:
     *   Reset to level 1.
    ...
    if ( level <= victim->level )
    {
        ...
        if ( level < victim->level )
	{
          ch_printf( ch, "Zmniejszasz %s poziom z %d do poziomu %d!\n\r", victim->name, victim->level, level );
	  send_to_char( "Na mlot Thora!  Bogowie zmniejszyli twoj poziom...\n\r", victim );
	}
        else
	{
          ch_printf( ch, "%s juz ma poziom %d.  Re-advancing...\n\r", victim->name, level );
	  send_to_char( "Deja vu! Znow otrzymujesz poziom jaki miales!\n\r", victim );
	}
        victim->level    = 1;
    ...
    {
        ch_printf( ch, "Zwiekszasz %s poziom z %d do poziomu %d!\n\r", victim->name, victim->level, level );
        if (victim->level >= LEVEL_AVATAR)
        {
          set_char_color( AT_IMMORT, victim );
          act( AT_IMMORT, "$n wykonuje dziwne gesty dlonmi i wskazuje swym palcem na Ciebie!",
               ch, NULL, victim, TO_VICT );
          act( AT_IMMORT, "$n wykonuje dziwne gesty dlonmi i wskazuje swym palcem na $N!",
               ch, NULL, victim, TO_NOTVICT );
          set_char_color( AT_WHITE, victim );
          send_to_char( "Czujesz sie bardzo dziwnie...\n\r\n\r", victim );
          set_char_color( AT_LBLUE, victim );
        }
        switch(level)
        {
        default:
          send_to_char( "Bogowie postanowili zwiekszyc twoj poziom!\n\r", victim );
          break;
        case LEVEL_IMMORTAL:
          do_help(victim, "M_GODLVL1_" );
          set_char_color( AT_WHITE, victim );
          send_to_char( "Co za pech... wszystkie twe przedmioty znikaja.\n\r", victim );
          while ( victim->first_carrying )
            extract_obj( victim->first_carrying );
          break;
        case LEVEL_ACOLYTE:
    ...
    for ( iLevel = victim->level ; iLevel < level; iLevel++ )
    {
        if (level < LEVEL_IMMORTAL)
          send_to_char( "Wzrasta twoj poziom!!\n\r", victim );
        victim->level += 1;
        advance_level( victim );
    }
...
void do_immortalize( CHAR_DATA *ch, char *argument )
 {
    ...
    argument = one_argument( argument, arg );
    if ( arg[0] == '\0' ) {
	send_to_char( "Skladnia:  immortalize <gracz>\n\r", ch );
	return;
    }
    if ( ( victim = get_char_room( ch, arg ) ) == NULL ) {
	send_to_char( "Nie ma tu tej osoby.\n\r", ch);
	return;
    }
    if ( IS_NPC( victim ) ) {
	send_to_char( "Nie na NPC.\n\r", ch );
	return;
    }
    if ( victim->level != LEVEL_AVATAR ) {
	send_to_char( "Ta osoba nie jest jeszcze godna uniesmiertelnienia.\n\r", ch );
	return;
    }

    send_to_char( "Uniesmiertelnianie osoby...\n\r", ch );
    set_char_color( AT_IMMORT, victim );
    act( AT_IMMORT, "$n zaczyna cicho szeptac... unosi swoje ramiona do nieba...",
	 ch, NULL, NULL, TO_ROOM );
    set_char_color( AT_WHITE, victim );
    send_to_char( "Poprzez Twoje cialo przebiega wiele dziwnych uczuc...\n\r\n\r", victim );
    set_char_color( AT_LBLUE, victim );
    do_help(victim, "M_GODLVL1_" );
    set_char_color( AT_WHITE, victim );
    send_to_char( "Prostujesz sie... wszystkie Twoje doczesne dobra opadaja.\n\r", victim );
    while ( victim->first_carrying )
	extract_obj( victim->first_carrying );
...
{
    send_to_char("Nie uzywana komenda.\n\r", ch);
/*    char buf[MAX_STRING_LENGTH];
    char arg[MAX_INPUT_LENGTH];
    CHAR_DATA *victim;
...
void do_force( CHAR_DATA *ch, char *argument )
{
    ...
    if ( arg[0] == '\0' || argument[0] == '\0' )
    {
	send_to_char( "Kogo zmusic i do czego?\n\r", ch );
	return;
    }
	...
	for ( vch = first_char; vch; vch = vch_next )
	{
	    ...
	    {
		act( AT_IMMORT, "$n naklania Cie do wykonania '$t'.", ch, argument, vch, TO_VICT );
	...
	if ( victim == ch )
	{
	    send_to_char( "Aye aye, juz robie!\n\r", ch );
	    return;
	}
	...
	{
	    send_to_char( "Zrob to sam jakzes taki Slodowy!\n\r", ch );
	    return;
	}
    ...
    act( AT_IMMORT, "$n naklania Cie do wykonania '$t'.", ch, argument, victim, TO_VICT );
...
void do_invis( CHAR_DATA *ch, char *argument )
{
	...
	if ( !is_number( arg ) )
	{
	   send_to_char( "Uzycie: invis | invis <poziom>\n\r", ch );
	   return;
	}
	level = atoi( arg );
	if ( level < 2 || level > get_trust( ch ) )
	{
	    send_to_char( "Nieprawidlowy poziom.\n\r", ch );
	    return;
	}
	...
	if (!IS_NPC(ch))
	  ...
	  ch_printf( ch, "Wizinvis ustawiony na poziom %d.\n\r", level );
        }
        
        if (IS_NPC(ch))       
        {
          ch->mobinvis = level;
          ch_printf( ch, "Mobinvis ustawiony na poziom %d.\n\r", level );
        }
	return;
    }
	...
	act( AT_IMMORT, "$n powoli wylania sie z nicosci.", ch, NULL, NULL, TO_ROOM );
	send_to_char( "Powoli pojawiasz sie z nicosci.\n\r", ch );
    }
    else
    {
	act( AT_IMMORT, "$n rozplywa sie w powietrzu.", ch, NULL, NULL, TO_ROOM );
	send_to_char( "Powoli rozplywasz sie w powietrzu.\n\r", ch );
...
void do_destroy( CHAR_DATA *ch, char *argument )
{
  ...
  if ( arg[0] == '\0' )
  {
      send_to_char( "Ktorego gracza plik ma zostac zniszczony?\n\r", ch );
      return;
  }
  ...
  name = capitalize( arg );
  sprintf( buf, "../player/%c/%s", tolower(arg[0]), name );
  sprintf( buf2, "../backup/player/%c/%s", tolower(arg[0]), name );
  if ( !rename( buf, buf2 ) )
  {
    AREA_DATA *pArea;
