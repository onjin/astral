...
char	*check_social_chan	args( ( CHAR_DATA *ch, char *command, char *argument ) );
...
bool check_pos( CHAR_DATA *ch, sh_int position )
{
	...
	{
	case POS_DEAD:
	    send_to_char( "Ciezko to uczynic gdy jest sie TRUPEM...\n\r", ch );
	    break;
	...
	case POS_RESTING:
	    send_to_char( "Taaaa... nie chce Ci sie ruszyc...\n\r", ch);
	    break;

	case POS_SITTING:
	    send_to_char( "Nie mozesz tego uczynic gdy siedzisz.\n\r", ch);
	    break;
...
/*	if ( ch->desc && ch->desc->original )
	...
	&&   !check_virtual_channels( ch, command, argument )
...
bool check_social( CHAR_DATA *ch, char *command, char *argument )
{
    ...
    {
	send_to_char( "Nie mozesz uzywac sociali!\n\r", ch );
	return TRUE;
    }
    ...
    {
    case POS_DEAD:
	send_to_char( "Lez spokojnie, jestes TRUPEM.\n\r", ch );
	return TRUE;

    case POS_INCAP:
    case POS_MORTAL:
	send_to_char( "Masz zbyt wiele ran by to uczynic.\n\r", ch );
	return TRUE;

    case POS_STUNNED:
	send_to_char( "Twoj umysl jest ogluszony.\n\r", ch );
	return TRUE;
...
/* Sociale na kanale Flash 24.04.1999 */
char *check_social_chan( CHAR_DATA *ch, char *command, char *argument )
{
    char arg[MAX_INPUT_LENGTH];
    CHAR_DATA *victim;
    SOCIALTYPE *social;
	...
	{
	    msg = act_string( social->others_no_arg, NULL, ch, NULL, ch, STRING_NONE);
	    return msg;
	}
	else
	    return FALSE;
	...
	if ( victim == ch )
	    ...
	    {
	        msg = act_string( social->others_auto, ch, ch, NULL, ch,  STRING_NONE );
	        return msg;
	    }
	    else if ( social->others_no_arg )
	    {
		msg = act_string( social->others_no_arg, NULL, ch, NULL, ch, STRING_NONE);
	        return msg;
	    }
	    ...
	    {
	    msg = act_string( social->others_found, victim, ch, NULL, victim , STRING_NONE );
	    return msg;
	    }
	    else if ( social->others_no_arg )
	    {
...
/* Sociale na kanale Flash koniec */
